...
- `preview` (`op` + operands) — observe, start, and drive **this session's dev preview** (`start`, `state`, `logs`, `screenshot`, `navigate` by route, …). Agents can boot via `op: "start"` (same path as the toolbar); stop stays human-only.
  The `preview` drive Chromium is a separate, origin-pinned **preview surface** (`preview:<sessionId>` in the browser registry): observations end with `Surface: preview (… pinned to <origin>) · URL: …`, and an origin escape points back at the `browser` tool for public URLs. Humans watch it live in the **Agent browser** pane (same screencast as public-web; the badge reads `preview`). The iframe preview pane is still the human's own view of the running app.
