...
let cachedConnConfig: ConnectionConfig | null = null;
let cachedAuthToken: AuthTokenRecord | null = null;
...
function writeConnectionConfig(config: ConnectionConfig) {
...
function readAuthToken(): AuthTokenRecord | null {
...
function writeAuthToken(record: AuthTokenRecord | null | undefined) {
...
function writeRemoteOrgs(orgs: RemoteOrg[]) {
...
ipcMain.handle('preview:pop-out', async (event, payload) => {
  ...
  if (existing && !existing.isDestroyed()) {
    try {
      ...
      console.warn(
        '[preview:pop-out] failed to reuse window, recreating:',
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    console.error(
      'Failed to start server, opening window anyway:',
      err instanceof Error ? err.message : err,
    );
  }
