...
export function phasesForRequest({ withGithub } = { withGithub: true }) {
  if (withGithub) return PROVISIONING_PHASES;
  return PROVISIONING_PHASES.filter((p) => !p.gh);
}

/** Fresh state — used on mount and on Retry. */
export function initialState({ withGithub } = { withGithub: true }) {
  const phases = phasesForRequest({ withGithub });
  return {
    phases: phases.map((p) => ({ ...p, status: 'pending', startedAt: null, finishedAt: null })),
    logs: [], // {line, at}
...
function applyPhase(state, event, ts) {
  const { phase, status, message } = event;
  const idx = state.phases.findIndex((p) => p.id === phase);
...
function applyLog(state, event, ts) {
...
function applyDone(state, event, ts) {
...
export function phaseTone(status) {
  switch (status) {
    case 'ok':
      return 'green';
    case 'started':
...
export function hasGithubFailure(state) {
  return state.phases.some((p) => p.gh && p.status === 'failed');
}
