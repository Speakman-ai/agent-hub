 ...
 */
function parseVersion(input: any) {
  ...
  const nums = parts.slice(0, 3).map((p: any) => {
    const n = Number(p);
    return Number.isFinite(n) && /^\d+$/.test(p) ? n : NaN;
  });
  if (nums.some((n: any) => Number.isNaN(n))) return null;
  return nums;
}
 ...
 */
export function compareSemver(a: any, b: any) {
