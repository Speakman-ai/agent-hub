...
/**
 * Merge a backfill/live batch without repeating rows that overlapped during a
 * reconnect. Records use their durable SQLite id as the cursor and sort key.
 * The returned `dropped` total combines server-reported slow-client drops and
 * local trimming, so a Logs screen can show an honest recovery indicator.
 */
