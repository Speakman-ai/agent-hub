...
describe('createDiffFileCountRefresher', () => {
  ...
  it('discards a stale earlier response that resolves AFTER a newer one', async () => {
    ...
    const refresh = createDiffFileCountRefresher({
      ...
      applyCount: (sid: any, count: any) => applied.push([sid, count]),
    });
  ...
  it('tracks sequences independently per session', async () => {
    const applied: any[] = [];
    const refresh = createDiffFileCountRefresher({
      fetchCount: async (sid: any) => (sid === 's1' ? 1 : 9),
      applyCount: (sid: any, count: any) => applied.push([sid, count]),
    });
    await refresh('s1');
    await refresh('s2');
...
describe('applyDiffCountWsEffect', () => {
  ...
  function harness(sessions: any) {
    ...
    return {
      refresh,
      bumpReloadToken,
      run: (event: any) => applyDiffCountWsEffect(event, { sessions, refresh, bumpReloadToken }),
    };
  }
