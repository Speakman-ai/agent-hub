...
describe('auditReport — DEFAULT_CATEGORIES', () => {
  it('exposes the expected five categories in storyboard order', () => {
    expect(DEFAULT_CATEGORIES.map((c) => c.id)).toEqual(['lint', 'tests', 'deps', 'auth', 'aws']);
  });

  it('weights sum to 100', () => {
    const sum = DEFAULT_CATEGORIES.reduce((n, c) => n + c.weight, 0);
    expect(sum).toBe(100);
  });
});
...
describe('auditReport — computeReadinessScore', () => {
  it('returns 100 when every weighted category is ok', () => {
    const cats = DEFAULT_CATEGORIES.map((c) => ({ ...c, status: 'ok' }));
    expect(computeReadinessScore(cats)).toBe(100);
  });

  it('returns 0 when every weighted category is fail', () => {
    const cats = DEFAULT_CATEGORIES.map((c) => ({ ...c, status: 'fail' }));
  ...
  it('returns null when every category is na', () => {
    const cats = DEFAULT_CATEGORIES.map((c) => ({ ...c, status: 'na' }));
...
describe('auditReport — normalizeReport', () => {
  ...
  it('clamps bad scores to 0..100', () => {
    expect(normalizeReport({ score: -5, categories: [] }).score).toBe(0);
    expect(normalizeReport({ score: 500, categories: [] }).score).toBe(100);
  });
  ...
  it('generates stable ids for findings/gaps without one', () => {
    ...
    expect(report.findings.map((f) => f.id)).toEqual(['f-0', 'f-1']);
    expect(report.gaps.map((g) => g.id)).toEqual(['g-0', 'g-1']);
  });
});
