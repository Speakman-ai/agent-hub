 ...
 */
export function inferHasMore(pageLength, pageSize = MESSAGES_PAGE_SIZE) {
 ...
 */
export function prependOlderMessages(prev, older) {
  const have = new Set(prev.map((m) => m.id));
  const fresh = (Array.isArray(older) ? older : []).filter((m) => m && !have.has(m.id));
 ...
 */
export function restoredScrollTop({ prevScrollTop, prevScrollHeight, newScrollHeight }) {
