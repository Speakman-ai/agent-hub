...
export function sortedColumnCards(cards: any, columnId: any) {
  return cards.filter((c: any) => c.column_id === columnId).sort(byPosition);
}
 ...
 */
export function isAfterMidpoint(activeRect: any, overRect: any) {
 ...
 */
export function resolveDropTarget(activeId: any, over: any, cards: any, activeRect?: any) {
...
export function computeMoveUpdates(
  ...
  const targetWithout = sortedColumnCards(cards, targetColumnId).filter(
    (c: any) => c.id !== activeId,
  );
  ...
  if (overCardId == null) {
    ...
    const overIdx = targetWithout.findIndex((c: any) => c.id === overCardId);
  ...
  const updates: any[] = [];
  newTargetOrder.forEach((c: any, idx: any) => {
    if (c.id === activeId) {
  ...
  if (sourceColumnId !== targetColumnId) {
    sortedColumnCards(cards, sourceColumnId)
      .filter((c: any) => c.id !== activeId)
      .forEach((c: any, idx: any) => {
        if (c.position !== idx) {
...
export function arrayMove(list: any, from: any, to: any) {
