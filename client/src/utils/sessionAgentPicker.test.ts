...
import { filterAgentsForPicker, groupAgentsByProject } from './sessionAgentPicker';
...
describe('sessionAgentPicker', () => {
  ...
  it('filterAgentsForPicker excludes inactive and roster ids', () => {
    ...
    expect(out.map((a: any) => a.id)).toEqual(['a2']);
  });
  ...
  it('groupAgentsByProject sorts groups and agents', () => {
    const groups = groupAgentsByProject(agents.filter((a: any) => a.active !== false));
    expect(groups.map((g: any) => g.projectName)).toEqual(['Apple', 'Zebra']);
    expect(groups[0].agents.map((a: any) => a.id)).toEqual(['a2']);
    expect(groups[1].agents.map((a: any) => a.id)).toEqual(['a1']);
  });
});
