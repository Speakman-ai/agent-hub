 ...
 */
export function formatSessionExport({ agent, messages, sessionEngine }: any) {
  const lines: any[] = [];
 ...
 * @returns {string} Formatted conversation text
 */
export function formatRoomExport({ room, messages }: any) {
  const lines: any[] = [];
 ...
 * @returns {string}
 */
export function buildNoteTitle({ kind, agent, room, now }: any) {
 ...
 */
export async function saveConversationAsNote({ api, projectId, title, content }: any) {
