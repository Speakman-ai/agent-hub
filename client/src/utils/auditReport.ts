 ...
 */
export function computeReadinessScore(categories: any = []) {
  let totalWeight = 0;
  let earned = 0;
...
function defaultWeightFor(id: any) {
  const def = DEFAULT_CATEGORIES.find((c: any) => c.id === id);
 ...
 *   else   → red   ("not ready")
 */
export function scoreBand(score: any) {
...
function defaultLabelFor(id: any) {
  const def = DEFAULT_CATEGORIES.find((c: any) => c.id === id);
...
function normalizeSeverity(s: any) {
  return s === 'info' || s === 'warn' || s === 'error' ? s : 'info';
}

function clamp(n: any, lo: any, hi: any) {
...
export function findingsByCategory(findings: any = []) {
  const map = new Map();
...
export function maxSeverity(findings: any = []) {
  let best = null;
  for (const f of findings) {
