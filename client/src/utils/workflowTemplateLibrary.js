...
export function getWorkflowTemplateMetaList() {
  return WORKFLOW_TEMPLATE_CATALOG.map(({ id, label, description }) => ({
    id,
    label,
    description,
  }));
 ...
 */
export function instantiateWorkflowTemplate(templateId, defaultAgentId, opts = {}) {
  const seed = WORKFLOW_TEMPLATE_CATALOG.find((s) => s.id === templateId);
  if (!seed) {
