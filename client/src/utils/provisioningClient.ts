 ...
 */
export async function provisionProject(payload: any) {
...
function readEnvNumber(name: any) {
  try {
...
function appendSinceQuery(wsUrl: any, since: any) {
 ...
 */
function logToolError({ action, code, summary, severity = 'soft', resolution = 'recovered' }: any) {
  try {
 ...
 * @returns {{ close: () => void }}
 */
export function subscribeProvisioningEvents(wsUrl: any, handlers: any) {
  const {
    onEvent,
    onClose,
    onError,
  ...
  const makeSocket = webSocketFactory || ((url: any) => new WebSocket(url));
  ...
  function attachSocket(url: any, { isReconnect }: any) {
    let socket: any;
    try {
