...
import { formatDuration } from '../hooks/useFinalizeRun.js';

export function computeStepDuration(step) {
...
export function labelForStepState(state) {
  switch (state) {
    case 'queued':
      return 'queued';
    case 'running':
...
export function stepIconFor(state) {
  switch (state) {
    case 'passed':
      return CheckCircle2;
    case 'failed':
...
export function stepToneFor(state) {
  switch (state) {
    case 'passed':
