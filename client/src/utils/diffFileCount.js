 ...
 * @returns {number}
 */
export function fileCountFromChangesSummary(summary) {
 ...
 */
export function setSessionFileCount(prev, sessionId, count) {
 ...
 * @returns {boolean}
 */
export function isWorktreeSession(session) {
 ...
 */
export function createDiffFileCountRefresher({ fetchCount, applyCount }) {
  ...
  return async function refreshDiffFileCount(sessionId) {
    if (!sessionId) return;
 ...
 */
export function applyDiffCountWsEffect(event, { sessions, refresh, bumpReloadToken }) {
