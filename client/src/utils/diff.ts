 ...
 */
export function shortenPath(filePath: any) {
  if (!filePath) return '';
 ...
 */
function parseCodexFileChanges(changes: any) {
  if (!changes.length) {
    return {
      filePath: '',
      action: 'Files',
...
function isDiffPlusLeader(line: any) {
...
function parseCursorEditStrategies(input: any) {
 ...
 */
export function parseDiffLines(tool: any, input: any) {
...
export function diffHasDisplayableLines(tool: any, input: any) {
  const { removals, additions } = parseDiffLines(tool, input);
  return removals.some((l: any) => l.trim()) || additions.some((l: any) => l.trim());
}
 ...
 */
export function isExplicitEmptyWrite(tool: any, input: any) {
