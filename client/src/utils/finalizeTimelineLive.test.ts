...
import { notifyFinalizeRunFromTimelineMessage } from './finalizeTimelineLive';
...
describe('notifyFinalizeRunFromTimelineMessage', () => {
  ...
  it('dispatches phase + completed events for ready_to_push timeline messages', () => {
    ...
    const phaseEvent = (window.dispatchEvent as any).mock.calls[0][0];
    ...
    const completedEvent = (window.dispatchEvent as any).mock.calls[1][0];
