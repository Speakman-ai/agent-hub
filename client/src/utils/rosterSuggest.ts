 ...
 */
export function suggestRoster({ tracks = DEFAULT_TRACKS, agents = [] }: any = {}) {
  return tracks.map((track: any) => {
    const match = agents.find((agent: any) => matchesTrack(agent, track));
    return {
      id: track.id,
      label: track.label,
...
function matchesTrack(agent: any, track: any) {
  ...
  return keywords.some((kw: any) => kw && haystack.includes(kw.toLowerCase()));
}
 ...
 * it "skip".
 */
export function initialRoster(suggestions: any = []) {
...
export function defaultAgentName(projectName: any, trackLabel: any) {
 ...
 */
export function initialRosterForCreate(suggestions: any = [], projectName: any = '') {
  return suggestions.map((s: any) => ({
    trackId: s.id,
    label: s.label,
    rationale: s.rationale,
 ...
 * Pass `null` to clear.
 */
export function setRosterAgent(roster: any, trackId: any, agentId: any) {
  return roster.map((row: any) => (row.trackId === trackId ? { ...row, agentId } : row));
}

/** Update the display name for a track's new agent (create flow). */
export function setRosterAgentName(roster: any, trackId: any, agentName: any) {
  return roster.map((row: any) => (row.trackId === trackId ? { ...row, agentName } : row));
}
 ...
 */
export function addCustomTrack(roster: any, { id, label }: any, projectName: any = '') {
  if (!id || !label) return roster;
  if (roster.some((r: any) => r.trackId === id)) return roster;
  return [
    ...roster,
    {
      trackId: id,
...
export function removeRosterTrack(roster: any, trackId: any) {
  return roster.filter((r: any) => r.trackId !== trackId);
}
 ...
 */
export function rosterToPayload(roster: any = [], { createAgents = false }: any = {}) {
  return {
    tracks: roster
      .filter((r: any) => r.label && r.trackId)
      .filter((r: any) => {
...
export function hasAnyAssignment(roster: any = []) {
  return roster.some((r: any) => !!r.agentId);
}

/** True when at least one track has a non-empty new-agent name (create flow). */
export function hasAnyNamedAgent(roster: any = []) {
  return roster.some((r: any) => (r.agentName || '').trim().length > 0);
}
