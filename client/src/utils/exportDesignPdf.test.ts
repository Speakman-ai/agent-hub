...
// don't run cleanly under jsdom. We mock both via (vi as any).mock so the util's own
...
import { exportDesignPdf } from './exportDesignPdf';

function fakeCanvas({ width = 800, height = 2000 }: any = {}) {
  return {
    width,
    height,
...
describe('exportDesignPdf', () => {
  ...
  beforeEach(() => {
    ...
    (addImageMock as any).mockClear();
    (addPageMock as any).mockClear();
    (saveMock as any).mockClear();
    (html2canvasMock as any).mockReset();
  ...
  it('loads the design via same-origin iframe and saves a PDF', async () => {
    (html2canvasMock as any).mockResolvedValue(fakeCanvas({ width: 800, height: 800 }));
  ...
  it('loads from srcBase (session worktree mount) when provided, not /design-files/', async () => {
    ...
    expect((fetchMock as any).mock.calls[0][0]).not.toContain('/design-files/');
    expect(saveMock!).toHaveBeenCalledTimes(1);
    expect((saveMock as any).mock.calls[0][0]).toBe('design-sess-1.pdf');
  });
  ...
  it('paginates long designs across multiple PDF pages', async () => {
    ...
    (html2canvasMock as any).mockResolvedValue(fakeCanvas({ width: 800, height: 4000 }));
    await exportDesignPdf({ designId: 'd-2', base: SAME_ORIGIN_BASE });

    expect((addImageMock as any).mock.calls.length).toBeGreaterThan(1);
    expect((addPageMock as any).mock.calls.length).toBe(
      (addImageMock as any).mock.calls.length - 1,
    );
    expect((saveMock as any).mock.calls[0][0]).toBe('design-d-2.pdf');
  });

  it('removes the hidden iframe after a successful export', async () => {
    (html2canvasMock as any).mockResolvedValue(fakeCanvas());
  ...
  it('removes the hidden iframe even if rendering fails', async () => {
    (html2canvasMock as any).mockRejectedValue(new Error('boom'));
  ...
  it('sanitizes unsafe characters in the filename', async () => {
    ...
    const savedAs = (saveMock as any).mock.calls[0][0];
  ...
  it('surfaces a 404 from the pre-flight probe with a clear message', async () => {
    (html2canvasMock as any).mockResolvedValue(fakeCanvas());
    (fetchMock as any).mockResolvedValueOnce({ ok: false, status: 404, text: async () => '' });
  ...
  it('surfaces a network error from the pre-flight probe', async () => {
    (html2canvasMock as any).mockResolvedValue(fakeCanvas());
    (fetchMock as any).mockRejectedValueOnce(new Error('net down'));
  ...
  it('falls back to srcdoc when the initial iframe load exposes no contentDocument', async () => {
    ...
    expect(saveMock!).toHaveBeenCalledTimes(1);
    expect((saveMock as any).mock.calls[0][0]).toBe('design-d-5.pdf');
  ...
  it('delegates PDF bytes to Electron saveDesignPdf instead of jsPDF.save', async () => {
    const saveDesignPdf = vi.fn().mockResolvedValue({ ok: true } as any);
    ...
    const arg = (saveDesignPdf as any).mock.calls[0][0];
