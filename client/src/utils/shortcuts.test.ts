...
describe('getDefaultShortcuts', () => {
  it('returns the web list in the browser profile', () => {
    ...
    expect(getDefaultShortcuts(false).find((s: any) => s.id === 'new-session')?.binding).toBe(
      'Mod+Alt+N',
    );
  });
  ...
  it('returns the electron list in the desktop profile', () => {
    ...
    expect(getDefaultShortcuts(true).find((s: any) => s.id === 'new-session')?.binding).toBe(
      'Mod+Alt+N',
    );
    expect(getDefaultShortcuts(true).find((s: any) => s.id === 'go-to-skills')?.binding).toBe(
      'Mod+Alt+Shift+K',
    );
  });
});
...
describe.each([
...
])('%s default shortcut registry', (_label: any, list: any) => {
  ...
  it('has no duplicate bindings', () => {
    const bindings = list.map((s: any) => s.binding);
...
describe('toggle-microphone shortcut', () => {
  const entry = WEB_DEFAULT_SHORTCUTS.find((s: any) => s.id === 'toggle-microphone');
