...
function inferCronFromApi(row: any) {
  const ex =
...
export function workflowDraftSnapshot(d: any) {
 ...
 */
export function workflowFromApi(apiWorkflow: any) {
 ...
 */
export function draftToPutBody(draft: any) {
  ...
  const steps = (draft.steps || []).map((s: any, i: any) => ({
    id: s.id || undefined,
    agentId: s.agent_id,
    stepProjectId: (() => {
