...
export function useFinalizeRun({
  ...
  useEffect(() => {
    ...
    const onActiveSeconds = (event) => {
      const detail = event?.detail || {};
      if (!matchesRun(detail)) return;
      const value = detail.active_seconds_consumed;
      if (typeof value !== 'number') return;
      setRun((prev) => (prev ? { ...prev, active_seconds_consumed: value } : prev));
    };
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => setWallTick((t) => t + 1), 1000);
  ...
  const steps = useMemo(
    () => Array.from(stepsByIndex.values()).sort((a, b) => a.index - b.index),
    [stepsByIndex],
  );
