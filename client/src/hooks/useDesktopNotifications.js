 ...
 */
export function sendDesktopNotification({ title, body, type = 'info', silent = false }) {
...
export function useDesktopNotifications() {
  const supported = useRef(
    isElectron()
      ? window.electronAPI.isNotificationSupported?.() !== false
      : isBrowserNotificationAvailable(),
  );

  const notify = useCallback(
    /** @param {NotificationOptions} options */
    ({ title, body, type = 'info', silent = false }) => {
