...
import { captureSnapshot, collectSnapshotStrings, sanitizeMaskedStyle } from './capture';
...
describe('capture-time masking: secrets never hit the wire', () => {
  ...
  it('preserves masked value length so replay geometry survives', () => {
    ...
    const cc = snapshot.children
      .find((c: any) => c.attributes?.id === 'checkout')
      .children.find((c: any) => c.attributes?.id === 'cc');
    expect(cc.masked).toBe(true);
    expect((cc as any).value).toBe('*'.repeat(CC_NUMBER.length));
  });
  ...
  it('keeps safe layout declarations from a masked element style', () => {
    const root = document.createElement('div');
    (root as any).innerHTML =
      `<input id="s" data-rum-mask style="--secret: 4111111111111111; display: none; width: 120px" />`;
    const snapshot = captureSnapshot(root.querySelector('#s'));
    expect((snapshot as any).attributes.style).toBe('display: none; width: 120px');
  });
  ...
  it('keeps inline style verbatim on UNMASKED nodes (scoping check)', () => {
    const root = document.createElement('div');
    (root as any).innerHTML = `<div id="u" style="--brand: teal; display: grid">ok</div>`;
    ...
    expect((snapshot as any).attributes.style).toContain('--brand: teal');
  });
  ...
  it('keeps safe structural attributes verbatim on a masked element', () => {
    ...
    const root = document.createElement('div');
    (root as any).innerHTML =
      `<input id="keepme" class="form-control" type="text" data-rum-mask title="leak" />`;
  ...
  it('masks a nested input under a masked subtree even when it carries an unmask hint', () => {
    ...
    const root = document.createElement('div');
    (root as any).innerHTML = `<div data-rum-mask><input id="n" data-rum-unmask /></div>`;
    const inp = root.querySelector('#n');
    (inp as any).value = 'leakme';
