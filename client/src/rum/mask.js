...
function hasFlag(el, attr, klass) {
 ...
 */
export function isAlwaysMaskedField(el) {
  if (!el || !el.tagName) return false;
  const tag = el.tagName.toLowerCase();
  ...
  return ALWAYS_MASK_PATTERNS.some((re) => re.test(haystack));
}
 ...
 */
export function classifyElement(el, options = {}) {
 ...
 */
export function maskValue(value, maskChar = DEFAULT_MASK_OPTIONS.maskChar) {
 ...
 */
export function shouldMaskValue(el, options = {}) {
