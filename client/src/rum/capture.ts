...
import { classifyElement, isBlocked, isFormControl, maskValue } from './mask';
 ...
 */
export function sanitizeMaskedStyle(styleText: any) {
  if (!styleText) return '';
  const kept: any[] = [];
...
function serializeAttributes(el: any, masked: any, maskChar: any) {
...
function readControlValue(el: any) {
...
function readAttrValue(el: any) {
  const v = el.getAttribute?.('value');
  return typeof v === 'string' ? v : '';
}

function serializeElement(el: any, options: any, maskChar: any, inheritedMask: any = false) {
  if (isBlocked(el)) {
...
function serializeNode(node: any, options: any, maskChar: any, inheritedMask: any) {
  if (!node) return null;
 ...
 */
export function captureSnapshot(root: any, options: any = {}) {
 ...
 */
export function collectSnapshotStrings(snapshot: any, acc: any = []) {
