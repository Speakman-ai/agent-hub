...
(vi as any).mock('./utils/api.js', async (importOriginal: any) => {
  ...
  return {
    ...
    api: {
      ...
      getSessions: vi.fn((agentId: any) => Promise.resolve(sessionsByAgent[agentId] || [])),
...
(vi as any).mock('./components/Sidebar.jsx', () => ({
  default: (props: any) => {
    if (props.onFocusSession) (globalThis as any).__ahTestFocusSession = props.onFocusSession;
    return <div data-testid="sidebar-stub" />;
  },
}));

(vi as any).mock('./components/TopBar.jsx', () => ({
  default: ({ accentColor, agent }: any) => (
    <div
...
describe('App — project accent on cross-project session switch', () => {
  beforeEach(() => {
    ...
    (globalThis as any).__ahTestFocusSession = undefined;
  });
  ...
  it('updates TopBar accent when focusing a session in another project', async () => {
    ...
    await waitFor(() => expect(typeof (globalThis as any).__ahTestFocusSession).toBe('function'));

    await act(async () => {
      (globalThis as any).__ahTestFocusSession('a-hub', 's-hub');
    });
    ...
    await act(async () => {
      (globalThis as any).__ahTestFocusSession('a-st', 's-st');
    });
