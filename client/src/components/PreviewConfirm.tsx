 ...
 * for the gate.
 */
export default function PreviewConfirm({ detected, onConfirm, onSkip }: any) {
  ...
  const addRoute = () => {
    ...
    setForm((prev: any) => ({ ...prev, captureRoutes: [...prev.captureRoutes, trimmed] }));
    setRouteDraft('');
  };

  const removeRoute = (idx: any) => {
    setForm((prev: any) => ({
      ...prev,
      captureRoutes: prev.captureRoutes.filter((_: any, i: any) => i !== idx),
    }));
  };
        ...
        <div className="space-y-3 pl-6">
          <div>
            ...
            <input
              ...
              onChange={(e: any) => setField('startScript', e.target.value)}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5 mb-2" data-testid="preview-confirm-routes">
              {form.captureRoutes.map((route: any, idx: any) => (
                <span
                  key={`${route}-${idx}`}
            ...
            <div className="flex gap-2">
              <input
                type="text"
                value={routeDraft}
                onChange={(e: any) => setRouteDraft(e.target.value)}
                onKeyDown={(e: any) => {
                  if (e.key === 'Enter') {
                    e.preventDefault();
                    addRoute();
 ...
 */
export function buildPreviewPatch(confirmed: any) {
