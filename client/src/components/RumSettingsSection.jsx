...
export default function RumSettingsSection({ projects = [], onOpenSession, showToast }) {
  ...
  const reloadClients = useCallback(async (pid) => {
    if (!pid) return;
            ...
            <dl className="grid grid-cols-[max-content_1fr] gap-x-3 gap-y-1.5 text-xs">
              ...
              <dd className="text-gray-300">
                {draft.cspHits?.length ? (
                  <span className="font-mono">{draft.cspHits.map((h) => h.path).join(', ')}</span>
                ) : (
