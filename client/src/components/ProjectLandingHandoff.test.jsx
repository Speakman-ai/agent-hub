...
import ProjectLandingHandoff, {
  ...
  summarizeStack,
} from './ProjectLandingHandoff.jsx';
...
describe('ProjectLandingHandoff — pure helpers', () => {
  it('pickTopFindings sorts by severity and limits to max', () => {
    ...
    expect(top.map((f) => f.id)).toEqual(['b', 'd', 'c']);
  });
...
describe('ProjectLandingHandoff — rendering', () => {
  it('renders project name, repo link, and band chip', () => {
    ...
    expect(repo.textContent).toContain('github.com/acme/my-cool-app');
    expect(screen.getByTestId('pl-summary-band').textContent).toMatch(/Needs work/);
    expect(screen.getByTestId('pl-summary-band').textContent).toMatch(/72/);
  });

  it('shows stack and integrations as chips', () => {
    render(<ProjectLandingHandoff {...defaultProps()} />);
    expect(screen.getByTestId('pl-stack').textContent).toMatch(/React/);
    expect(screen.getByTestId('pl-stack').textContent).toMatch(/Node\.js/);
    expect(screen.getByTestId('pl-integrations').textContent).toMatch(/GitHub/);
    expect(screen.getByTestId('pl-integrations').textContent).toMatch(/Database/);
    expect(screen.getByTestId('pl-apptype').textContent).toBe('Web app');
  });
...
describe('ProjectLandingHandoff — callbacks', () => {
  it('Chat button fires onStartChat with agentId + trackId', () => {
    ...
    fireEvent.click(screen.getByTestId('pl-chat-architect'));
  ...
  it('Open-project CTA fires onOpenProject with projectId + repoUrl', () => {
    ...
    fireEvent.click(screen.getByTestId('pl-next-open'));
  ...
  it('Starter-task CTAs fire onOpenStarterTask with task.type', () => {
    ...
    fireEvent.click(screen.getByTestId('pl-next-kanban'));
    expect(onOpenStarterTask).toHaveBeenCalledWith({
      projectId: 'proj-1',
      task: { type: 'kanban' },
    });
    fireEvent.click(screen.getByTestId('pl-next-skills'));
    expect(onOpenStarterTask).toHaveBeenCalledWith({
      projectId: 'proj-1',
      task: { type: 'skills' },
    });
  ...
  it('primary "Brief {lead}" CTA surfaces when a preferred lead track is assigned', () => {
    ...
    expect(brief.textContent).toMatch(/Hub Lead/);
  ...
  it('close button fires onClose', () => {
    ...
    fireEvent.click(screen.getByTestId('pl-close'));
