...
} from '../utils/coordinationBlocks';
import { deriveAssistantTailOutcome } from '../utils/assistantTailOutcome';
import { stripAssistantControlBlocks } from '../utils/controlBlocks';
...
function SessionTail({
  ...
  const hasBrowserActivity = (effectiveEvents ?? []).some(
    ({ event }: any) => event?.type === 'browser_tool_activity',
  );
  if (
    blocks.length === 0 &&
...
export function eventsToBlocks(events: any) {
  ...
  let textBuf: any = null; // { partials: '', final: '' }
  let exploredBuf: any = null; // { items: [{ use, result }] }
  let thinkingBuf = '';
...
function truncateStatusDetail(s: any, max: any) {
  if (!s) return '';
...
function SystemBanner({ system, messageEngine, messageModel }: any) {
...
function trimmedModelId(v: any) {
  if (v == null) return '';
  const s = String(v).trim();
  return s;
}

function ThinkingBlock({ text, defaultOpen }: any) {
...
function truncateOneLine(s: any, max: any) {
  if (!s) return '';
 ...
 */
export function DiffView({ tool, input, toolResult }: any) {
  ...
  const addedCount = additions.filter((l: any) => l.trim()).length;
  const removedCount = removals.filter((l: any) => l.trim()).length;
      ...
      {!hasLines ? (
        ...
        <>
          <div className="overflow-x-auto">
            {previewLines.slice(0, DIFF_PREVIEW_LINES).map((line: any, i: any) => (
              <div
                key={`p${i}`}
 ...
 */
export function ExploredChip({ items, defaultOpen }: any) {
  const [open, setOpen] = useState(defaultOpen ?? false);
...
export function TodoListCard({ use, result, defaultOpen }: any) {
 ...
 */
function PlanProposalCard({ use, result, defaultOpen }: any) {
...
function SubagentCard({ use, result, defaultOpen }: any) {
  const [open, setOpen] = useState(defaultOpen ?? false);
 ...
 * correct reason text.
 */
function buildMalformedHandoffProps(malformed: any, sessionHandoffs: any) {
  const serverRow = Array.isArray(sessionHandoffs)
    ? sessionHandoffs.find(
        (h: any) => h?.status === 'failed' && (!h?.to_agent_id || h.to_agent_id === '(unknown)'),
      )
    : null;
 ...
 */
function pickHandoffRow(block: any, rows: any) {
  ...
  return (
    rows.find((r: any) => r.status === 'delivered' && match(r)) ||
    rows.find((r: any) => match(r)) ||
    (rows.length === 1 ? rows[0] : null)
  );
}

function ResultFooter({ result }: any) {
  const parts: any[] = [];
...
function CheckpointBlock({ event }: any) {
  return (
...
function RateLimitBlock({ event }: any) {
...
function ErrorBlock({ message }: any) {
  return (
...
function UnknownBlock({ event }: any) {
  return (
