...
beforeEach(() => {
  ...
  (api.getMyGrokAuth as any).mockResolvedValue(defaultPasteAuth());
  (api.getMyGrokBrowserAuth as any).mockResolvedValue(defaultBrowserAuth());
...
describe('MyGrokAuthSection', () => {
  ...
  it('renders the device user-code and verification URL after start, and copies to clipboard', async () => {
    (api.startMyGrokDeviceLogin as any).mockResolvedValue({
      ok: true,
      loginId: 'd1',
  ...
  it('stops polling once uiStatus=authenticated', async () => {
    ...
    (api.getMyGrokBrowserAuth as any).mockResolvedValueOnce(defaultBrowserAuth());
    (api.getMyGrokBrowserAuth as any).mockResolvedValueOnce(defaultBrowserAuth());
    (api.getMyGrokBrowserAuth as any).mockResolvedValue(
      defaultBrowserAuth({
  ...
  it('falls back to a paste-key save when no browser is available', async () => {
    (api.putMyGrokAuth as any).mockResolvedValue(
    ...
    fireEvent.change(input as any, { target: { value: 'xai-test-key' } } as any);
