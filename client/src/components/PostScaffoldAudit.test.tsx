...
import PostScaffoldAudit from './PostScaffoldAudit';
...
describe('PostScaffoldAudit', () => {
  ...
  it('renders the audit report when the load succeeds', async () => {
    ...
    expect((screen.getByTestId('audit-score-value') as any).textContent).toBe('88');
  });
  ...
  it('falls back to default tracks with seeded agent names when suggest fails', async () => {
    ...
    expect((screen.getByTestId('roster-agent-name-frontend') as any).value).toBe(
      'Scrabble Frontend',
    );
  });
  ...
  it('confirm button is disabled until a track has an agent name', async () => {
    ...
    fireEvent.change(screen.getByTestId('roster-agent-name-architect' as any), {
      target: { value: '   ' },
    });
  ...
  it('surfaces save errors without invoking onConfirmed', async () => {
    ...
    expect((screen.getByTestId('psa-save-error') as any).textContent).toMatch(/db down/);
  });
  ...
  it('refresh replaces the report with the new payload', async () => {
    ...
    await waitFor(() => {
      expect((screen.getByTestId('audit-score-value') as any).textContent).toBe('30');
    });
    await act(async () => {
      fireEvent.click(screen.getByTestId('audit-refresh' as any) as any);
    });
    await waitFor(() => {
      expect((screen.getByTestId('audit-score-value') as any).textContent).toBe('95');
    });
  ...
  it('refresh that returns a jobId falls back to re-fetching the report', async () => {
    ...
    await waitFor(() => {
      expect((screen.getByTestId('audit-score-value') as any).textContent).toBe('40');
    });
    await act(async () => {
      fireEvent.click(screen.getByTestId('audit-refresh' as any) as any);
    });
    await waitFor(() => {
      expect((screen.getByTestId('audit-score-value') as any).textContent).toBe('77');
    });
