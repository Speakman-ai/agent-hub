 ...
 */
function DelegationPanel({ delegations, onCancel, sessionId, throttled }: any) {
  ...
  const allDone = delegations.every(
    (d: any) => d.status === 'done' || d.status === 'error' || d.status === 'cancelled',
  );
  const hasErrors = delegations.some((d: any) => d.status === 'error');
  const doneCount = delegations.filter((d: any) => d.status === 'done').length;

  const toggleAgent = (delegationId: any) => {
