...
function err(body: any, status: any = 400) {
...
describe('OpenProjectWizard — dual-source GitHub detection', () => {
  ...
  async function triggerDetection() {
    ...
    fireEvent.click(retryBtn as any);
  }
  ...
  it('saves a trimmed PAT via /api/auth/github/connect-token and refetches status', async () => {
    ...
    fireEvent.click(
      screen.getByRole('button', { name: /Use a personal access token/i } as any) as any,
    );
  ...
  it('surfaces a server-side rejection of an invalid token without flipping to the connected state', async () => {
    ...
    fireEvent.click(
      await screen.findByRole('button', { name: /Paste a personal access token/i } as any),
    );
