...
import { scoreBand, findingsByCategory, maxSeverity } from '../utils/auditReport';
 ...
 */
export default function AuditReport({ report, onRefresh }: any) {
  if (!report) {
    return (
      <section
...
function ScoreHeader({ score, band, onRefresh }: any) {
  const tone =
    band === 'green'
...
function CategoryList({ categories, grouped }: any) {
...
function CategoryRow({ category, findings }: any) {
...
function severityBadgeTone(sev: any) {
  switch (sev) {
    case 'error':
...
function FindingsList({ findings }: any) {
...
function FindingIcon({ severity }: any) {
