...
import DesignView, { clampSplitPct } from './DesignView';
...
(vi as any).mock('../utils/exportDesignPdf.js', () => ({
...
(vi as any).mock('../utils/api.js', () => ({
  api: {
    ...
    putMyAgentModelOverride: vi.fn((_id: any, body: any) =>
...
describe('DesignView', () => {
  ...
  beforeEach(() => {
    ...
    window.matchMedia = vi.fn((query: any) => ({
      matches: false,
      media: query,
      addEventListener: (_event: any, handler: any) => matchMediaListeners.push(handler),
      removeEventListener: () => {},
      addListener: () => {},
      removeListener: () => {},
      onchange: null,
  ...
  it('does not send fetch with credentials: include (would break cross-origin remote mode)', async () => {
    ...
    const initArg = (globalThis.fetch as any).mock.calls[0][1];
  ...
  it('re-fetches index.html when reloadToken increments', async () => {
    ...
    expect((globalThis.fetch as any).mock.calls[0][0]).toContain('v=0');

    rerender(<DesignView {...baseProps} reloadToken={1} />);
    await waitFor(() => {
      expect(globalThis.fetch).toHaveBeenCalledTimes(2);
    });
    expect((globalThis.fetch as any).mock.calls[1][0]).toContain('v=1');
  });
  ...
  it('sends a design_chat WS message when the user submits the composer', () => {
    ...
    fireEvent.change(textbox, { target: { value: 'purple gradient hero' } } as any);
    fireEvent.keyDown(textbox, { key: 'Enter', shiftKey: false } as any);
  ...
  it('shows a Cancel button that emits design_cancel while processing', () => {
    ...
    fireEvent.click(cancelBtn as any);
  ...
  describe('resizable split pane', () => {
    ...
    it('shrinks the chat pane when ArrowLeft is pressed on the resizer', () => {
      ...
      act(() => {
        resizer.focus();
        fireEvent.keyDown(resizer, { key: 'ArrowLeft' } as any);
      });
    ...
    it('Home key snaps chat pane to its minimum width and End to its maximum', () => {
      ...
      const resizer = screen.getByTestId('design-split-resizer');
      act(() => {
        fireEvent.keyDown(resizer, { key: 'Home' } as any);
      });
      expect(resizer.getAttribute('aria-valuenow')).toBe('15');
      act(() => {
        fireEvent.keyDown(resizer, { key: 'End' } as any);
      });
    ...
    it('persists the split percentage to localStorage after resizing', () => {
      ...
      const resizer = screen.getByTestId('design-split-resizer');
      act(() => {
        fireEvent.keyDown(resizer, { key: 'ArrowRight', shiftKey: true } as any); // +5 → 55
      });
    ...
    it('Enter and Space reset the split to the default of 50', () => {
      ...
      expect(resizer.getAttribute('aria-valuenow')).toBe('30');
      act(() => {
        fireEvent.keyDown(resizer, { key: 'Enter' } as any);
      });
      expect(resizer.getAttribute('aria-valuenow')).toBe('50');
      act(() => {
        fireEvent.keyDown(resizer, { key: 'ArrowLeft' } as any); // 48
      });
      expect(resizer.getAttribute('aria-valuenow')).toBe('48');
      act(() => {
        fireEvent.keyDown(resizer, { key: ' ' } as any);
      });
      expect(resizer.getAttribute('aria-valuenow')).toBe('50');
    });
    ...
    it('does not apply inline flexBasis styles on mobile viewports (<768px)', () => {
      ...
      expect((chatPane as any).style.flexBasis).toBe('');
    });
    ...
    it('applies inline flexBasis styles on desktop viewports (≥768px)', () => {
      ...
      const resizer = screen.getByTestId('design-split-resizer');
      const chatPane = resizer.previousElementSibling;
      expect((chatPane as any).style.flexBasis).toBe('70%');
    });
  });

  describe('PDF export', () => {
    beforeEach(() => {
      (exportDesignPdf as any).mockClear();
    });
    ...
    it('invokes exportDesignPdf with the design id and name when clicked', async () => {
      ...
      const call = (exportDesignPdf as any).mock.calls[0][0];
  ...
  describe('Forward to agent', () => {
    beforeEach(() => {
      (api.forwardDesign as any).mockClear();
    });
    ...
    it('opens the modal and calls forwardDesign when an agent is chosen and submitted', async () => {
      ...
      fireEvent.click(screen.getByTestId('design-forward-open' as any) as any);
      expect(await screen.findByTestId('forward-design-modal-backdrop')).toBeInTheDocument();
      fireEvent.click(screen.getByText('Lead' as any) as any);
      fireEvent.click(screen.getByRole('button', { name: 'Forward' } as any) as any);
      await waitFor(() => {
  ...
  describe('per-user model override reconciliation on engine change', () => {
    ...
    beforeEach(() => {
      (api.updateDesign as any).mockClear();
      (api.deleteMyAgentModelOverride as any).mockClear();
      (api.getModelConfig as any).mockClear();
      (api.getMyAgentModelOverrides as any).mockClear();
    });

    it('clears an override that is incompatible with the newly selected engine', async () => {
      (api.getModelConfig as any).mockResolvedValueOnce(TWO_ENGINE_CONFIG);
      (api.getMyAgentModelOverrides as any).mockResolvedValueOnce({
        agentModelOverrides: { __design_studio__: 'claude-sonnet-4-6' },
      });
      (api.updateDesign as any).mockResolvedValueOnce({ id: 'd-1', agent_engine: 'codex-cli' });
      (api.deleteMyAgentModelOverride as any).mockResolvedValueOnce({ agentModelOverrides: {} });
    ...
    it('does not clear when there is no per-user model override to reconcile', async () => {
      (api.getModelConfig as any).mockResolvedValueOnce(TWO_ENGINE_CONFIG);
      (api.getMyAgentModelOverrides as any).mockResolvedValueOnce({ agentModelOverrides: {} });
      (api.updateDesign as any).mockResolvedValueOnce({ id: 'd-1', agent_engine: 'codex-cli' });
