...
function CommitDetailView({ projectId, sha, onBack, onOpenCommit }: any) {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then(setDetail)
      .catch((err: any) => setError(String(err?.message || err || 'Failed to load commit')));
  }, [projectId, sha]);
...
function CommitRow({ commit, onOpen }: any) {
  return (
    <button
      type="button"
      onClick={onOpen}
...
export default function RepositoryPage({ projectId, project, onOpenPulls, onToast }: any) {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((d: any) => setOpenPrCount(Array.isArray(d?.pulls) ? d.pulls.length : null))
