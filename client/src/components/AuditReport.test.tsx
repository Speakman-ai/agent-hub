...
import { normalizeReport } from '../utils/auditReport';
...
describe('AuditReport', () => {
  ...
  it('renders score and band for a green report', () => {
    ...
    expect((screen.getByTestId('audit-score-value') as any).textContent).toBe('100');
    const band = screen.getByTestId('audit-score-band');
    expect((band as any).textContent).toBe('Ready');
  ...
  it('renders amber band for middling score', () => {
    ...
    render(<AuditReport report={r} />);
    expect((screen.getByTestId('audit-score-band') as any).textContent).toBe('Needs work');
  ...
  it('renders red band for failing score', () => {
    ...
    render(<AuditReport report={r} />);
    expect((screen.getByTestId('audit-score-band') as any).textContent).toBe('Not ready');
  ...
  it('renders em-dash when score is null', () => {
    ...
    render(<AuditReport report={r} />);
    expect((screen.getByTestId('audit-score-value') as any).textContent).toBe('—');
  });
  ...
  it('shows a severity badge on a category when findings reference it', () => {
    ...
    expect((badge as any).textContent).toContain('2');
    expect((badge as any).textContent.toLowerCase()).toContain('error');
  });
