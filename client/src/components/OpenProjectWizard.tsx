...
function deriveIdFromName(name: any) {
  return name
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-|-$/g, '');
...
function StepIndicator({ currentStep }: any) {
  return (
    <div className="flex items-center justify-center gap-3 mb-6">
      {STEP_LABELS.map((label: any, i: any) => {
        const stepNum = i + 1;
...
export default function OpenProjectWizard({
  ...
  const [cloneError, setCloneError] = useState<any>(null);
  const [showTargetBrowser, setShowTargetBrowser] = useState(false);
  const cloneIdRef = useRef<any>(null);
  ...
  const [repoInfo, setRepoInfo] = useState<any>(null); // { hasRemote, owner, repo, url }
  const [repoOwner, setRepoOwner] = useState('');
  const [repoName, setRepoName] = useState('');
  const [testResult, setTestResult] = useState<any>(null); // { ok, repoInfo } or { ok: false, error }
  ...
  const [oauthError, setOauthError] = useState<any>(null);
  ...
  const analyzeIdRef = useRef<any>(null);
  ...
  useEffect(() => {
    const handler = (e: any) => {
      ...
      if (data.type === 'analyze-progress') {
        if (data.message) {
          setProgressLog((prev: any) => [...prev.slice(-19), data.message]);
        } else if (data.chunk || data.text) {
          setProgressText((prev: any) => prev + (data.chunk || data.text || ''));
        }
      }
      if (data.type === 'analyze-complete') {
        ...
        if (normalized.agents.length) {
          ...
          normalized.agents.forEach((a: any, i: any) => {
            agentMap[i] = true;
          });
  ...
  useEffect(() => {
    const handler = (e: any) => {
      ...
      if (data.type === 'clone-progress') {
        setCloneLog((prev: any) => [...prev.slice(-29), data.message]);
      }
  ...
  const handleCreate = useCallback(async () => {
    setCreating(true);
    try {
      const agents = (analysisResult?.agents || []).filter((_: any, i: any) => selectedAgents[i]);
    ...
    >
      ...
      >
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  {isElectronLocal ? (
                    <button
                      onClick={async () => {
                        const dir = await (window.electronAPI as any)?.selectDirectory?.();
                        if (dir) setPath(dir);
                      }}
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <input
                      type="text"
                      value={cloneTarget}
                      onChange={(e: any) => setCloneTarget(e.target.value)}
            ...
            <div>
              ...
              >
                {progressLog.length > 0
                  ? progressLog.map((line: any, i: any) => (
                      <div
                        key={i}
                        className={
                ...
                {analysisResult.techStack?.length > 0 && (
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {analysisResult.techStack.map((tech: any) => (
                        <span
                          key={tech}
                ...
                {analysisResult.agents?.length > 0 && (
                  <div>
                    ...
                    <div className="space-y-2">
                      {analysisResult.agents.map((agent: any, i: any) => {
                        const isLead =
          ...
          <div className="space-y-4">
            ...
            {detectedPreview && (
              <PreviewConfirm
                ...
                onConfirm={(form: any) => setPreviewDecision(form)}
                    ...
                    return (
                      ...
                      >
                        <input
                          ...
                          onChange={() =>
                            setSelectedAgents((prev: any) => ({ ...prev, [i]: !prev[i] }))
                          }
            ...
            <div>
              ...
              <textarea
                ...
                onChange={(e: any) =>
                  setContextFiles((prev: any) => ({ ...prev, [activeTab]: e.target.value }))
                }
                rows={8}
        ...
        <ServerBrowser
          ...
          onSelect={(dir: any) => setPath(dir)}
          initialPath={path || ''}
        />
        <ServerBrowser
          isOpen={showTargetBrowser}
          onClose={() => setShowTargetBrowser(false)}
          onSelect={(dir: any) => setCloneTarget(dir)}
