...
import FinalizeChecksRoundBlock from './FinalizeChecksRoundBlock';

(vi as any).mock('../FinalizeStepLogModal.jsx', () => ({
  default: ({ open, stepName }: any) =>
    open ? <div data-testid="log-modal">{stepName}</div> : null,
}));
...
describe('FinalizeChecksRoundBlock', () => {
  it('renders step rows and opens log modal', async () => {
    ...
    fireEvent.click(screen.getByText('test' as any) as any);
