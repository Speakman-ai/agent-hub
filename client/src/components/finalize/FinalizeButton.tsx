 ...
 */
export function pushConfirmMessage(isResolveSession: any) {
...
export default function FinalizeButton({
  ...
  const [worktreeHeadSha, setWorktreeHeadSha] = useState<any>(null);
  const optimisticTimerRef = useRef<any>(null);
  ...
  useEffect(() => {
    let cancelled = false;
    fetchGithubConnected().then((connected: any) => {
  ...
  const headFresh = (validatedHeadSha: any) => {
    if (!worktreeHeadSha) return true;
    return !!validatedHeadSha && validatedHeadSha === worktreeHeadSha;
  };

  const phaseFresh = (summary: any) =>
    phasePassed(summary) && headFresh(summary?.validated_head_sha);
  ...
  const noChangesOr = (active: any) => (!canShip ? noChangesHint : active);
  const activeSuffix =
