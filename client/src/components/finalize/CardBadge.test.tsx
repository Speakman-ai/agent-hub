...
describe('<CardBadge />', () => {
  ...
  it('renders phase label and active-time as the primary metric', () => {
    ...
    expect((screen.getByTestId('finalize-card-badge-label') as any).textContent).toBe('reviewing');
    expect((screen.getByTestId('finalize-card-badge-active') as any).textContent).toMatch(
      /4m active/,
    );
  });
  ...
  it('marks the badge as paused when status is dispatching', () => {
    ...
    expect((screen.getByTestId('finalize-card-badge-label') as any).textContent).toBe(
      'awaiting fix',
    );
  });
  ...
  it('renders a pushed badge after success', () => {
    ...
    render(<CardBadge sessionId="s1" />);
    expect((screen.getByTestId('finalize-card-badge-label') as any).textContent).toBe('pushed');
  });
