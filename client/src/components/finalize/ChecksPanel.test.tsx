...
function setHook(overrides: any = {}) {
...
beforeEach(() => {
  (useFinalizeRun as any).mockReset();
});
...
describe('<ChecksPanel />', () => {
  ...
  it('renders the phase pill and both active and wall clocks distinctly labelled', () => {
    ...
    expect((screen.getByTestId('finalize-run-phase-pill') as any).textContent).toMatch(/reviewing/);
    ...
    expect((active as any).textContent).toMatch(/active/);
    expect((active as any).textContent).toMatch(/4m/);
    expect((wall as any).textContent).toMatch(/wall/);
    expect((wall as any).textContent).toMatch(/6m/);
  ...
  it('surfaces the pause indicator when the run is waiting on session turn-end', () => {
    ...
    expect((screen.getByTestId('finalize-run-pause-indicator') as any).textContent).toMatch(
  ...
  it('dispatches log modal open when view logs is clicked', async () => {
    ...
    fireEvent.click(screen.getByTestId('finalize-step-jump' as any) as any);
  ...
  it('renders trigger source = "agent block" when triggered from inside a session', () => {
    ...
    render(<ChecksPanel sessionId="s1" />);
    expect((screen.getByTestId('finalize-run-trigger') as any).textContent).toMatch(/agent block/);
  });
  ...
  it('renders an unknown trigger_source verbatim instead of mislabelling it (NB4)', () => {
    ...
    render(<ChecksPanel sessionId="s1" />);
    expect((screen.getByTestId('finalize-run-trigger') as any).textContent).toMatch(
      /webhook event/,
    );
    expect((screen.getByTestId('finalize-run-trigger') as any).textContent).not.toMatch(
      /UI button/,
    );
  });
  ...
  it('renders the failure_reason chip on a terminal failure', () => {
    ...
    render(<ChecksPanel sessionId="s1" />);
    expect((screen.getByTestId('finalize-run-failure-code') as any).textContent).toBe(
  ...
  it('explains review_failed with no CI steps in the empty-steps panel', () => {
    ...
    expect((empty as any).textContent).toMatch(/Review failed before CI steps ran/);
    expect((empty as any).textContent).toMatch(/agenthub:review-verdict/);
  });
