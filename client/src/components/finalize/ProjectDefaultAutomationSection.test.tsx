...
import ProjectDefaultAutomationSection from './ProjectDefaultAutomationSection';
...
describe('ProjectDefaultAutomationSection', () => {
  ...
  it('loads and reflects the stored default level', async () => {
    (api.getProjectUserSettings as any).mockResolvedValueOnce({
      projectId: 'proj-1',
  ...
  it('persists a chosen level via PUT', async () => {
    ...
    (api.updateProjectUserSettings as any).mockResolvedValueOnce({
      projectId: 'proj-1',
    ...
    fireEvent.click(screen.getByDisplayValue('merge' as any) as any);
  ...
  it('clears the preference by selecting "No preference"', async () => {
    ...
    fireEvent.click(screen.getByDisplayValue('__none__' as any) as any);
  ...
  it('reverts the selection when the save fails', async () => {
    ...
    (api.updateProjectUserSettings as any).mockRejectedValueOnce(new Error('boom'));
    render(<ProjectDefaultAutomationSection projectId="proj-1" />);

    await waitFor(() => expect(screen.getByDisplayValue('manual')).toBeChecked());
    fireEvent.click(screen.getByDisplayValue('push' as any) as any);
