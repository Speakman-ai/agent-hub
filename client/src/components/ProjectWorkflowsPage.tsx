...
async function mapPool(items: any, concurrency: any, mapper: any) {
...
function runStatusBadgeClass(status: any) {
...
function StepDot({ title, kind }: any) {
...
export default function ProjectWorkflowsPage({
  ...
  const [monitorWorkflowId, setMonitorWorkflowId] = useState<any>(null);
  const [rowActionId, setRowActionId] = useState<any>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const enriched = await mapPool(wfList, WORKFLOW_ENRICH_CONCURRENCY, async (w: any) => {
        let lastRun = null;
        let stepRuns: any[] = [];
        try {
  ...
  const goChat = () => {
    const agents = project?.agents?.filter((a: any) => a.active !== false) || [];
    const first = agents[0];
    if (first && onSelectAgent) onSelectAgent(first.id);
    onNavigate('chat');
  };

  const startRun = async (wfId: any) => {
    const token = projectId;
    setRowActionId(wfId);
    try {
