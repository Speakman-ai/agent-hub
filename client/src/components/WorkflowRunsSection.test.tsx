...
describe('WorkflowRunsSection', () => {
  ...
  it('renders timeline and progress for selected run detail', async () => {
    (api.getProjectWorkflows as any).mockResolvedValue([
      {
        id: 'wf-1',
        name: 'Release',
        steps: [
    ...
    (api.getWorkflowRuns as any).mockResolvedValue([{ id: 'run-aa', status: 'running' }]);
    (api.getWorkflowRunDetail as any).mockResolvedValue({
  ...
  it('shows run-level error banner and failed step on error', async () => {
    ...
    (api.getWorkflowRuns as any).mockResolvedValue([{ id: 'run-e', status: 'error' }]);
    (api.getWorkflowRunDetail as any).mockResolvedValue({
  ...
  it('does not call setError when an older getWorkflowRunDetail rejects after a newer fetch', async () => {
    let rejectFirst: any;
    const firstPending = new Promise((_res: any, rej: any) => {
      rejectFirst = rej;
    });
    ...
    (api.getWorkflowRuns as any).mockResolvedValue([
      { id: 'runone11', status: 'success' },
      { id: 'runtwo22', status: 'success' },
    ]);

    (api.getWorkflowRunDetail as any)
    ...
    fireEvent.click(screen.getByRole('button', { name: /runtwo22/i } as any) as any);

    await waitFor(() =>
      expect((api.getWorkflowRunDetail as any).mock.calls.length).toBeGreaterThanOrEqual(2),
    );
  ...
  it('ignores stale getWorkflowRunDetail after switching runs', async () => {
    ...
    const pending: any[] = [];
    (api.getWorkflowRunDetail as any).mockImplementation(() => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /run-byyy/i } as any) as any);
  ...
  it('clears detail output when switching runs before the new detail resolves', async () => {
    let resolveB: any;
    const detailBPending = new Promise((res: any) => {
      resolveB = res;
    });
    ...
    fireEvent.click(screen.getByRole('button', { name: /runbbb22/i } as any) as any);
  ...
  it('does not keep prior workflow runs selected while the next list is loading', async () => {
    let resolveWf2: any;
    const wf2Promise = new Promise((res: any) => {
      resolveWf2 = res;
    });
    ...
    (api.getWorkflowRuns as any).mockImplementation((_pid: any, wid: any) => {
      if (wid === 'wf-1') {
    ...
    fireEvent.click(screen.getByRole('button', { name: /Hub workflow runs/i } as any) as any);
    ...
    fireEvent.change(screen.getByRole('combobox' as any), { target: { value: 'wf-2' } });
  ...
  it('omits progressbar role when the workflow defines zero steps', async () => {
    (api.getProjectWorkflows as any).mockResolvedValue([{ id: 'wf-1', name: 'W', steps: [] }]);
    (api.getWorkflowRuns as any).mockResolvedValue([{ id: 'runone11', status: 'success' }]);
    (api.getWorkflowRunDetail as any).mockResolvedValue({
      run: { id: 'runone11', status: 'success', error: null },
      step_runs: [],
    });
  ...
  it('clears list error after getWorkflowRuns succeeds following a failure', async () => {
    let n = 0;
    (api.getProjectWorkflows as any).mockResolvedValue([{ id: 'wf-1', name: 'W', steps: [] }]);
    (api.getWorkflowRuns as any).mockImplementation(() => {
      n += 1;
