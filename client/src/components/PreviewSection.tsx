...
import { envRowsFromDraftAndSecrets, isReservedEnvKey } from '../utils/projectEnvRows';
...
export function formFromProject(project: any) {
...
export function mergeDraftIntoForm(form: any, draft: any) {
  ...
  if (primary?.services?.length && !next.composeServices.length) {
    next.composeServices = primary.services.map((s: any) => s.name);
  }
  ...
  if (
    ...
    primary?.services?.find((s: any) => s.name === primary.suggestedEntryService)?.name
  ) {
...
export function validateComposeForm(form: any) {
...
export function buildBuildPayload(form: any, envRows: any, { needsBootstrap }: any) {
  ...
  const payload: Record<string, any> = {
    ...
    envVars: (envRows || [])
      ...
      .filter((r: any) => !isReservedEnvKey(r.key.trim()))
      .map((r: any) => ({
        key: r.key.trim(),
        value: r.value ?? '',
...
function PreviewBuildLog({ logTail }: any) {
...
function ComposePreviewChecklist({ items }: any) {
  if (!items?.length) return null;
  const failCount = items.filter((i: any) => i.status === 'fail').length;
  const warnCount = items.filter((i: any) => i.status === 'warn').length;
  return (
    <section
...
function RunningPreviewsPanel({ projectId, onOpenSession }: any) {
...
function ScanSummary({ draft }: any) {
  if (!draft) return null;
  const services =
    draft.detected?.compose?.services ??
    draft.composeCandidates?.flatMap((c: any) =>
      c.services.map((s: any) => `${c.file}: ${s.name}`),
    ) ??
    [];
  return (
    <section
...
export default function PreviewSection({
  ...
  const [project, setProject] = useState(projects?.find((p: any) => p.id === projectId) || null);
  ...
  const [buildPhase, setBuildPhase] = useState<any>(null);
  const [buildResult, setBuildResult] = useState<any>(null);
  const [buildError, setBuildError] = useState<any>(null);
  const [setupStarted, setSetupStarted] = useState(false);
  const buildStatusRef = useRef<any>(null);
  ...
  const reloadDraft = useCallback(async (pid: any) => {
    ...
    try {
      ...
      setDraft(d);
      const found = projectsRef.current.find((p: any) => p.id === pid);
  ...
  useEffect(() => {
    const found = projects?.find((p: any) => p.id === projectId) || null;
    setProject(found);
  ...
  useEffect(() => {
    ...
    if (!projects.some((p: any) => p.id === projectId)) setProjectId(projects[0].id);
  }, [projects, projectId]);
  ...
  const updateEnvRow = (idx: any, patch: any) => {
    setEnvRows((rows: any) => rows.map((r: any, i: any) => (i === idx ? { ...r, ...patch } : r)));
  };
  const addEnvRow = () =>
    setEnvRows((rows: any) => [...rows, { key: '', value: '', kind: 'secret', sources: [] }]);
  const removeEnvRow = (idx: any) =>
    setEnvRows((rows: any) => rows.filter((_: any, i: any) => i !== idx));
        ...
        <>
            ...
            {needsBootstrap && (
              <div>
                ...
                <textarea
                  ...
                  onChange={(e: any) => setField('composeYaml', e.target.value)}
                  rows={12}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e: any) => setField('composeFile', e.target.value)}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e: any) => setField('composeEnvFile', e.target.value)}
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e: any) => setField('composeEntryService', e.target.value)}
                  className="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-sm"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e: any) => setField('composeEntryPort', e.target.value)}
                  className="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-sm"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e: any) => setField('healthPath', e.target.value)}
                  className="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-sm"
              ...
              <div>
                ...
                <input
                  ...
                  value={form.idleTTL}
                  onChange={(e: any) => setField('idleTTL', e.target.value)}
                  className="w-full bg-gray-900 border border-gray-700 rounded-lg px-3 py-2 text-sm"
          ...
          >
            ...
            <div className="space-y-2">
              {envRows.map((row: any, idx: any) => (
                <div key={`${row.key}-${idx}`} className="grid grid-cols-12 gap-2 items-center">
                  <input
                    type="text"
                    value={row.key}
                    onChange={(e: any) => updateEnvRow(idx, { key: e.target.value })}
                  ...
                  <input
                    ...
                    value={row.value}
                    onChange={(e: any) => updateEnvRow(idx, { value: e.target.value })}
                    placeholder={
                  ...
                  <select
                    value={row.kind}
                    onChange={(e: any) => updateEnvRow(idx, { kind: e.target.value })}
