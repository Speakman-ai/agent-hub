...
beforeEach(() => {
  ...
  (api.getMyCursorAuth as any).mockResolvedValue(defaultPasteAuth());
  (api.getMyCursorBrowserAuth as any).mockResolvedValue(defaultBrowserAuth());
...
describe('MyCursorAuthSection', () => {
  ...
  it('renders the login URL banner once startMyCursorLogin resolves with a loginUrl', async () => {
    (api.startMyCursorLogin as any).mockResolvedValue({
      ok: true,
      loginId: 'l1',
  ...
  it('stops polling once uiStatus=authenticated', async () => {
    ...
    (api.getMyCursorBrowserAuth as any).mockResolvedValueOnce(defaultBrowserAuth());
    (api.getMyCursorBrowserAuth as any).mockResolvedValue(
      defaultBrowserAuth({
  ...
  it('saves the paste-key fallback through putMyCursorAuth', async () => {
    (api.putMyCursorAuth as any).mockResolvedValue(
    ...
    const input = await screen
      ...
      .catch(async () => {
        ...
        return all.find((el: any) => el.tagName === 'INPUT');
      });
    fireEvent.change(input as any, { target: { value: 'cursor-test-key' } } as any);
