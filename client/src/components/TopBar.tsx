...
const MODEL_LABELS = {
  ...
  'grok-4.7-high-fast': { label: 'Grok 4.7 Fast', short: '4.7 Fast' },
  'cursor-grok-4.6-high': { label: 'Cursor Grok 4.6', short: 'Grok 4.6' },
  'cursor-grok-4.5-high': { label: 'Cursor Grok 4.5', short: 'Grok 4.5' },
  ...
  'grok-4.7': { label: 'Grok 4.7', short: '4.7' },
  'grok-4.7-build-fast': { label: 'Grok 4.7 Fast', short: '4.7 Fast' },
  'grok-4.6': { label: 'Grok 4.6', short: '4.6' },
  'grok-4.5': { label: 'Grok 4.5', short: '4.5' },
...
export default function TopBar({
  ...
  const [exportState, setExportState] = useState<any>(null); // null | 'summarizing' | 'copied' | 'saving' | 'saved'
  const modelRef = useRef<any>(null);
  const engineRef = useRef<any>(null);
  const reasoningRef = useRef<any>(null);
  ...
  const filteredEngineOptions = modelConfig?.engineValidModels
    ? ENGINE_OPTIONS.filter((e: any) => (modelConfig.engineValidModels[e.id]?.length ?? 0) > 0)
    : ENGINE_OPTIONS;
      ...
      <div className="flex items-center gap-1.5 md:gap-3">
        ...
        {agent && isCodex && onReasoningEffortChange && (
          <div className="hidden sm:flex items-center gap-1.5" ref={reasoningRef}>
            <div className="relative">
              <button
                onClick={() => setReasoningOpen((v: any) => !v)}
                className="flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-lg bg-gray-800 hover:bg-gray-700 transition-colors border border-gray-700"
        ...
        {agent && (
          <div className="sm:hidden relative">
            <button
              onClick={() => setMobileMenuOpen((v: any) => !v)}
