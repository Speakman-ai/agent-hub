...
describe('PersonalOAuthConfigSection', () => {
  ...
  it('shows the configured client ID and a Remove button when configured', async () => {
    (fetchMock as any).mockResolvedValueOnce(respond({ configured: true, clientId: 'Iv1.abcdef' }));
    render(<PersonalOAuthConfigSection />);
    await waitFor(() => {
  ...
  it('PUTs both clientId and clientSecret on save', async () => {
    ...
    fireEvent.change(screen.getByTestId('personal-oauth-client-id' as any), {
      target: { value: 'Iv1.x' },
    });
    fireEvent.change(screen.getByTestId('personal-oauth-client-secret' as any), {
