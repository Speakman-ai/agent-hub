...
export default function AgentRosterPicker({
  ...
  const handleAssignExisting = (trackId: any, agentId: any) => {
    onChange(setRosterAgent(roster, trackId, agentId || null));
  };

  const handleAgentName = (trackId: any, agentName: any) => {
    onChange(setRosterAgentName(roster, trackId, agentName));
  };

  const handleRemove = (trackId: any) => {
    ...
    >
      ...
      {createAgents ? (
        <input
          type="text"
          value={row.agentName || ''}
          onChange={(e: any) => onAgentName(row.trackId, e.target.value)}
          disabled={disabled}
        ...
        <select
          value={row.agentId || ''}
          onChange={(e: any) => onAssignExisting(row.trackId, e.target.value)}
          disabled={disabled}
