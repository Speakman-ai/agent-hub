 ...
 */
export default function OrchestrationTimelinePanel({ entries, sessionId, compact = false }: any) {
              ...
              {prSummary.checks.length > 0 &&
                (() => {
                  const relevant = prSummary.checks.filter(
                    (c: any) => c.conclusion === 'failure' || c.status === 'in_progress',
                  );
                  if (relevant.length === 0) return null;
                  return (
                    <ul className="mt-1.5 space-y-0.5">
                      {relevant.slice(0, 5).map((chk: any, i: any) => {
