...
export default function CardContextMenu({
  ...
  const cardLabelSet = useMemo(
    () =>
      new Set(
        (card.labels ? String(card.labels).split(',') : [])
          .map((l: any) => l.trim())
          .filter(Boolean),
      ),
    [card.labels],
  ...
  const items = useMemo(() => {
    const assigneeChildren = agents.map((a: any) => ({
      key: `agent-${a.id}`,
      label: a.name,
    ...
    return [
      ...
      },
      {
        ...
        label: 'Priority',
        icon: Flag,
        submenu: PRIORITY_OPTIONS.map((p: any) => ({
          key: `pri-${p}`,
          label: cap(p),
  ...
  const openItem = openKey ? items.find((i: any) => i.key === openKey) : null;
  ...
  const onKeyDown = (e: any) => {
    ...
    if (submenu) {
      if (e.key === 'ArrowDown') {
        e.preventDefault();
        setSubActive((i: any) => Math.min(i + 1, submenu.length - 1));
      } else if (e.key === 'ArrowUp') {
        e.preventDefault();
        setSubActive((i: any) => Math.max(i - 1, 0));
    ...
    if (e.key === 'ArrowDown') {
      e.preventDefault();
      setActive((i: any) => Math.min(i + 1, items.length - 1));
    } else if (e.key === 'ArrowUp') {
      e.preventDefault();
      setActive((i: any) => Math.max(i - 1, 0));
            ...
            return (
              ...
              >
                ...
                {item.submenu && isOpen && (
                  ...
                  >
                    {item.submenu.map((child: any, ci: any) => (
                      <button
                        key={child.key}
                        type="button"
                        role="menuitem"
