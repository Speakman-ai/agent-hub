...
(vi as any).mock('./WorkflowRunsSection.jsx', () => ({
...
describe('ProjectWorkflowsPage', () => {
  ...
  it('starts a run when Run is clicked', async () => {
    ...
    (api.getWorkflowRuns as any).mockResolvedValue([]);
    (api.startWorkflowRun as any).mockResolvedValue({ id: 'r1', status: 'pending' });
    ...
    fireEvent.click(screen.getByRole('button', { name: /^Run$/i } as any) as any);
    await waitFor(() => {
  ...
  it('GitHub opens Settings → Projects with expandProjectId', async () => {
    ...
    await waitFor(() => expect(screen.getByText('W')).toBeInTheDocument());
    fireEvent.click(screen.getByRole('button', { name: /^GitHub$/i } as any) as any);
  ...
  it('Builder navigates to workflow editor view', async () => {
    ...
    await waitFor(() => expect(screen.getByText('W')).toBeInTheDocument());
    fireEvent.click(screen.getByRole('button', { name: /^Builder$/i } as any) as any);
  ...
  it('renders monitor embed using mocked WorkflowRunsSection', async () => {
    ...
    await waitFor(() => expect(screen.getByText('W')).toBeInTheDocument());
    fireEvent.click(screen.getByRole('button', { name: /^Monitor$/i } as any) as any);
