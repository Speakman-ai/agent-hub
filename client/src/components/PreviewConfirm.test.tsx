...
describe('PreviewConfirm', () => {
  ...
  it('"Looks good" emits accept with the detected values intact', () => {
    ...
    const [form, reason] = (onConfirm as any).mock.calls[0];
  ...
  it('Edit → modify → Save emits edit with the new form payload', () => {
    const onConfirm = vi.fn();
    render(<PreviewConfirm detected={VITE_DETECTED} onConfirm={onConfirm} />);
    fireEvent.click(screen.getByTestId('preview-confirm-edit' as any) as any);

    // Tweak start script and idle TTL.
    fireEvent.change(screen.getByTestId('preview-confirm-startScript' as any), {
      target: { value: 'pnpm dev' },
    });
    fireEvent.change(screen.getByTestId('preview-confirm-idleTTL' as any), {
      target: { value: '900' },
    });

    // Add a new capture route via the input + Add button.
    fireEvent.change(screen.getByTestId('preview-confirm-route-input' as any), {
      target: { value: '/about' },
    });
    fireEvent.click(screen.getByRole('button', { name: 'Add' } as any) as any);

    fireEvent.click(screen.getByTestId('preview-confirm-save-edits' as any) as any);
    expect(onConfirm!).toHaveBeenCalledTimes(1);
    const [form, reason] = (onConfirm as any).mock.calls[0];
  ...
  it('removes a capture route via its X button', () => {
    ...
    fireEvent.click(screen.getByTestId('preview-confirm-edit' as any) as any);
    fireEvent.click(screen.getByLabelText('Remove route /about' as any) as any);
    fireEvent.click(screen.getByTestId('preview-confirm-save-edits' as any) as any);
    const [form] = (onConfirm as any).mock.calls[0];
    expect(form.captureRoutes).toEqual(['/']);
  });
  ...
  it('Cancel from edit mode reverts to the original summary view', () => {
    ...
    fireEvent.click(screen.getByTestId('preview-confirm-cancel-edits' as any) as any);
