...
describe('AgentRosterPicker — assign existing agents', () => {
  ...
  it('preselects the suggested agent for each row', () => {
    ...
    expect(screen.getByTestId('roster-agent-backend').value).toBe('');
  });
  ...
  it('fires onChange with the updated roster when a row is reassigned', () => {
    ...
    fireEvent.change(screen.getByTestId('roster-agent-backend'), {
      target: { value: 'hub-backend' },
    });
    expect(onChange).toHaveBeenCalledTimes(1);
    const next = onChange.mock.calls[0][0];
    expect(next.find((r) => r.trackId === 'backend').agentId).toBe('hub-backend');
    // Architect row unchanged.
    expect(next.find((r) => r.trackId === 'architect').agentId).toBe('hub-lead');
  });

  it('clearing the select assigns null', () => {
    const onChange = vi.fn();
    render(<AgentRosterPicker roster={baseRoster()} agents={agents} onChange={onChange} />);
    fireEvent.change(screen.getByTestId('roster-agent-architect'), { target: { value: '' } });
    const next = onChange.mock.calls[0][0];
    expect(next.find((r) => r.trackId === 'architect').agentId).toBeNull();
  });

  it('adds a custom track via the Add button', () => {
    const onChange = vi.fn();
    render(<AgentRosterPicker roster={baseRoster()} agents={agents} onChange={onChange} />);
    fireEvent.change(screen.getByTestId('roster-custom-label'), { target: { value: 'Docs' } });
    fireEvent.click(screen.getByTestId('roster-custom-add'));
    expect(onChange).toHaveBeenCalledTimes(1);
    const next = onChange.mock.calls[0][0];
  ...
  it('adds a custom track via Enter key', () => {
    ...
    fireEvent.change(input, { target: { value: 'QA Lead' } });
    fireEvent.keyDown(input, { key: 'Enter' });
    expect(onChange).toHaveBeenCalledTimes(1);
    expect(onChange.mock.calls[0][0][2].label).toBe('QA Lead');
  });
  ...
  it('disables Add when label is empty or whitespace', () => {
    ...
    fireEvent.change(screen.getByTestId('roster-custom-label'), { target: { value: '   ' } });
    expect(btn).toBeDisabled();
  });
  ...
  it('custom rows expose a remove button that fires onChange', () => {
    ...
    fireEvent.click(screen.getByTestId('roster-remove-custom-1'));
    const next = onChange.mock.calls[0][0];
    expect(next.find((r) => r.trackId === 'custom-1')).toBeUndefined();
