...
import ForwardDesignModal from './ForwardDesignModal';
...
function DesignStudioEngineModelSelect({
  ...
  const engineOptions = DESIGN_STUDIO_ENGINES.filter(
    (id: any) => (modelConfig.engineValidModels[id]?.length ?? 0) > 0,
  );
  ...
  const handleEngineChange = async (e: any) => {
...
export function clampSplitPct(pct: any) {
  const n = Number(pct);
...
export default function DesignView({
  ...
  const splitContainerRef = useRef<any>(null);
  ...
  const [isMd, setIsMd] = useState<any>(() => {
  ...
  const onResizerPointerDown = useCallback((e: any) => {
    e.preventDefault();
  ...
  const onResizerKeyDown = useCallback((e: any) => {
    const step = e.shiftKey ? 5 : 2;
    if (e.key === 'ArrowLeft') {
      e.preventDefault();
      setSplitPct((p: any) => clampSplitPct(p - step));
    ...
    <div className="flex flex-col h-full bg-gray-950">
          ...
          <div className="min-w-0">
            ...
            {design.linkedProjects?.length > 0 && (
              <div className="flex items-center gap-1 mt-0.5">
                ...
                {design.linkedProjects.slice(0, 3).map((p: any) => (
      ...
      {showForwardDesign && (
        <ForwardDesignModal
          ...
          onForward={(body: any) => api.forwardDesign(design.id, body)}
 ...
 */
function DesignChat({ design, messages, streaming, thinking, processing, send }: any) {
  ...
  const scrollToBottom = useCallback((instant: any) => {
...
function DesignMessage({ message }: any) {
 ...
 */
export function injectBaseHref(html: any, baseHref: any) {
 ...
 */
export function DesignCanvas({ designId, srcBase, reloadToken, onManualReload }: any) {
  const base = getServerBase();
  const [srcdoc, setSrcdoc] = useState<any>(null);
