...
import ProjectWorkflowBuilder from './ProjectWorkflowBuilder';
...
describe('ProjectWorkflowBuilder', () => {
  ...
  it('loads workflow and marks dirty after name change', async () => {
    (api.getProjectWorkflow as any).mockResolvedValue({
      id: 'wf-1',
      name: 'Ship',
      trigger_type: 'manual',
      default_payload: {},
    ...
    fireEvent.change(screen.getByDisplayValue('Ship' as any), { target: { value: 'Ship 2' } });
  ...
  it('discard restores baseline', async () => {
    ...
    fireEvent.change(screen.getByDisplayValue('One' as any), { target: { value: 'Two' } });
    expect(await screen.findByText(/Unsaved changes/i)).toBeInTheDocument();

    fireEvent.click(screen.getByRole('button', { name: /^Discard$/i } as any) as any);
  ...
  it('ignores stale getProjectWorkflow after workflowId changes', async () => {
    let slowResolve: any;
    (api.getProjectWorkflow as any).mockImplementation((_pid: any, wid: any) => {
      if (wid === 'slow-wf') {
  ...
  it('new workflow saves via POST and navigates', async () => {
    (api.createProjectWorkflow as any).mockResolvedValue({
      id: 'new-wf',
      name: 'N',
      trigger_type: 'manual',
      steps: [],
  ...
  it('new workflow shows template library and applies Dev Review seed', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /^Dev Review$/i } as any) as any);
  ...
  it('new workflow discard after template uses local-baseline toast copy', async () => {
    ...
    await waitFor(() => expect(screen.getByText(/Template library/i)).toBeInTheDocument());
    fireEvent.click(screen.getByRole('button', { name: /^Content$/i } as any) as any);
