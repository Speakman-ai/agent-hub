...
describe('eventsToBlocks — agenthub:ask recovery', () => {
  ...
  it('renders a picker for flat fenced envelope (askId + question at top level)', () => {
    ...
    const blocks = eventsToBlocks(events);
    expect(blocks.some((b: any) => b.kind === 'ask_question')).toBe(true);
    expect(blocks.find((b: any) => b.kind === 'ask_question')?.event.askId).toBe(
      'preview-bootstrap-approval',
    );
    const textBlock = blocks.find((b: any) => b.kind === 'text');
...
describe('eventsToBlocks — ExitPlanMode routing', () => {
  ...
  it('routes ExitPlanMode tool_use to a plan_proposal block, not a generic tool block', () => {
    ...
    expect(blocks.some((b: any) => b.kind === 'error')).toBe(false);
    expect(blocks.some((b: any) => b.kind === 'tool')).toBe(false);
  });
...
describe('eventsToBlocks — explored coalescer', () => {
  const wrap = (events: any) => events.map((event: any, i: any) => ({ seq: i, event }));
  const read = (id: any, file: any) => ({
    type: 'tool_use',
    id,
    tool: 'Read',
    input: { file_path: file },
  });
  const grep = (id: any, pattern: any) => ({
    type: 'tool_use',
    id,
    tool: 'Grep',
    input: { pattern },
  ...
  it('coalesces 3 reads + 1 grep into a single explored block with all items', () => {
    ...
    expect(blocks[0].items.map((i: any) => i.use.tool)).toEqual(['Read', 'Read', 'Grep', 'Read']);
  });
...
describe('eventsToBlocks — control block stripping (regression: v1.13.0)', () => {
  ...
  it('produces no text block when the ONLY content is a control block (agent emitting just the invocation)', () => {
    ...
    expect(blocks.filter((b: any) => b.kind === 'text')).toHaveLength(0);
  });
});
...
describe('eventsToBlocks — thinking coalescing', () => {
  ...
  it('keeps separate thinking blocks when interrupted by a tool call', () => {
    ...
    const blocks = eventsToBlocks(events);
    const thinking = blocks.filter((b: any) => b.kind === 'thinking');
