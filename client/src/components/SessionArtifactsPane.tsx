...
import { downloadArtifact } from '../utils/artifactContent';
...
export default function SessionArtifactsPane({
  ...
  return (
    <aside
      ...
      className={`hidden lg:flex flex-col shrink-0 border-l border-gray-800 bg-gray-950 relative ${selectedArtifact ? 'w-[720px]' : 'w-[420px]'}`}
    ...
    >
      ...
      <div className="flex items-center gap-2 px-3 py-2 border-b border-gray-800 bg-gray-900/60">
        {selectedArtifact ? (
          <button
            ...
            data-testid="session-artifact-viewer-back"
        ...
        <div className="flex-1 min-w-0">
          <div className="text-xs font-semibold text-gray-100 truncate">
            {selectedArtifact?.filename || 'Artifacts'}
          ...
          <div className="text-[10px] text-gray-500 truncate">
            {selectedArtifact ? 'Artifact preview' : 'Documents generated this session'}
          </div>
        </div>
        {selectedArtifact && (
          <button
            ...
            disabled={busyId === selectedArtifact.id}
            title="Download"
            aria-label={`Download ${selectedArtifact.filename}`}
      ...
      {/* Body */}
      <div className={`flex-1 min-h-0 ${selectedArtifact ? 'overflow-hidden' : 'overflow-y-auto'}`}>
        {error && (
          <div
        ...
        {selectedArtifact ? (
          <SessionArtifactViewer sessionId={sessionId} artifact={selectedArtifact} />
        ) : null}

        {!selectedArtifact && loading && artifacts.length === 0 && (
        ...
        {!selectedArtifact && !loading && artifacts.length === 0 && !error && (
          <div
