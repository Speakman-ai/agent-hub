...
import { buildWorkflowRunTimeline, isWorkflowRunActive } from '../utils/workflowRunTimeline.js';
...
function pickLiveOutput(timeline, run) {
  ...
  if (st === 'error') {
    const failed = [...timeline.rows]
      .reverse()
      .find((r) => r.stepRun && r.displayStatus === 'error');
...
export default function WorkflowRunsSection({ projectId, embedWorkflowId = null }) {
  ...
  const hasActiveRunInList = runs.some((r) => isWorkflowRunActive(r));
