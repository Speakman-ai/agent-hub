...
import { markdownComponents, fencedCodeLanguage } from './MarkdownRenderer.jsx';
...
describe('MarkdownRenderer mermaid routing', () => {
  ...
  it('shows a styled error box if mermaid.render rejects', async () => {
    ...
    mermaidModule.default.render.mockRejectedValueOnce(new Error('Parse error: bad syntax'));
    ...
    expect(container.textContent).toContain('Parse error: bad syntax');
    // Original source is preserved in the error box so the user can fix it.
    expect(container.textContent).toContain('not-a-real-diagram');
  });
});
