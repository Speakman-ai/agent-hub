...
import ConnectFirstScreen from './ConnectFirstScreen';
import { getConnectionConfig, saveConnectionConfig, testConnection } from '../utils/connection';
...
describe('ConnectFirstScreen — actions', () => {
  ...
  it('"Use this computer" falls through to children (SetupWizard)', async () => {
    ...
    fireEvent.click(screen.getByTestId('use-local-btn' as any) as any);
    await waitFor(() => {
      expect(screen.getByTestId('app-child')).toBeInTheDocument();
    });
  ...
  it('"Connect to existing server" shows the remote form', async () => {
    ...
    fireEvent.click(screen.getByTestId('use-remote-btn' as any) as any);
  ...
  it('successful test enables Connect; clicking saves config and navigates', async () => {
    (testConnection as any).mockResolvedValue({ ok: true, message: 'Connected' } as any);
    ...
    fireEvent.change(screen.getByLabelText(/server url/i as any), {
      target: { value: 'https://hub.example.com' },
    });
    fireEvent.click(screen.getByRole('button', { name: /test connection/i } as any) as any);
    ...
    await act(async () => {
      fireEvent.click(connectBtn as any);
    });
  ...
  it('failed test surfaces error and keeps Connect disabled', async () => {
    (testConnection as any).mockResolvedValue({ ok: false, message: 'Connection refused' } as any);
  ...
  it('strips trailing slashes from the server URL before saving', async () => {
    (testConnection as any).mockResolvedValue({ ok: true, message: 'ok' } as any);
    ...
    await act(async () => {
      fireEvent.click(screen.getByRole('button', { name: /connect & sign in/i } as any) as any);
    });
