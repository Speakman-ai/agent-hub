...
describe('MessageInput microphone hotkey wiring', () => {
  it('exposes a toggleRecording imperative handle that starts and stops recording', async () => {
    ...
    let started: any;
    await act(async () => {
      started = (ref.current! as any).toggleRecording();
    });
    ...
    expect((ref.current! as any).isRecording()).toBe(true);

    // Toggle again → stop.
    await act(async () => {
      (ref.current! as any).toggleRecording();
    });
    await waitFor(() =>
  ...
  it('is a no-op when the composer is disabled and not mid-stream', async () => {
    ...
    let result: any;
    await act(async () => {
      result = (ref.current! as any).toggleRecording();
    });
