...
} from '../utils/workflowTemplateLibrary';
...
function newStepRow(agentId: any) {
  return {
...
function agentsInProject(agentsList: any, projectKey: any) {
  const pid = String(projectKey || '');
  return (Array.isArray(agentsList) ? agentsList : []).filter(
    (a: any) => a && a.projectId === pid && a.active !== false,
  );
}
...
export default function ProjectWorkflowBuilder({
  ...
  const [baselineDraft, setBaselineDraft] = useState<any>(null);
  ...
  const [wfMeta, setWfMeta] = useState<any>(null);
  ...
  const otherProjectsSorted = useMemo(() => {
    const list = Array.isArray(projects)
      ? projects.filter((p: any) => p && p.id && p.id !== projectId)
      : [];
  ...
  const applyLoaded = useCallback((apiRow: any) => {
  ...
  const applyWorkflowTemplate = (templateId: any) => {
    if (!isNew) return;
  ...
  const updateStep = (index: any, patch: any) => {
    setDraft((prev: any) => {
  ...
  const moveStep = (index: any, dir: any) => {
    setDraft((prev: any) => {
      const steps = [...prev.steps];
      const j = index + dir;
      ...
      return {
        ...prev,
        steps: steps.map((s: any, i: any) => ({ ...s, step_order: i })),
      };
    });
  };
  ...
  const removeStep = (index: any) => {
    setDraft((prev: any) => ({
      ...
      steps: prev.steps
        ...
        .map((s: any, i: any) => ({ ...s, step_order: i })),
    }));
  };
            ...
            <>
              {isNew && (
                <div className="rounded-xl border border-gray-800 bg-gray-900/40 p-4 space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {getWorkflowTemplateMetaList().map((t: any) => (
                      <button
                        key={t.id}
                        type="button"
                        title={
              ...
              <div className="rounded-xl border border-gray-800 bg-gray-900/40 p-4">
                ...
                <input
                  type="text"
                  value={draft.name}
                  onChange={(e: any) => setDraft((p: any) => ({ ...p, name: e.target.value }))}
                  ...
                  <div className="rounded-xl border border-gray-800 bg-gray-900/40 p-4 space-y-3">
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e: any) =>
                          setDraft((p: any) => ({ ...p, default_payload_str: e.target.value }))
                        }
                        spellCheck={false}
                        rows={8}
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e: any) => {
                          ...
                          if (v === 'custom') {
                            setDraft((p: any) => ({ ...p, cron_mode: 'custom' }));
                            return;
                          }
                          setDraft((p: any) => ({ ...p, cron_mode: v, cron_expr: '' }));
                        }}
                      ...
                      >
                        {WORKFLOW_CRON_MODES_ORDER.map((k: any) => (
                    ...
                    {draft.cron_mode === 'custom' && (
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e: any) =>
                            setDraft((p: any) => ({ ...p, cron_expr: e.target.value }))
                          }
                          placeholder="0 * * * *"
                  ...
                  <div className="rounded-xl border border-gray-800 bg-gray-900/40 p-4 space-y-3">
                    ...
                    <label className="flex items-center gap-2 text-sm text-gray-200">
                      <input
                        ...
                        onChange={(e: any) =>
                          setDraft((p: any) => ({ ...p, webhook_enabled: e.target.checked }))
                        }
                      />
                ...
                <div className="space-y-4">
                  ...
                  {draft.steps.length === 0 ? (
                    ...
                    <ul className="space-y-3">
                      {draft.steps.map((step: any, index: any) => {
                        const stepPid =
                                ...
                                <div className="sm:col-span-2">
                                  ...
                                  <select
                                    ...
                                    onChange={(e: any) => {
                                      ...
                                      const nextAgent =
                                        pool.find((a: any) => a.id === step.agent_id)?.id ||
                                        pool[0]?.id ||
                                        '';
                                      updateStep(index, {
                                        step_project_id: v,
