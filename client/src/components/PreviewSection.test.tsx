...
describe('PreviewSection', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    (api.getPreviewEnvironmentDraft as any).mockResolvedValue({ draft: monorepoDraft });
    (api.getProjectSecrets as any).mockResolvedValue({ secrets: [] });
    (api.getProjectPreviews as any).mockResolvedValue({ previews: [] });
  });
  ...
  it('lists running previews and supports purge all', async () => {
    ...
    (api.purgeAllProjectPreviews as any).mockResolvedValue({
      ok: true,
      stopped: 1,
      failed: [],
    } as any);
    ...
    fireEvent.click(screen.getByTestId('preview-purge-all-button' as any) as any);
  ...
  it('reveals editable form after Start setup', async () => {
    ...
    await waitFor(() => expect(api.getPreviewEnvironmentDraft).toHaveBeenCalled());
    await act(async () => {
      fireEvent.click(screen.getByTestId('preview-start-setup-button' as any) as any);
    });
    await waitFor(() => {
  ...
  it('shows validation error when entry service is missing', async () => {
    ...
    await waitFor(() => expect(api.getPreviewEnvironmentDraft).toHaveBeenCalled());
    await act(async () => {
      fireEvent.change(screen.getByTestId('preview-compose-entry-service' as any), {
        target: { value: '' },
      });
      fireEvent.click(screen.getByTestId('preview-build-button' as any) as any);
    });
  ...
  it('starts agent walkthrough only when user clicks Agent walkthrough', async () => {
    ...
    (api.startPreviewWizard as any).mockResolvedValueOnce({ sessionId: 's1', agentId: 'a1' });
    await act(async () => {
      fireEvent.click(screen.getByTestId('preview-walkthrough-button' as any) as any);
    });
    await waitFor(() => {
  ...
  it('forwards readyTimeoutMs from the form to buildPreviewEnvironment when set', async () => {
    (api.buildPreviewEnvironment as any).mockResolvedValue({ ok: true, steps: {} } as any);
    render(<PreviewSection projects={configuredProjects} />);
    await waitFor(() => expect(api.getPreviewEnvironmentDraft).toHaveBeenCalled());
    await act(async () => {
      fireEvent.change(screen.getByTestId('preview-ready-timeout' as any), {
        target: { value: '1500000' },
      });
      fireEvent.click(screen.getByTestId('preview-build-button' as any) as any);
    });
    await waitFor(() => expect(api.buildPreviewEnvironment).toHaveBeenCalled());
    const [, payload] = (api.buildPreviewEnvironment as any).mock.calls[0];
