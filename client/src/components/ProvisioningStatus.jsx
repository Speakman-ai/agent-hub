...
import {
  ...
  hasGithubFailure,
  LOG_BUFFER_MAX,
} from '../utils/provisioningStatus.js';
...
export default function ProvisioningStatus({
  ...
  const state = useMemo(() => {
    return events.reduce((acc, ev) => reduceEvent(acc, ev), initialState({ withGithub }));
  }, [events, withGithub]);
...
function SuccessCard({ state, onClose, onOpenRepo }) {
  return (
    <section
...
function PartialCard({ state, onClose, onOpenRepo }) {
...
function FailureCard({ state, onRetry, onClose }) {
  const cls = classifyError(state.error);
  return (
    <section
