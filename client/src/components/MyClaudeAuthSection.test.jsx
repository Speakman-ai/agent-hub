...
import MyClaudeAuthSection, { StatusChip } from './MyClaudeAuthSection.jsx';
import { api } from '../utils/api.js';

beforeEach(() => {
  api.getMyClaudeAuth.mockReset();
  api.putMyClaudeAuth.mockReset();
});
...
describe('MyClaudeAuthSection — load + render', () => {
  it('renders masked credential values and the "Using yours" chip for both slots', async () => {
    api.getMyClaudeAuth.mockResolvedValue({
  ...
  it('degrades gracefully on 401 (legacy global apiKey, no per-user identity)', async () => {
    api.getMyClaudeAuth.mockRejectedValue(new Error('401: Authentication required'));
  ...
  it('shows a retry button on non-401 load errors', async () => {
    api.getMyClaudeAuth.mockRejectedValueOnce(new Error('500: boom')).mockResolvedValueOnce({
      anthropicApiKey: null,
      claudeCodeOAuthToken: null,
      claudeCodeOAuthExpiresAt: null,
      claudeCodeOAuthExpired: null,
...
describe('MyClaudeAuthSection — save flows', () => {
  ...
  it('PUTs only the anthropicApiKey field when saving — other fields stay untouched', async () => {
    api.getMyClaudeAuth.mockResolvedValue(baseAuth());
    api.putMyClaudeAuth.mockResolvedValue(baseAuth({ anthropicApiKey: '****new' }));
  ...
  it('Clear button PUTs anthropicApiKey: "" to clear the field', async () => {
    api.getMyClaudeAuth.mockResolvedValue(baseAuth({ anthropicApiKey: '****existing' }));
    api.putMyClaudeAuth.mockResolvedValue(baseAuth());
  ...
  it('collapses whitespace in pasted OAuth tokens before saving', async () => {
    api.getMyClaudeAuth.mockResolvedValue(baseAuth());
    api.putMyClaudeAuth.mockResolvedValue(baseAuth({ claudeCodeOAuthToken: '****abcd' }));
    ...
    fireEvent.click(screen.getByRole('button', { name: /save oauth token/i }));
  ...
  it('shows the error message in an alert when PUT rejects', async () => {
    api.getMyClaudeAuth.mockResolvedValue(baseAuth());
    api.putMyClaudeAuth.mockRejectedValue(new Error('500: server fell over'));
