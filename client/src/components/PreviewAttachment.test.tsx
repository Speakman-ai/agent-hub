...
describe('PreviewAttachment', () => {
  ...
  it('toggles the iframe on Open/Hide click', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /Open in iframe/i } as any) as any);
    expect(container!.querySelector('iframe')).not.toBeNull();
    fireEvent.click(screen.getByRole('button', { name: /Hide iframe/i } as any) as any);
    expect(container!.querySelector('iframe')).toBeNull();
  });
  ...
  it('forwards the full event (including wizard intent) to onConfigure', () => {
    ...
    fireEvent.click(screen.getByRole('link', { name: /Configure preview/i } as any) as any);
    expect(onConfigure!).toHaveBeenCalledTimes(1);
    const passed = (onConfigure as any).mock.calls[0][0];
  ...
  it('hides the teach-moment card when the user clicks Skip', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /Skip/i } as any) as any);
