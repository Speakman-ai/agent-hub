...
import EpicView from './EpicView.jsx';
...
describe('EpicView', () => {
  ...
  it('lists epics and navigates to detail', async () => {
    ...
    fireEvent.click(screen.getByTestId('epic-list-item-e1'));
    expect(onOpenEpic).toHaveBeenCalledWith('e1');
  });

  it('adds a ticket on the epic detail screen', async () => {
    api.createCard.mockResolvedValue({ id: 'c2' });
    api.getBoard.mockResolvedValueOnce(board).mockResolvedValueOnce({
      ...board,
      cards: [
        ...board.cards,
    ...
    fireEvent.change(screen.getByTestId('epic-add-ticket-input'), {
      target: { value: 'New ticket' },
    });
    fireEvent.click(screen.getByTestId('epic-add-ticket-button'));
