 ...
 */
export function filterDesignForwardTargets(agents, design) {
  if (!Array.isArray(agents) || !design) return [];
  const base = agents.filter((a) => a && a.active !== false);
  const linkedIds = (design.linkedProjects || []).map((p) => p?.id).filter(Boolean);
  if (linkedIds.length === 0) return base;
  const linkedSet = new Set(linkedIds);
  const inLinked = base.filter((a) => linkedSet.has(a.projectId));
