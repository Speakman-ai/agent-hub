...
describe('SessionPreviewPane', () => {
  ...
  it('clicking refresh bumps the iframe key (forcing a reload)', async () => {
    ...
    fireEvent.click(screen.getByTestId('session-preview-pane-refresh' as any) as any);
  ...
  it('renders unavailable state with a Configure button wired to onConfigure', () => {
    ...
    fireEvent.click(screen.getByTestId('session-preview-pane-configure' as any) as any);
  ...
  it('pop-out opens a window with the expected URL and switches to reattach placeholder', async () => {
    ...
    fireEvent.click(screen.getByTestId('session-preview-pane-popout' as any) as any);
    expect(popOut!).toHaveBeenCalledTimes(1);
    expect((popOut as any).mock.calls[0][0]).toBe('http://localhost:4101/board');
    expect((popOut as any).mock.calls[0][1]).toBe('agent-hub-preview-s-1');
    expect((popOut as any).mock.calls[0][2]).toMatch(/width=1280/);
  ...
  it('browser pop-out auto-reattaches when the window closes', async () => {
    const fakeWindow: Record<string, any> = { closed: false, close: vi.fn() };
  ...
  it('reattach restores the iframe view', async () => {
    ...
    fireEvent.click(screen.getByTestId('session-preview-pane-popout' as any) as any);
    expect(screen.getByTestId('session-preview-pane-popped')).toBeInTheDocument();
    fireEvent.click(screen.getByTestId('session-preview-pane-reattach' as any) as any);
  ...
  it('touch handler is throttled to at most once per 30 s under rapid activity', async () => {
    ...
    let t = 1_000_000;
    (dateNow as any).mockImplementation(() => t);
    try {
  ...
  it('Close button invokes onClose', async () => {
    ...
    fireEvent.click(screen.getByTestId('session-preview-pane-close' as any) as any);
  ...
  it('reads pane width from localStorage on mount', async () => {
    ...
    expect((aside as any).style.width).toBe('800px');
  });
  ...
  it('widens the pane when the resize handle is dragged left', async () => {
    ...
    expect((aside as any).style.width).toBe('660px');
  ...
  it('loads per-session width when sessionId changes', async () => {
    ...
    expect((aside as any).style.width).toBe('420px');
    ...
    expect((aside as any).style.width).toBe('900px');
  });
});
