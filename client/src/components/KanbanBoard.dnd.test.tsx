...
(vi as any).mock('@dnd-kit/sortable', () => ({
...
const makeBoard = (cards: any = [], columns: any) => ({
  board: { id: 'b1' },
  columns: columns || [
...
const renderBoard = async (cards: any, columns?: any) => {
  (api.getBoard as any).mockResolvedValue(makeBoard(cards, columns));
...
const dragEnd = async ({ activeId, overId, activeRect, overRect }: any) => {
  await act(async () => {
...
describe('KanbanBoard DnD integration (real handleDragEnd)', () => {
  ...
  it('cross-column TOP-half drop inserts BEFORE the hovered card', async () => {
    ...
    // B stays at 0.
    expect((api.moveCard as any).mock.calls.filter((c: any) => c[1] === 'card-b')).toEqual([]);
  });
  ...
  it('cross-column BOTTOM-half drop inserts AFTER the hovered card (regression)', async () => {
    ...
    expect((api.moveCard as any).mock.calls.filter((c: any) => c[1] === 'card-b')).toEqual([]);
    expect((api.moveCard as any).mock.calls.filter((c: any) => c[1] === 'card-c')).toEqual([]);
  });
