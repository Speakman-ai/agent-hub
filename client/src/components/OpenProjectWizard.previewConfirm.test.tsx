 ...
 */
function seedWizardAtReviewStep(extras: any = {}) {
...
describe('OpenProjectWizard — preview-defaults confirmation', () => {
   ...
   */
  function fireCloneWs(detail: any) {
    act(() => {
  ...
  it('PATCHes the project with the accepted preview config after onboard succeeds', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /create project/i } as any) as any);

    await waitFor(() => {
      expect(onProjectCreated!).toHaveBeenCalledTimes(1);
    });
    expect(fetchMock!).toHaveBeenCalledTimes(2);
    const [onboardCall, patchCall] = (fetchMock as any).mock.calls;
  ...
  it('skips the PATCH when the user never confirmed a preview decision', async () => {
    ...
    expect((fetchMock as any).mock.calls[0][0]).toBe('/api/projects/onboard');
  });
});
