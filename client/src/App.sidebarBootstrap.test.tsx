...
describe('App — sidebar loading vs projects/sessions', () => {
  ...
  it('does not clear session loading on stale getSessions if activeAgentId changes (race)', async () => {
    ...
    expect(typeof (globalThis as any).__ahTestSelectAgent).toBe('function');
    await act(async () => {
  ...
  it('does not snap back to prior agent when switching with an active session', async () => {
    ...
    const callsBefore = (api.getSessions as any).mock.calls.length;
    await act(async () => {
      (globalThis as any).__ahTestSelectAgent('agent-2');
    });
    await waitFor(
    ...
    await waitFor(() => {
      expect((api.getSessions as any).mock.calls.length).toBe(callsBefore + 1);
    });
  });
});
