...
vi.mock('./components/Sidebar.jsx', () => ({
  default: function MockSidebar(p) {
    if (typeof globalThis !== 'undefined' && p.onOpenProject) {
      globalThis.__ahNewProjectCTA = p.onOpenProject;
    }
    return (
      <div
...
vi.mock('./components/NewProjectAdaptiveFlow.jsx', () => ({
  default: function MockAdaptiveFlow(p) {
    if (typeof globalThis !== 'undefined') {
      globalThis.__ahAdaptiveOnClose = p.onClose;
      globalThis.__ahAdaptiveOnProjectCreated = p.onProjectCreated;
    }
...
describe('App — "+ New Project" CTA routes to adaptive flow', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    localStorage.clear();
    delete globalThis.__ahNewProjectCTA;
    delete globalThis.__ahAdaptiveOnClose;
    delete globalThis.__ahAdaptiveOnProjectCreated;
  ...
  it('mounts NewProjectAdaptiveFlow when the CTA fires, and unmounts on onClose', async () => {
    ...
    await waitFor(
      () => {
        expect(typeof globalThis.__ahNewProjectCTA).toBe('function');
      },
      { timeout: 3000 },
    );
    ...
    expect(screen.queryByTestId('legacy-wizard-mock')).not.toBeInTheDocument();
    expect(typeof globalThis.__ahAdaptiveOnClose).toBe('function');

    // Fire the flow's onClose — the adaptive mount should come down.
    await act(async () => {
      globalThis.__ahAdaptiveOnClose();
    });
