...
test.describe('Create project — happy path', () => {
  test('wizard → provisioning → audit → roster → landing', async ({ page, seed }) => {
    ...
    const capturedRosterPost: Array<{ tracks?: Array<{ id?: string; agentId?: string | null }> }> =
      [];
    ...
    await page.route('**/api/projects/*/roster', async (route) => {
      ...
      await route.fulfill({
        ...
        contentType: 'application/json',
        body: JSON.stringify({
          tracks: (body.tracks || []).map(
            (t: { id?: string; label?: string; agentId?: string | null; custom?: boolean }) => ({
              id: t.id,
              label: t.label || t.id,
    ...
    const frontendRow = lastPost.tracks?.find(
      (t: { id?: string; agentId?: string | null }) => t.id === 'frontend',
    );
