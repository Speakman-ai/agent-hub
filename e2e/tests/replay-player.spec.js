...
const read = (rel) => readFileSync(path.join(clientDir, rel), 'utf8');
  ...
  test('renders a real recording inside the CSP-locked sandbox iframe', async ({ page }) => {
    ...
    const events = await page.evaluate(
      () =>
        new Promise((resolve) => {
          const evts = [];
          const stop = window.rrweb.record({ emit: (e) => evts.push(e) });
          setTimeout(() => {
    ...
    await page.evaluate((doc) => {
      document.getElementById('player').srcdoc = doc;
    }, srcDoc);

    // 3. Wait for the sandbox to announce ready, then stream the events in.
    await page.waitForFunction(() => window.__signals.some((s) => s.type === 'ready'), null, {
      timeout: 20_000,
    });
    await page.evaluate(
      ({ ch, evts }) => {
        const win = document.getElementById('player').contentWindow;
    ...
    await page.waitForFunction(() => window.__signals.some((s) => s.type === 'playing'), null, {
      timeout: 20_000,
    });
