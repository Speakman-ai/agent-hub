 ...
 * in the design doc:
 *
 *   • Button is visible for card-linked sessions and labeled
 *     "Finalize Code Changes" in the idle state.
 *   • Clicking the button POSTs `/api/projects/:projectId/cards/:cardId/finalize`.
 *   • On 200, the button optimistically transitions to a disabled state
 *     whose label includes "Finalizing" within 5s.
 *   • Sessions that are NOT linked to a card do not render the button
...
test.describe('Finalize Code Changes button', () => {
  test('idle → click → disabled "Finalizing" state', async ({ page, seed, request }) => {
    ...
    await page.route(
      `**/api/projects/${project.id}/cards/${card.id}/finalize`,
      async (route) => {
    ...
    await page.route(
      `**/api/projects/${project.id}/finalize/*/cancel`,
      async (route) => {
        if (route.request().method() !== 'POST') return route.continue();
        await route.fulfill({
          status: 200,
    ...
    // ── Assertion 1: button mounts in the idle state ──────────────────
    ...
    await expect(finalizeBtn).toContainText('Finalize Code Changes');

    // ── Assertion 2: clicking POSTs to the start endpoint ─────────────
    await finalizeBtn.click();
    await expect.poll(() => finalizeStartCalls.length, { timeout: 5000 }).toBeGreaterThan(0);

    // ── Assertion 3: button transitions to disabled "Finalizing" ──────
    //
    ...
    // refetch to land. Either path must settle to disabled + "Finalizing"
    // within 5s of the click.
    await expect(finalizeBtn).toBeDisabled({ timeout: 5000 });
    await expect(finalizeBtn).toContainText(/Finalizing/i, { timeout: 5000 });

    // The cancel affordance renders alongside the disabled state.
    await expect(page.getByTestId('finalize-code-changes-cancel')).toBeVisible({
      timeout: 5000,
    });
  });
