...
export function buildSkillBuilderModePreamble(args: {
  ...
  return [
    ...
    '2. **Interview** for trigger phrases, inputs/outputs, success criteria, and reusable authentication/configuration required by external integrations. Use `agenthub:ask` pickers when helpful; bias toward action.',
    ...
    '- External integrations that need a standing API key, token, username, or password must declare those fields in `credentials:` frontmatter. Never put secret values in SKILL.md.',
