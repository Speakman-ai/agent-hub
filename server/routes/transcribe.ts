 ...
 *   (also overridable per-request with the `X-Transcription-Provider` header):
 *     - `openai` (default) — OpenAI Whisper (`/v1/audio/transcriptions`) with
 ...
 * per-request override, so a caller cannot force the other configured
 * provider/key. Unknown / unset values resolve to the safe default (`openai`).
 */
