...
export function buildDevServerKickoffPrompt(
  ...
  return [
    ...
    '`prEnv.devServer` (optional build command, start command, port map, env,',
    'secret references, health path, monorepo cwd). This is a **worktree-backed**',
    'session on a fresh',
    ...
    '2b. **Build command (optional)** — Ask whether the app needs a one-time build before the server starts (e.g. `npm run build`, `docker compose build`, a bundler/compile pass) that is expensive and only changes when code changes. If so, set `buildCommand`; it runs to completion before `startCommand` (after any apt packages) and a non-zero exit fails the start. Leave it unset when `startCommand` already builds on the fly (e.g. `npm run dev`). Setting it enables the toolbar’s **Rebuild App** vs **Restart Server** split — Restart Server reuses the last build.',
