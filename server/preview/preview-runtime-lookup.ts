/**
 * Minimal preview runtime surface for port lookup, worktree sync, and
 * proxies. The full {@link DevServerRuntime} class is much wider.
 */
 ...
 * port. A dev server can expose several `portMap` entries, so the lookup
 * takes an optional internal port: omitted → the primary host port; set →
 * the mapped host port for that specific internal port (the
 * `/preview/proxy/p/<internalPort>` sub-mount).
 */
 ...
 * keeps the primary `/preview/proxy/` mount, extras resolve to
