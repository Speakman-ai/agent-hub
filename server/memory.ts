...
/**
 * Options accepted by the memory reconciliation paths.
 *
 * Historically this carried `{claudeBin, spawnEnv, cwd}` and shelled out
 * directly to Claude. The reconciliation flow now goes through the
 * shared one-shot prompt resolver so memory writes survive a missing
 * Claude install — Cursor/Codex/Gemini are all valid drivers when
 * Claude is unavailable.
 */
export interface MemoryRunOptions {
  /**
   * Whole AppConfig — needed by the resolver to read every engine's
   * binary path / api-key. Callers always have this in scope.
   */
  cfg: AppConfig;
  ...
  timeoutMs?: number;
  /**
   * Caller's preferred engine. Defaults to `claude-code` to preserve
   * the historical behaviour for users who do have Claude set up; the
...
/**
 * Run a one-shot prompt for the memory reconciliation flow.
 *
 * Trust assumptions for `--permission-mode bypassPermissions` (Claude path):
...
async function runMemoryPrompt(
  ...
  if (resolved.fallbackUsed) {
    console.warn(
      `[Memory Reconciliation] Preferred engine "${preferredEngine ?? 'claude-code'}" unavailable (${resolved.fallbackFromReason}); using "${resolved.engine}".`,
    );
  }
...
export function reconcileMemoryAfterSession(
  ...
  return withReconcileLock(async () => {
    ...
    try {
      const result = await runMemoryPrompt(prompt, systemPrompt, opts);
...
export function reconcileMemoryFromWiki(
  ...
  return withReconcileLock(async () => {
    ...
    try {
      const result = await runMemoryPrompt(prompt, systemPrompt, {
        ...opts,
        timeoutMs: 5 * 60 * 1000,
      });
      if (!result || result.includes('NO_CHANGES_NEEDED')) {
