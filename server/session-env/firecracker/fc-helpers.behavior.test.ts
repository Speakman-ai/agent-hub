...
describe('fc-prepare-disks.sh worktree copy', () => {
  ...
  /**
   * Production-shaped privilege boundary: uid 1000 with NOPASSWD sudo (the
   * Finalize helper image's `runner` account) can regain root via `sudo -n`
   * after a plain setpriv drop, but not when `--no-new-privs` is set.
   *
   * Requires root (to install a temporary sudoers grant). The Finalize runner
   * Dockerfile also runs this same check at image build time as root so CI
   * that only ever executes Vitest as `runner` still gates the regression.
   */
  it.skipIf(process.platform !== 'linux' || process.getuid?.() !== 0)(
    'no_new_privs blocks sudo regain for a sudo-enabled uid (smudge-filter class)',
    () => {
      const base = mkdtempSync(path.join(os.tmpdir(), 'fc-nopriv-'));
      const sudoers = path.join(base, 'sudoers');
      const probeUid = 61999;
      const probeGid = 61999;
      try {
        // Mirror the helper image: a non-root account with passwordless sudo.
        writeFileSync(sudoers, `nopriv-probe ALL=(ALL) NOPASSWD:ALL\n`, { mode: 0o440 });
        const addGroup = spawnSync('groupadd', ['-g', String(probeGid), 'nopriv-probe'], {
          encoding: 'utf8',
        });
        // group may already exist from a prior crash — tolerate.
        if (addGroup.status !== 0 && !/already exists/.test(addGroup.stderr || '')) {
          expect(addGroup.status, addGroup.stderr).toBe(0);
        }
        const addUser = spawnSync(
          'useradd',
          ['-u', String(probeUid), '-g', String(probeGid), '-M', '-s', '/bin/bash', 'nopriv-probe'],
          { encoding: 'utf8' },
        );
        if (addUser.status !== 0 && !/already exists/.test(addUser.stderr || '')) {
          expect(addUser.status, addUser.stderr).toBe(0);
        }
        spawnSync('bash', ['-c', `cat '${sudoers}' > /etc/sudoers.d/nopriv-probe`], {
          encoding: 'utf8',
        });
        ...
        expect(unprotected.status, `${unprotected.stdout}\n${unprotected.stderr}`).toBe(0);
        expect(unprotected.stdout.trim()).toBe('0');

        const protectedDrop = spawnSync(
          'setpriv',
          [
            `--reuid=${probeUid}`,
            `--regid=${probeGid}`,
        ...
        expect(protectedDrop.status, `${protectedDrop.stdout}\n${protectedDrop.stderr}`).not.toBe(
          0,
        );
      } finally {
        spawnSync('rm', ['-f', '/etc/sudoers.d/nopriv-probe']);
        spawnSync('userdel', ['nopriv-probe']);
        spawnSync('groupdel', ['nopriv-probe']);
        rmSync(base, { recursive: true, force: true });
      }
    },
  );

  it('Finalize runner Dockerfile gates the no_new_privs sudo-regain regression at build time', () => {
    const dockerfile = readFileSync(path.join(here, '../../finalize/runner/Dockerfile'), 'utf8');
    // Must run as root before `USER runner`, against the real sudo-enabled account.
    expect(dockerfile).toMatch(/NOPASSWD:ALL/);
    expect(dockerfile).toMatch(/no_new_privs blocks sudo regain/);
    expect(dockerfile).toMatch(/setpriv .*--no-new-privs/);
    expect(dockerfile).toMatch(/--reuid=runner/);
  });
