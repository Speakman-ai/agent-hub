...
describe('workflow runner', () => {
  ...
  it('completes a one-step run with merged payload (mocked CLI)', async () => {
    ...
    const stepId = uuidv4();
    const runId = uuidv4();
    stmts!.createWorkflow.run(wfId, projectId, 'Flow', 'manual', '{"a":0}', null, null, null, null);
  ...
  it('on_failure=continue: run succeeds when the only step always fails', async () => {
    ...
    const stepId = uuidv4();
    const runId = uuidv4();
    stmts!.createWorkflow.run(wfId, projectId, 'Continue', 'manual', '{}', null, null, null, null);
    stmts!.createWorkflowStep.run(
      stepId,
      wfId,
      agentId,
