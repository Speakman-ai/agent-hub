/**
 * Agent Hub API Test Suite
 *
 * Comprehensive integration tests for the Express REST API.
 * Tests are grouped by resource (projects, agents, sessions, kanban, wiki, etc.)
 * and run against a real SQLite database in a temp directory.
 */
...
describe('Agents', () => {
  ...
  describe('GET /api/agents', () => {
    it('lists all agents with enriched data', async () => {
      await createAgent(); // ensure at least one exists
...
describe('Sessions', () => {
  ...
  describe('GET /api/agents/:agentId/sessions', () => {
    it('lists sessions for an agent', async () => {
      const agent = await createAgent();
      await createSession({ agentId: agent.id, name: 'S1' });
      await createSession({ agentId: agent.id, name: 'S2' });
...
describe('Kanban Board', () => {
  ...
  describe('Cards CRUD', () => {
    it('creates a card', async () => {
      const card = await createCard(testProject.id, { title: 'Test Task', priority: 'high' });
    ...
    it('lists cards for a project', async () => {
      await createCard(testProject.id);
    ...
    it('updates a card', async () => {
      const card = await createCard(testProject.id);
      const res = await request
    ...
    it('moves a card between columns', async () => {
      ...
      const doneCol = boardRes.body.columns.find((c) => c.name === 'Done');
  ...
  describe('Card Comments', () => {
    it('creates and lists comments on a card', async () => {
      ...
      // Create comment (returns array of all comments for the card)
    ...
    it('deletes a comment', async () => {
      ...
      // Create returns an array of all comments; pick the first one
      const commentId = commentRes.body[0].id;
      await request
  ...
  describe('Columns', () => {
    it('creates a custom column', async () => {
      // Column create returns array of all columns for the board
      const res = await request
      ...
      expect(Array.isArray(res.body)).toBe(true);
      const qa = res.body.find((c) => c.name === 'QA');
    ...
    it('updates a column name', async () => {
      ...
      const tempCol = createRes.body.find((c) => c.name === 'Temp Col');

      // Update requires name + position (both are used in the SQL UPDATE)
      const res = await request
    ...
    it('deletes a column', async () => {
      ...
      const delCol = createRes.body.find((c) => c.name === 'Delete Me Col');
...
describe('Wiki', () => {
  ...
  describe('GET /api/projects/:projectId/wiki/:slug', () => {
    it('returns a wiki page by slug', async () => {
      const page = await createWikiPage(testProject.id, { title: 'Slug Test Page' });
      const res = await request
  ...
  describe('PUT /api/projects/:projectId/wiki/:slug', () => {
    it('updates a wiki page', async () => {
      const page = await createWikiPage(testProject.id, { title: 'Update Me' });
      const res = await request
...
describe('Rooms', () => {
  ...
  describe('Room agents', () => {
    it('adds and removes agents from a room', async () => {
      ...
      // Verify agent is in the room (room GET returns enriched agents with `id` field)
