...
export const MAX_PENDING_CONTEXT_BYTES = 128 * 1024;
...
A wiki write from this session auto-stamps the linked card as reviewed (\`documented = 1\`). If you skip writing, that is fine: on merge the project's docs agent reviews the landed change. Historical undocumented cards stay queued until someone asks to backfill (\`wiki.sh document-backfill\`).`;
        ...
        if (projectMode !== 'workflow') {
          const linkedCardLine = options.sessionHasLinkedCard
            ? `**This session is already linked to a kanban card** (the card whose assignment spawned you). Do NOT create another card for this task — pick up the linked card, move it to In Progress while you work, and self-report progress via comments on that card. Do **not** move it to Done yourself: Done means merged, and the platform closes the card automatically when your change lands. Only create *new* cards for genuinely separate follow-up work you discover along the way.`
...
\`\`\`
**These shells are watched by default, so end your turn after starting one.** The Hub wakes this session for an agent progress check-in every **10 minutes** (deferred while busy), and when the command finishes. Commands have **no automatic runtime cutoff**. At each check-in, assess the captured output, process liveness, and durable progress; silence alone does not mean a command is stuck. Let progressing commands continue and stop a command only with evidence that it cannot make progress. Do not poll or sleep-loop. Pass \`--no-watch\` to disable progress and completion wakes. Pass \`--timeout-sec <n>\` only when an explicit deadline is needed; values above 30 minutes are supported.

The command runs in the session worktree and its output streams to the session's **Background shells panel**, where the human can see what you are waiting on and cancel it. Statuses are \`running\`, \`exited\` (code 0), \`failed\` (non-zero or crashed), \`stopped\`, and \`timed_out\` (the cap fired). On \`timed_out\`, the explicitly requested deadline fired; inspect durable progress before retrying. Everything is scoped to this session and is reaped when the session is archived or deleted; a Hub restart also kills them and you get a system message listing what died. Keep using your normal Bash tool for work that finishes inside the turn.`;
...
**This is the mechanism for collecting a username/password or any one-off secret the user must provide interactively** — a web-app login, a one-time token, anything you would otherwise be tempted to ask for in chat prose. In that situation emit an \`agenthub:credential-request\` block immediately: do not tell the user "there is no secure prompt", and do not reach for an external secret manager (Agent Hub has **no** 1Password / \`op\` CLI, Bitwarden, vault, or service-token integration, so an \`OP_SERVICE_ACCOUNT_TOKEN\` or a paste into chat is never the answer — the masked card is the secure channel). **Never route the secret itself through the \`agenthub:ask\` picker; its input renders in plaintext.** The picker is only for the *non-secret* parts of a login: if a skill needs an environment or similar choice (e.g. dev vs prod), ask that first with \`agenthub:ask\`, then collect the username/password through this \`agenthub:credential-request\` block.

**Reusable integration tokens are the exception — do not request them here.** When an installed skill declares a persistent credential in its \`credentials:\` frontmatter (e.g. a GitHub PAT), that token is stored once by the user under **Settings → My Skill Credentials** and Agent Hub injects it into your environment on every spawn. Check your environment variables first; if the token is missing, point the user to that Settings credential flow rather than firing a one-time \`agenthub:credential-request\`. Use the session block for interactive one-off secrets; use the persistent skill-credential store for standing integration tokens.
