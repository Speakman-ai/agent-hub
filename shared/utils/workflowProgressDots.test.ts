...
describe('workflowProgressDots', () => {
  it('orders steps by step_order', () => {
    ...
    expect(orderedStepsFromWorkflow(wf).map((s: any) => s.id)).toEqual(['a', 'b']);
  });
  ...
  it('buildWorkflowStepDots aligns runs with ordered steps', () => {
    ...
    expect(dots.map((d: any) => d.kind)).toEqual(['success', 'pending']);
    expect(dots.map((d: any) => d.title)).toEqual(['One', 'Two']);
  });
});
