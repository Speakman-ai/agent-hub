...
resource "aws_instance" "app" {
  ...
  lifecycle {
    ...
    # `ops/scripts/sync-hub-env.sh` against the `hub_env_managed` and
    # `hub_env_managed_keys` outputs, which upserts the managed .env keys,
    # removes owned keys the config stopped emitting, and restarts the container
    # in place. That is the same path used by hand for the
    # FINALIZE_FLEET_MAX_AGENTS fleet bump.
    #
