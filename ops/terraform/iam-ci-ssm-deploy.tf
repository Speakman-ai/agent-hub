...
locals {
  ...
  # Every box CI may SSM, mirrored from the rollout workflows' repo Variables.
  # The singular input is retained for local/backward-compatible callers and
  # unioned with the complete list CI supplies. Deliberately absent from
  ...
  ci_ssm_expected_instance_ids = sort(toset(concat(
    trimspace(var.ci_ssm_expected_deploy_instance_id) != "" ? [trimspace(var.ci_ssm_expected_deploy_instance_id)] : [],
    [for id in var.ci_ssm_expected_deploy_instance_ids : trimspace(id) if trimspace(id) != ""],
  )))
  ci_ssm_missing_expected_instance_ids = sort(tolist(setsubtract(
    toset(local.ci_ssm_expected_instance_ids),
    toset(local.ci_ssm_target_instance_ids),
  )))
  ci_ssm_expected_covered = length(local.ci_ssm_missing_expected_instance_ids) == 0
...
# DOCKER_DEPLOY_INSTANCE_ID / DOCKER_DEPLOY_DEV_INSTANCE_ID repo Variables
# (which the rollout workflows SSM) and the ci_ssm_deploy_* tfvars (which scope
# the grant). Nothing in the tree can see both, so release CI passes both in as
# TF_VAR_ci_ssm_expected_deploy_instance_ids and this resource asserts the grant
# covers every non-empty target. The precondition on the policy only proves something
...
resource "terraform_data" "ci_ssm_deploy_target_guard" {
  # Keyed on the expected ids rather than on ci_ssm_deploy_enabled: disabling the
  ...
  count = length(local.ci_ssm_expected_instance_ids) > 0 ? 1 : 0
  ...
  input = {
    expected = local.ci_ssm_expected_instance_ids
  ...
  lifecycle {
    precondition {
      ...
      error_message = "CI expects to SSM [${join(", ", local.ci_ssm_expected_instance_ids)}] (DOCKER_DEPLOY_INSTANCE_ID / DOCKER_DEPLOY_DEV_INSTANCE_ID repo Variables) but this workspace creates no grant: enable_ci_ssm_deploy_after_ecr_push is false, or neither ci_ssm_deploy_instance_id / ci_ssm_deploy_instance_ids nor ci_ssm_deploy_instance_tags is set. Every rollout would fail with AccessDeniedException on ssm:SendCommand. Enable the grant here, or clear the ci_ssm_expected_deploy_instance_* assertion inputs if a different workspace owns it."
    }
    precondition {
      condition     = local.ci_ssm_expected_covered
      error_message = "CI ssm:SendCommand grant does not cover every box CI restarts. Missing [${join(", ", local.ci_ssm_missing_expected_instance_ids)}] from expected [${join(", ", local.ci_ssm_expected_instance_ids)}] (DOCKER_DEPLOY_INSTANCE_ID / DOCKER_DEPLOY_DEV_INSTANCE_ID repo Variables, passed in as TF_VAR_ci_ssm_expected_deploy_instance_ids); ci_ssm_deploy_instance_id / ci_ssm_deploy_instance_ids / ci_ssm_deploy_instance_tags resolved [${join(", ", local.ci_ssm_target_instance_ids)}] in ${var.aws_region}. A deploy box was most likely replaced: point ci_ssm_deploy_instance_tags at its Name tag and/or set ci_ssm_deploy_instance_id / ci_ssm_deploy_instance_ids to the new id(s). Failing here is the cheap version — the alternative is a mid-rollout AccessDeniedException."
    }
  }
}

output "ci_ssm_deploy_target_instance_ids" {
  description = "Instance ids the CI role may SSM after an ECR push. Must contain every configured DOCKER_DEPLOY_INSTANCE_ID / DOCKER_DEPLOY_DEV_INSTANCE_ID repo Variable."
