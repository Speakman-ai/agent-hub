 ...
 */
export function buildUpsertSecretsPayload(existing, entry) {
  const rows = Array.isArray(existing) ? existing.filter(Boolean) : [];
  const key = (entry.key || '').trim();
  const next = rows
    .filter((r) => r.key !== key)
    .map((r) => ({ key: r.key, value: r.value, kind: r.kind }));
...
export function displaySecretValue(row) {
  if (!row) return '';
 ...
 */
export function describeSecretsPermissionError(err, action = 'read') {
