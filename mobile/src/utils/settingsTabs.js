 ...
 */
export function normalizeSettingsTab(rawTab, validIds, legacyIds) {
  if (!rawTab) return 'general';
  const isValid =
    validIds instanceof Set ? validIds.has(rawTab) : Array.isArray(validIds) && validIds.includes(rawTab);
