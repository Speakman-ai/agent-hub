...
describe('queueIndicatorText', () => {
    ...
    it('uses singular "message" for queue length 1', () => {
        expect(queueIndicatorText({ roomProcessing: true, roomQueueLength: 1 })).toBe('1 message queued — will be sent after agents finish');
    });
    it('uses plural "messages" for queue length > 1', () => {
        expect(queueIndicatorText({ roomProcessing: true, roomQueueLength: 4 })).toBe('4 messages queued — will be sent after agents finish');
    });
    it('coerces non-number queue lengths safely', () => {
        expect(queueIndicatorText({ roomProcessing: true, roomQueueLength: '2' })).toContain('2 messages');
