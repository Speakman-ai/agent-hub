...
describe('describeResolvedTarget', () => {
  ...
  it('formats a plain-text description for the RN <Text> child', () => {
    expect(describeResolvedTarget({ branch: 'feat/ci', sessionId: 'sess-1' })).toBe(
      'Branch feat/ci in session sess-1',
    );
  });
});
describe('pickFinalizeStatus', () => {
  it('returns null when no wizard has been spawned yet', () => {
    ...
    expect(
      pickFinalizeStatus({ lastSessionId: '', target: { branch: 'x', sessionId: 'y' } }),
    ).toBeNull();
  });
...
describe('shouldRefreshOnWizardComplete', () => {
  it('returns false for the started event (refresh only on complete)', () => {
    expect(
      shouldRefreshOnWizardComplete({ type: 'finalize_wizard_started', projectId: 'p1' }, 'p1'),
    ).toBe(false);
  });
  it('returns true when the complete event matches the current project', () => {
    expect(
      shouldRefreshOnWizardComplete({ type: 'finalize_wizard_complete', projectId: 'p1' }, 'p1'),
    ).toBe(true);
  });
  it('returns false when the event is for a different project', () => {
    expect(
      shouldRefreshOnWizardComplete({ type: 'finalize_wizard_complete', projectId: 'other' }, 'p1'),
    ).toBe(false);
  });
  ...
  it('returns false when no current project is selected', () => {
    expect(
      shouldRefreshOnWizardComplete({ type: 'finalize_wizard_complete', projectId: 'p1' }, ''),
    ).toBe(false);
  });
});
