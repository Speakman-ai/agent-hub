 ...
 * @returns {string|null}
 */
export function pickLinkedPrUrl(summary) {
  return (
 ...
 */
export function linkedPrBadge(summary) {
  if (!pickLinkedPrUrl(summary)) return null;
  const review = String(summary?.linkedCard?.review_status || '').toLowerCase();
  if (review === 'merged') return { key: 'merged', label: 'Merged', tone: 'purple' };
  if (review === 'changes_requested')
    return { key: 'pending_revisions', label: 'Pending revisions', tone: 'red' };
  if (review === 'approved') return { key: 'approved', label: 'Approved', tone: 'emerald' };
 ...
 */
export function normalizeSessionSummary(data) {
 ...
 * @returns {boolean}
 */
export function shouldShowViewChanges(session) {
 ...
 */
export function splitSessionRoster(sessionAgents) {
  const list = Array.isArray(sessionAgents) ? sessionAgents.filter(Boolean) : [];
  return {
    executor: list.find((a) => a.role === 'executor') || null,
    advisors: list.filter((a) => a.role === 'advisor'),
  };
}
