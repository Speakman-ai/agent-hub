...
export function prStateBadge(pr: any) {
    ...
    if (pr.draft)
        return { label: 'draft', color: colors.gray400, bg: colors.gray700_40 };
    const s = (pr.state || '').toLowerCase();
    if (s === 'open')
        return { label: 'open', color: colors.emerald400, bg: colors.emerald900_40 };
    if (s === 'closed')
        return { label: 'closed', color: colors.red400, bg: colors.red900_50 };
...
export function summarizeReviews(reviews: any) {
    ...
    for (const r of sorted) {
        ...
        if (state === 'COMMENTED' && prev && prev !== 'COMMENTED')
            continue;
        byUser.set(key, state);
    }
    const states = [...byUser.values()];
    if (states.some((s: any) => s === 'CHANGES_REQUESTED'))
        return 'changes_requested';
    if (states.some((s: any) => s === 'APPROVED'))
        return 'approved';
    if (states.some((s: any) => s === 'COMMENTED'))
        return 'commented';
    if (states.some((s: any) => s === 'PENDING'))
        return 'pending';
    return 'none';
}
/**
...
export function reviewDecisionListBadge(decision: any) {
    ...
    if (d === 'APPROVED')
        return reviewsBadge('approved');
    if (d === 'CHANGES_REQUESTED')
        return reviewsBadge('changes_requested');
    if (d === 'REVIEW_REQUIRED')
        return reviewsBadge('pending');
    return null;
}
/**
...
export function prListRowSuggestsResolvableWork(pr: any) {
    ...
    if (LIST_ROW_CONFLICT_MERGE_STATES.has(ms))
        return true;
    if (ms === 'unstable')
        return true;
    const rollup = pr.check_rollup;
    if (Array.isArray(rollup) && rollup.length > 0) {
        if (summarizeChecks(rollup).overall === 'failure')
            return true;
    }
    if (String(pr.review_decision || '').toUpperCase() === 'CHANGES_REQUESTED')
        return true;
    return false;
}
/**
...
export function prListRowResolveDisabledHeuristic(pr: any) {
    if (!pr || String(pr.state || '').toLowerCase() !== 'open')
        return true;
    if (prListRowSuggestsResolvableWork(pr))
        return false;
    if (pr.mergeable !== true && pr.mergeable !== false)
        return false;
    const rollup = pr.check_rollup;
