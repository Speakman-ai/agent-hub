...
import { buildDesignFileUrl, pickEntryFile, formatFileSize } from './designFiles';
...
describe('buildDesignFileUrl', () => {
    ...
    it('builds a plain path with the static mount prefix', () => {
        expect(buildDesignFileUrl(BASE, 'sess-1', 'index.html')).toBe(`${BASE}/session-files/sess-1/design/index.html`);
    });
    it('encodes URL-significant characters in each segment', () => {
        // Spaces, #, ?, % would otherwise break or truncate the link.
        expect(buildDesignFileUrl(BASE, 'sess-1', 'my page.html')).toBe(`${BASE}/session-files/sess-1/design/my%20page.html`);
        expect(buildDesignFileUrl(BASE, 'sess-1', 'a#b?c%d.css')).toBe(`${BASE}/session-files/sess-1/design/a%23b%3Fc%25d.css`);
    });
    it('preserves / separators while encoding nested segments', () => {
        expect(buildDesignFileUrl(BASE, 'sess-1', 'assets/my icons/logo (1).svg')).toBe(`${BASE}/session-files/sess-1/design/assets/my%20icons/logo%20(1).svg`);
    });
});
