...
import { canReceivePush, resolveExpoProjectId, buildDeviceRegistrationBody, registerForPushNotifications, presentLocalNotification, } from './push';
...
describe('resolveExpoProjectId', () => {
    it('prefers expoConfig.extra.eas.projectId', () => {
        expect(resolveExpoProjectId({ expoConfig: { extra: { eas: { projectId: 'abc' } } } })).toBe('abc');
    });
    it('falls back to legacy manifest shapes', () => {
        expect(resolveExpoProjectId({ manifest: { extra: { eas: { projectId: 'legacy1' } } } })).toBe('legacy1');
...
describe('presentLocalNotification', () => {
    it('schedules a trigger-less notification', async () => {
        ...
        const id = await presentLocalNotification({ Notifications }, { title: 'T', body: 'B', data: { k: 1 } });
        expect(id).toBe('id-1');
