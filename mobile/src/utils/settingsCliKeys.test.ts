...
import { CLI_KEY_PROVIDERS, providerKeyConfigured, providerStatusLabel, buildPutMyAuthBody, codexDeviceLoginLabel, githubStatusLabel, } from './settingsCliKeys';
describe('CLI_KEY_PROVIDERS', () => {
    it('covers all five engines with unique ids', () => {
        const ids = CLI_KEY_PROVIDERS.map((p: any) => p.id);
        expect(ids).toEqual(['claude', 'cursor', 'gemini', 'codex', 'grok']);
    });
    it('grok pastes an xAI key and routes to /auth/me/grok-auth', () => {
        const grok = CLI_KEY_PROVIDERS.find((p: any) => p.id === 'grok');
        ...
        expect(providerStatusLabel('grok', { apiKey: null, hostConfigFallback: { apiKey: true } })).toBe('Using host-configured key');
    });
});
describe('providerKeyConfigured', () => {
    it('claude: either API key or OAuth token counts', () => {
        ...
        expect(providerKeyConfigured('claude', { anthropicApiKey: null, claudeCodeOAuthToken: null })).toBe(false);
    });
...
describe('providerStatusLabel', () => {
    it('claude combines key and oauth state', () => {
        expect(providerStatusLabel('claude', {
            anthropicApiKey: 'sk-…wxyz',
            claudeCodeOAuthToken: 'tok',
            claudeCodeOAuthExpired: false,
        })).toBe('API key sk-…wxyz · OAuth token configured');
        expect(providerStatusLabel('claude', { claudeCodeOAuthToken: 'tok', claudeCodeOAuthExpired: true })).toBe('OAuth token (expired)');
    ...
    it('single-key engines show key, host fallback, or not configured', () => {
        ...
        expect(providerStatusLabel('gemini', { apiKey: null, hostConfigFallback: { apiKey: true } })).toBe('Using host-configured key');
...
describe('codexDeviceLoginLabel', () => {
    ...
    it('labels the known statuses', () => {
        expect(codexDeviceLoginLabel({ deviceLogin: { uiStatus: 'authenticated', oauth: { loggedIn: true } } })).toBe('Signed in with ChatGPT');
    ...
    it('passes through unknown statuses readably', () => {
        expect(codexDeviceLoginLabel({ deviceLogin: { uiStatus: 'some_new_state' } })).toBe('some new state');
    });
});
