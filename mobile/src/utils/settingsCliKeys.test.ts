...
describe('CLI_KEY_PROVIDERS', () => {
  ...
  it('grok pastes an xAI key and routes to /auth/me/grok-auth', () => {
    ...
    expect(
      providerStatusLabel('grok', { apiKey: null, hostConfigFallback: { apiKey: true } }),
...
describe('codexDeviceLoginLabel', () => {
  ...
  it('labels the known statuses', () => {
    expect(
      codexDeviceLoginLabel({
        deviceLogin: { uiStatus: 'authenticated', oauth: { loggedIn: true } },
      }),
    ).toBe('Signed in with ChatGPT');
    expect(codexDeviceLoginLabel({ deviceLogin: { uiStatus: 'authenticated' } })).toBe(
      'Authenticated',
    );
    expect(codexDeviceLoginLabel({ deviceLogin: { uiStatus: 'pending' } })).toBe(
      'Sign-in in progress…',
    );
