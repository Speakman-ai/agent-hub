...
const card = (id, columnId = 'c1') => ({ id, column_id: columnId });
...
describe('appendCardPage', () => {
  ...
  it('tolerates null / undefined inputs', () => {
    expect(appendCardPage(null, [card('a')]).map((c) => c.id)).toEqual(['a']);
    expect(appendCardPage([card('a')], null).map((c) => c.id)).toEqual(['a']);
