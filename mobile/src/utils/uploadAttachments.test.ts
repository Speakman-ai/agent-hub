...
describe('uploadAttachments — routes each kind to the right endpoint', () => {
    it('images → api.uploadImage(dataUrl, name)', async () => {
        const api = makeApi();
        const out = await uploadAttachments([{ kind: 'image', name: 'a.png', dataUrl: 'data:image/png;base64,AAA', uri: 'file:///a' }], api);
    ...
    it('generic files → api.uploadFile with octet-stream fallback', async () => {
        const api = makeApi();
        await uploadAttachments([{ kind: 'file', name: 'notes.txt', uri: 'file:///tmp/notes.txt' }], api);
        expect(api.uploadFile).toHaveBeenCalledWith({
