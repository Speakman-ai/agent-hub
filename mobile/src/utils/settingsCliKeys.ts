...
export const CLI_KEY_PROVIDERS = [
    ...
    },
    {
        ...
        placeholder: 'xai-...',
        description: 'Used when sessions you own spawn the Grok Build CLI. On web/desktop you can ' +
...
export function providerStatusLabel(provider: any, body: any) {
    ...
    if (provider === 'claude') {
        ...
        if (body.anthropicApiKey)
            parts.push(`API key ${body.anthropicApiKey}`);
        if (body.claudeCodeOAuthToken) {
            parts.push(body.claudeCodeOAuthExpired
                ? 'OAuth token (expired)'
    ...
    if (body.apiKey)
        return `API key ${body.apiKey}`;
    if (body.hostConfigFallback?.apiKey)
        return 'Using host-configured key';
    return 'Not configured';
}
/**
