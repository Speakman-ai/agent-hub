 ...
 *
 * Mirrored from `client/src/utils/authorAllowlist.js` so the mobile Webhooks
 * settings tab can parse/serialize the same comma-separated input as the web
 * client. The backend stores the allowlist as a JSON string (e.g.
 * `'["mcsteen"]'`) in `webhook_configs.author_allowlist`. The UI represents it
 * as a comma-separated text input for human ergonomics. These helpers handle
 * the two-way conversion plus the usual trim/dedupe/empty-drop hygiene.
 */
