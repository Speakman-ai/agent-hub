...
import { parseTranscribeUploadResult, transcribeAudio, } from './transcribeAudio';
vi.mock('./config', () => ({
...
describe('parseTranscribeUploadResult', () => {
    ...
    it('maps 501 to configuration hint', () => {
        const result = parseTranscribeUploadResult(501, JSON.stringify({ hint: 'Set xAI key' }));
...
describe('transcribeAudio', () => {
    ...
    it('throws user-facing message on provider error', async () => {
        ...
        await expect(transcribeAudio('file:///rec.webm', 'audio/webm', { fileSystem: fs })).rejects.toThrow('bad format');
    });
