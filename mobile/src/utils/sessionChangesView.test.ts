...
describe('worktreeStatusLine', () => {
  ...
  it('describes uncommitted and unpushed states', () => {
    expect(worktreeStatusLine({ hasUncommitted: true, hasUnpushed: false })).toBe(
      'uncommitted changes',
    );
    expect(worktreeStatusLine({ hasUncommitted: false, hasUnpushed: true })).toBe(
      'unpushed commits',
    );
