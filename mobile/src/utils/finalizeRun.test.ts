...
import { isTerminalStatus, isFinalizeBlocked, describeRunPhase, shouldPollFinalizeFallback, FINALIZE_LIVE_FRESH_MS, __test, } from './finalizeRun';
...
describe('isTerminalStatus', () => {
    ...
    it('matches the canonical web-side terminal set', () => {
        expect([...TERMINAL_STATUSES].sort()).toEqual(['pushed', 'failed', 'timed_out', 'infra_error', 'cancelled', 'stalled_no_response'].sort());
    });
});
describe('isFinalizeBlocked', () => {
    ...
    it('matches the canonical web-side blocked set', () => {
        expect([...FINALIZE_BLOCKED_STATUSES].sort()).toEqual(['queued', 'rebasing', 'reviewing', 'running', 'dispatching', 'pushing'].sort());
    });
    it('TERMINAL and FINALIZE_BLOCKED are disjoint', () => {
        const overlap = [...TERMINAL_STATUSES].filter((s: any) => FINALIZE_BLOCKED_STATUSES.has(s));
...
describe('shouldPollFinalizeFallback', () => {
    ...
    const fresh = { sessionId: SID, bump: NOW - 1000 }; // 1s old
    const stale = { sessionId: SID, bump: NOW - (FINALIZE_LIVE_FRESH_MS + 1) };
    it('does not poll when there is no session', () => {
        expect(shouldPollFinalizeFallback({ connected: false, lastEvent: null, sessionId: null, now: NOW })).toBe(false);
    });
    it('skips polling while connected with a fresh event for this session', () => {
        expect(shouldPollFinalizeFallback({ connected: true, lastEvent: fresh, sessionId: SID, now: NOW })).toBe(false);
    });
    it('polls when the socket is disconnected, even with a fresh event (drop case)', () => {
        ...
        expect(shouldPollFinalizeFallback({ connected: false, lastEvent: fresh, sessionId: SID, now: NOW })).toBe(true);
    });
    it('polls when the last event is stale despite being connected (silent / reconnect-missed-frame)', () => {
        expect(shouldPollFinalizeFallback({ connected: true, lastEvent: stale, sessionId: SID, now: NOW })).toBe(true);
    });
    it('polls when the last event belongs to a different session or none seen', () => {
        expect(shouldPollFinalizeFallback({
            connected: true,
            lastEvent: { sessionId: 'other', bump: NOW },
            sessionId: SID,
            now: NOW,
        })).toBe(true);
        expect(shouldPollFinalizeFallback({ connected: true, lastEvent: null, sessionId: SID, now: NOW })).toBe(true);
    });
