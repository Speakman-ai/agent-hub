...
import {
  pickLinkedPrUrl,
  linkedPrBadge,
  dedupeSkillInvocations,
  formatInjectedBytes,
  normalizeSessionSummary,
  shouldShowViewChanges,
  splitSessionRoster,
} from './sessionExtras';
...
describe('pickLinkedPrUrl', () => {
  ...
  it('falls back to finalizePrUrl then sessionTitlePrUrl', () => {
    ...
    expect(
      pickLinkedPrUrl({ sessionTitlePrUrl: 'https://github.com/o/r/pull/3' }),
...
describe('linkedPrBadge', () => {
  ...
  ])('maps review_status %s → %s (%s)', (status, key, tone) => {
...
describe('dedupeSkillInvocations', () => {
  ...
  it('keeps the most recent row per skill id (camelCase rows)', () => {
    ...
    expect(out.find((r) => r.skillId === 'kanban').id).toBe(2);
  });
...
describe('splitSessionRoster', () => {
  it('splits executor and advisors, tolerating junk', () => {
    ...
    expect(out.advisors.map((a) => a.id)).toEqual(['a2', 'a3']);
  });
