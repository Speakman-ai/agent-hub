...
import { eventsToBlocks, describeTool } from './sessionTailBlocks';
...
describe('eventsToBlocks — progress_step handling', () => {
    ...
    it('still renders assistant_text and tool_use around progress_step events', () => {
        ...
        expect(blocks.map((b: any) => b.kind)).toEqual(['text', 'tool', 'text']);
    });
});
...
describe('eventsToBlocks — explored coalescer', () => {
    const read = (id: any, file: any) => ({ type: 'tool_use', id, tool: 'Read', input: { file_path: file } });
    const grep = (id: any, pattern: any) => ({ type: 'tool_use', id, tool: 'Grep', input: { pattern } });
    const ok = (id: any) => ({ type: 'tool_result', toolUseId: id, output: 'ok', isError: false });
...
describe('eventsToBlocks — misc', () => {
    ...
    it('drops checkpoint and rate_limit (web parity)', () => {
        ...
        expect(blocks.map((b: any) => b.kind)).toEqual(['system', 'thinking']);
    });
    it('maps ask_user_question with event payload', () => {
        const blocks = eventsToBlocks(wrap([{ type: 'ask_user_question', askId: 'q1', questions: [{ question: 'why?' }] }]));
...
describe('eventsToBlocks — thinking coalescing', () => {
    it('merges consecutive thinking chunks into one block', () => {
        ...
        expect(blocks.filter((b: any) => b.kind === 'thinking')).toHaveLength(1);
...
describe('describeTool', () => {
    it('matches web headlines for Read/Grep', () => {
        ...
        expect(describeTool('Grep', { pattern: 'TODO', path: '/src/utils.ts' }).headline).toBe('Search utils.ts for /TODO/');
    });
});
