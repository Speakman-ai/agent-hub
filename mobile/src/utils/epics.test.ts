...
import {
  ...
  DEFAULT_EPIC_COLOR,
  DEFAULT_EPIC_FORM,
  DEFAULT_EPIC_LIST_STATE_FILTER,
  epicFormFromRow,
...
describe('epicBranchTogglePatch', () => {
  it('sets, preserves, and clears the feature branch field', () => {
    ...
    ).toEqual({ pr_base_branch: 'feature/platform-reliability' });
    expect(
      epicBranchTogglePatch(
        { name: 'Platform reliability', pr_base_branch: 'feature/manual' },
        true,
      ),
    ).toEqual({ pr_base_branch: 'feature/manual' });
    expect(
      epicBranchTogglePatch(
        { name: 'Platform reliability', pr_base_branch: 'feature/manual' },
        false,
      ),
    ).toEqual({ pr_base_branch: '' });
  });
});
