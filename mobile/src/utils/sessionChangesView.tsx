 ...
 * the header shows.
 */
export function normalizeChangesSummary(body: any) {
    ...
    const totals = files.reduce((acc: any, f: any) => {
 ...
 */
export function classifyDiffLine(line: any) {
 ...
 */
export function describeDiff(diff: any, { maxLines = MAX_RENDER_LINES }: any = {}) {
    if (!diff)
        return { kind: 'loading' };
    if (diff.error)
        return { kind: 'error', message: String(diff.error) };
    if (diff.binary)
        return { kind: 'binary' };
    if (diff.tooLarge)
 ...
 */
export function worktreeStatusLine(worktree: any) {
 ...
 * @returns {object|null}
 */
export function resolveLiveSession({ sessionId, sessions, cronSessions, fetched, routeSession, }: any = {}) {
    if (sessionId) {
        const fromContext = (Array.isArray(sessions) && sessions.find((s: any) => s?.id === sessionId)) ||
            (Array.isArray(cronSessions) && cronSessions.find((s: any) => s?.id === sessionId));
        if (fromContext)
            return fromContext;
 ...
 * @returns {boolean}
 */
export function shouldFetchSessionRow({ sessionId, contextSession }: any = {}) {
