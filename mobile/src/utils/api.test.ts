...
describe('api threads helpers — URL + method parity with web client', () => {
  ...
  it('runSupportTicketInvestigation sends the selected engine and model', async () => {
    ...
    const [url, init] = lastCall();
    expect(url).toBe(
      'https://example.test/api/projects/agent-hub/support-tickets/tkt-model/investigate',
    );
  ...
  it('runSupportTicketInvestigation sends an empty object without a selection', async () => {
    ...
    const [url, init] = lastCall();
    expect(url).toBe(
      'https://example.test/api/projects/agent-hub/support-tickets/tkt-default/investigate',
    );
    expect(init?.method).toBe('POST');
...
describe('api Google connection helpers — mobile parity with web client', () => {
  ...
  it('startGoogleOAuth includes returnTo and space-delimited incremental scopes', async () => {
    ...
    const [url, init] = lastCall();
    expect(url).toBe(
      'https://example.test/api/auth/google/start?returnTo=%2Fsettings%3Ftab%3Daccount&scopes=https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fcalendar.events+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fgmail.modify',
    );
    expect(init.method ?? 'GET').toBe('GET');
  });
  ...
  it('listGoogleCalendarEvents → GET /google/calendar/events with range params', async () => {
    ...
    const [url, init] = lastCall();
    expect(url).toBe(
      'https://example.test/api/google/calendar/events?timeMin=2026-07-01T00%3A00%3A00Z&timeMax=2026-07-08T00%3A00%3A00Z&timeZone=America%2FLos_Angeles&maxResults=100',
    );
    expect(init.method ?? 'GET').toBe('GET');
  });
...
describe('api deployment helpers — URL + body parity with web client', () => {
  ...
  it('listDeployments includes optional query params', async () => {
    ...
    const [url, init] = lastCall();
    expect(url).toBe(
      'https://example.test/api/projects/agent-hub/deployments?environment=prod&limit=20&offset=40',
    );
    expect(init.method ?? 'GET').toBe('GET');
  });
  ...
  it('retryReleaseNotification POSTs to the scoped retry endpoint', async () => {
    ...
    const [url, init] = lastCall();
    expect(url).toBe(
      'https://example.test/api/projects/agent-hub/deployments/dep-1/release-notifications/note%2F1/retry',
    );
    expect(init.method).toBe('POST');
    expect(JSON.parse(init.body)).toEqual({});
  });
  it('adjustDeploymentReleaseItem PUTs inclusion status and reason', async () => {
    ...
    const [url, init] = lastCall();
    expect(url).toBe(
      'https://example.test/api/projects/agent-hub/deployments/dep-1/release-items/card-1',
    );
    expect(init.method).toBe('PUT');
    expect(JSON.parse(init.body)).toEqual({
      inclusionStatus: 'excluded',
...
describe('api kanban pagination helpers — URL parity with web client', () => {
  ...
  it('getColumnCards(projectId, columnId, { limit }) → first page URL', async () => {
    ...
    const [url] = lastCall();
    expect(url).toBe(
      'https://example.test/api/projects/agent-hub/board/columns/col-1/cards?limit=50',
    );
  });
