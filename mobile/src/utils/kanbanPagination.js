...
// no-op setState.
export function appendCardPage(existing, page) {
  const list = Array.isArray(existing) ? existing : [];
  const incoming = Array.isArray(page) ? page : [];
  if (incoming.length === 0) return list;
  const seen = new Set(list.map((c) => c.id));
  const fresh = incoming.filter((c) => c && !seen.has(c.id));
...
export function pagingEntry(nextCursor, total, loaded) {
...
export function seedPagingFromBoard({ columns, cards, cursors, counts } = {}) {
  ...
  for (const col of cols) {
    const loaded = all.filter((c) => c.column_id === col.id).length;
...
export function loadedCountsByColumn(cards) {
  const counts = {};
...
export function canLoadMore(entry) {
