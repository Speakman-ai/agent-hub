...
describe('resolveCronSkillPrincipalEngine — server-parity resolution', () => {
  ...
  it('returns null when the project is unresolvable', () => {
    expect(
      resolveCronSkillPrincipalEngine({ project_id: 'nope' }, [PROJECT_CODEX]),
    ).toBeNull();
...
describe('effectiveCronEngine', () => {
  it('honors the explicit per-row engine first', () => {
    expect(
      effectiveCronEngine(
        { engine: 'gemini-cli', project_id: 'codex-project' },
        [PROJECT_CODEX],
      ),
    ).toBe('gemini-cli');
  });
  ...
  it('inherits from the skill principal when engine is blank', () => {
    expect(
      effectiveCronEngine({ engine: '', project_id: 'codex-project' }, [PROJECT_CODEX]),
    ).toBe('codex-cli');
  });
...
describe('inheritedCronEngineForHelper', () => {
  it('returns the inherited engine when it differs from the default', () => {
    expect(
      inheritedCronEngineForHelper({ project_id: 'codex-project' }, [PROJECT_CODEX]),
    ).toBe('codex-cli');
  });
  ...
  it('returns null when the picker already has an explicit engine', () => {
    expect(
      inheritedCronEngineForHelper(
        { engine: 'codex-cli', project_id: 'codex-project' },
        [PROJECT_CODEX],
      ),
    ).toBeNull();
  });
