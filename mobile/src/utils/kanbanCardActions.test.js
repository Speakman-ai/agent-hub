...
describe('buildCardActions', () => {
  ...
  it('checks the current column in the status submenu', () => {
    ...
    const checked = status.options.filter((o) => o.checked);
  ...
  it('checks the current priority (defaulting to medium)', () => {
    ...
    expect(pri.options.find((o) => o.checked).action).toEqual({
      type: 'setPriority',
      priority: 'medium',
    });
    const high = group(buildCardActions({ id: 'c1', priority: 'high' }, baseCtx), 'priority');
    expect(high.options.find((o) => o.checked).action).toEqual({
      type: 'setPriority',
      priority: 'high',
    });
  });
  ...
  it('lists agents and checks the current assignee, with no Unassign when unassigned', () => {
    ...
    expect(assignee.options.map((o) => o.label)).toEqual(['Dev', 'Docs']);
    expect(assignee.options.some((o) => o.action?.type === 'unassign')).toBe(false);
  });
  ...
  it('marks active labels as checked and keepOpen, toggling via action', () => {
    ...
    const bug = lbl.options.find((o) => o.label === 'bug');
    const ui = lbl.options.find((o) => o.label === 'ui');
  ...
  it('offers "No epic" plus each epic, checking the linked one', () => {
    ...
    const checked = epic.options.find((o) => o.checked);
  ...
  it('exposes copy id / copy link leaves', () => {
    ...
    expect(copy.options.map((o) => o.action.type)).toEqual(['copyId', 'copyLink']);
  });

  it('tolerates missing context', () => {
    const actions = buildCardActions({ id: 'c1' });
    expect(actions.map((a) => a.key)).toContain('status');
