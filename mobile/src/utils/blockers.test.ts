...
describe('shouldConfirmMove', () => {
    ...
    it('skips when no unresolved blockers', () => {
        expect(shouldConfirmMove(card([{ done: true }]), 'src', { id: 'tgt', name: 'In Progress' })).toBe(false);
    });
    it('skips when target is Done', () => {
        expect(shouldConfirmMove(card([{ done: false }]), 'src', { id: 'd', name: 'Done' })).toBe(false);
    });
    it('skips when target is "Backlog" (back-compat for custom boards)', () => {
        expect(shouldConfirmMove(card([{ done: false }]), 'src', { id: 'b', name: 'Backlog' })).toBe(false);
    });
    it('prompts when blocked card moves into sensitive column', () => {
        expect(shouldConfirmMove(card([{ done: false }]), 'src', { id: 'p', name: 'In Progress' })).toBe(true);
    });
});
