...
import { prNumberFromUrl, parseNativePrUrl, openPrDashboardStatusBadge, relativePrTime, diffSummary, prStateBadge, summarizeChecks, checksBadge, summarizeReviews, reviewsBadge, mergeableBadge, reviewDecisionListBadge, mergePipelineListBadge, prListRowSuggestsResolvableWork, prListRowResolveDisabledHeuristic, buildPrActivityTimeline, } from './prFormatting';
...
describe('openPrDashboardStatusBadge', () => {
    ...
    it('shows requested changes from review status', () => {
        expect(openPrDashboardStatusBadge({ reviewStatus: 'changes_requested' })?.label).toBe('Requested changes');
    });
    it('shows awaiting review as fallback', () => {
        expect(openPrDashboardStatusBadge({ reviewStatus: 'awaiting_review' })?.label).toBe('Awaiting review');
    });
});
...
describe('diffSummary', () => {
    it('formats additions, deletions, files', () => {
        expect(diffSummary({ additions: 10, deletions: 2, changed_files: 4 })).toBe('+10 · -2 · 4 files');
    });
...
describe('prStateBadge', () => {
    it('returns merged for merged_at present', () => {
        expect(prStateBadge({ state: 'closed', merged_at: '2026-04-17T00:00:00Z' }).label).toBe('merged');
    });
...
describe('mergePipelineListBadge', () => {
    it('returns Blocked', () => {
        expect(mergePipelineListBadge({ mergeable: true, merge_state_status: 'BLOCKED' })?.label).toBe('Blocked');
    });
});
...
describe('prListRow resolve heuristics', () => {
    it('suggests work when checks fail', () => {
        expect(prListRowSuggestsResolvableWork({
            state: 'open',
            mergeable: true,
    ...
    it('disables list resolve when mergeable clean and no signals', () => {
        expect(prListRowResolveDisabledHeuristic({
            state: 'open',
            mergeable: true,
            mergeable_state: 'clean',
            check_rollup: [],
