...
describe('uploadFile — binary upload parity with web client', () => {
    it('POSTs to /api/upload/file with X-Filename + Content-Type + auth headers', async () => {
        ...
        const result = await uploadFile({ uri: 'file:///local/video.mp4', name: 'clip.mp4', type: 'video/mp4' }, { fileSystem: fs });
    ...
    it('throws when the server returns a non-2xx status', async () => {
        ...
        await expect(uploadFile({ uri: 'file:///big.mp4', name: 'big.mp4' }, { fileSystem: fs })).rejects.toThrow(/API error: 413/);
    });
    it('throws when the success body is not valid JSON', async () => {
        const fs = makeFakeFileSystem({ status: 200, body: 'not-json' });
        await expect(uploadFile({ uri: 'file:///x.mp4', name: 'x.mp4' }, { fileSystem: fs })).rejects.toThrow(/Invalid upload response/);
    });
    it('requires fileRef.uri', async () => {
        const fs = makeFakeFileSystem();
        await expect(uploadFile({ name: 'x.mp4' }, { fileSystem: fs })).rejects.toThrow(/fileRef\.uri is required/);
