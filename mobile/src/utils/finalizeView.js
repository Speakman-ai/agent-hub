 ...
 */
export function summarizeChecks(steps) {
 ...
 */
export function groupThreadsByFile(threads) {
 ...
 */
export function deriveFinalizeButton({ status, fullyValidated = false, hasChanges = true } = {}) {
 ...
 * @returns {boolean}
 */
export function canPush({ status, hasChanges = false } = {}) {
 ...
 * @returns {boolean}
 */
export function isFullyValidated(phases) {
  const checks = phases?.checks;
  const review = phases?.review;
  if (!checks || !review) return false;
  const passed = (p) => p?.status === 'ready_to_push' || p?.status === 'pushed';
  return (
    passed(checks) &&
    passed(review) &&
 ...
 */
export function isFreshGeneration(capturedGen, currentGen) {
 ...
 */
export function normalizeFinalizeRunResult(data) {
  return {
    run: data?.run ?? null,
 ...
 */
export function normalizeReviewerThreads(resp) {
  return {
