...
export function eventsToBlocks(events: any) {
    ...
    for (let i = 0; i < list.length; i++) {
        ...
        if (t === 'assistant_text') {
            ...
            flushThinking();
            if (!textBuf)
                textBuf = { partials: '', final: '' };
            if (event.partial)
                textBuf.partials += event.text || '';
            else
                textBuf.final += event.text || '';
            continue;
        }
        if (t === 'tool_result')
            continue;
        ...
        if (t === 'progress_step')
            continue;
        if (t === 'browser_tool_activity')
            continue;
        if (t === 'checkpoint')
            continue;
        if (t === 'rate_limit')
            continue;
        if (shouldSuppressStreamEvent(event))
            continue;
        if (t === 'tool_use') {
            const toolId = event.id != null ? String(event.id) : '';
            if (toolId && lastToolUseIndex.get(toolId) !== i)
                continue;
        ...
        default: {
            for (const v of Object.values(obj)) {
                if (typeof v === 'string' && v)
                    return { headline: tool, arg: truncateOneLine(v, 80) };
            }
