 ...
 */
export function pickInitialProjectId(projects, currentId) {
  if (!Array.isArray(projects) || projects.length === 0) return '';
  if (currentId && projects.find((p) => p && p.id === currentId)) return currentId;
 ...
 */
export function describeResolvedTarget(target) {
 ...
 */
export function pickFinalizeStatus({ lastSessionId, target }) {
 ...
 */
export function shouldRefreshOnWizardComplete(event, currentProjectId) {
