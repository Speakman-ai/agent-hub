...
describe('mobile engine picker constants', () => {
    ...
    it('defaults codex-cli to gpt-5.6', () => {
        expect(ENGINE_DEFAULT_MODELS['codex-cli']).toBe('gpt-5.6');
        const allowed = ENGINE_MODELS['codex-cli'].map((m: any) => m.id);
        expect(allowed).toContain('gpt-5.6');
    ...
    it('exposes only Codex models accepted under ChatGPT OAuth', () => {
        ...
        expect(models).toEqual(['gpt-5.6', 'gpt-5.5', 'gpt-5.4', 'gpt-5.4-mini', 'gpt-5.2']);
