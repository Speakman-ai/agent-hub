...
describe('buildAssigneeOptions', () => {
  ...
  it('handles non-array input gracefully', () => {
    expect(buildAssigneeOptions(null)).toEqual([{ id: '', name: 'Unassigned' }]);
    expect(buildAssigneeOptions(undefined)).toEqual([{ id: '', name: 'Unassigned' }]);
  });
});
describe('validModelsForAgent', () => {
  ...
  it('returns models for the agents own engine when no override is set', () => {
    ...
    expect(validModelsForAgent(agents, modelConfig, 'CodexAgent')).toEqual(['gpt-5-codex']);
  });
  it('returns models for the override engine instead of the agents own', () => {
    ...
    expect(validModelsForAgent(agents, modelConfig, 'ClaudeAgent', 'codex-cli')).toEqual([
      'gpt-5-codex',
    ]);
  });
