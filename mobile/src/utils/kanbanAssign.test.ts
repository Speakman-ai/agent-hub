...
import { findAgentByName, hasActiveSession, buildAssigneeOptions, filterAgentsByProject, validModelsForAgent, engineEntriesWithModels, effectiveAssignEngine, assignedSessionId, } from './kanbanAssign';
...
describe('filterAgentsByProject', () => {
    ...
    it('returns only agents belonging to the given project', () => {
        expect(filterAgentsByProject(agents, 'proj-1').map((a: any) => a.id)).toEqual(['a1', 'a3']);
    });
    it('does not leak agents from other projects (regression for the assign picker)', () => {
        ...
        expect(result.some((a: any) => a.projectId !== 'proj-2')).toBe(false);
    });
    ...
    it('composes with buildAssigneeOptions to scope the picker', () => {
        ...
        expect(opts.map((o: any) => o.name)).toEqual(['Unassigned', 'Alpha', 'Gamma']);
    });
});
...
describe('validModelsForAgent', () => {
    ...
    it('returns models for the override engine instead of the agents own', () => {
        ...
        expect(validModelsForAgent(agents, modelConfig, 'ClaudeAgent', 'codex-cli')).toEqual(['gpt-5-codex']);
    });
    it('trims and ignores blank overrides (falls back to the agent default)', () => {
        expect(validModelsForAgent(agents, modelConfig, 'ClaudeAgent', '   ')).toEqual(['claude-opus-4-8', 'claude-sonnet-4-20250514']);
        expect(validModelsForAgent(agents, modelConfig, 'ClaudeAgent', '')).toEqual(['claude-opus-4-8', 'claude-sonnet-4-20250514']);
    });
    it('returns [] when modelConfig is missing or the engine has no models', () => {
        ...
        expect(validModelsForAgent(agents, modelConfig, 'ClaudeAgent', 'cursor-agent')).toEqual([]);
    });
...
describe('engineEntriesWithModels', () => {
    it('returns engine keys whose model arrays are non-empty', () => {
        expect(engineEntriesWithModels({
            engineValidModels: {
...
describe('effectiveAssignEngine', () => {
    ...
    it('returns the override when set (non-blank)', () => {
        expect(effectiveAssignEngine(agents, 'ClaudeAgent', 'codex-cli')).toBe('codex-cli');
    });
