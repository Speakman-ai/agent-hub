 ...
 */
export function normalizePrFiles(payload) {
...
export function summarizePrFiles(files) {
 ...
 */
export function annotatePatchLines(patch) {
 ...
 */
export function commentAnchorFor(line) {
...
export function commentsForFile(comments, filename) {
  if (!Array.isArray(comments) || !filename) return [];
  return comments.filter((c) => c && c.file_path === filename);
}
