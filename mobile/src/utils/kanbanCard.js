...
export function assigneeColors(name) {
  const raw = (name ?? '').trim();
 ...
 */
export function cardShareUrl(baseUrl, projectId, cardId) {
  const root = (baseUrl ?? '').trim().replace(/\/+$/, '');
  if (!root || !projectId || cardId == null) return null;
  return `${root}/projects/${projectId}/board?card=${cardId}`;
}

/** Card labels (string "a,b" or array) -> trimmed non-empty string array. */
export function cardLabelList(labels) {
  if (!labels) return [];
 ...
 */
export function toggleLabelCsv(labelsInput, label) {
...
function unresolvedBlockerCount(card) {
  if (!Array.isArray(card?.blockers)) return 0;
  return card.blockers.filter((b) => !b.done).length;
}
 ...
 */
export function cardMetaModel(card, { board, epics = [] } = {}) {
