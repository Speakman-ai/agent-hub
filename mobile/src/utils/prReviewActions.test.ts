...
import {
  ...
  buildDismissReviewPayload,
  canDismissReview,
  prDetailCapabilities,
} from './prReviewActions';
...
describe('canDismissReview', () => {
  ...
  it('refuses comment reviews, already-dismissed reviews, and non-native PRs', () => {
    ...
    expect(
      canDismissReview({ source: 'user-oauth', pr: { state: 'open' } }, { state: 'APPROVED' }),
    ).toBe(false);
...
describe('buildInlineCommentPayload', () => {
  ...
  it('coerces numeric-string lines', () => {
    expect(
      buildInlineCommentPayload({ filePath: 'a.js', line: '12', body: 'x' }).payload.line,
    ).toBe(12);
  });
