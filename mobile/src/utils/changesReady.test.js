...
describe('resolveAutoMergeDefault', () => {
  ...
  it('returns false when autoMerge is explicitly false', () => {
    expect(
      resolveAutoMergeDefault({ githubWorkflow: { autoMerge: false } }),
    ).toBe(false);
  });

  it('returns true when autoMerge is true', () => {
    expect(
      resolveAutoMergeDefault({ githubWorkflow: { autoMerge: true } }),
    ).toBe(true);
  });

  it('coerces truthy-but-not-boolean autoMerge to a boolean', () => {
    expect(
      resolveAutoMergeDefault({ githubWorkflow: { autoMerge: 'yes' } }),
    ).toBe(true);
  });
});
