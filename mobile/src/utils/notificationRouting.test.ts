...
import { resolveEventKey, resolveAgentIdForSession, routeNotificationTap, notificationRouteToNavigation, } from './notificationRouting';
describe('resolveEventKey', () => {
    it('prefers `event` over `type` when both are present', () => {
        expect(resolveEventKey({ event: 'awaiting_feedback', type: 'awaiting_input' })).toBe('awaiting_feedback');
    });
...
describe('routeNotificationTap — chat events', () => {
    it.each(['awaiting_feedback', 'ready_to_push', 'pushed'])('routes %s (foreground payload shape) to chat', (event: any) => {
...
describe('notificationRouteToNavigation', () => {
    ...
    it('maps kanban / threads / support kinds to their screens', () => {
        expect(notificationRouteToNavigation({ kind: 'kanban', projectId: 'p1', cardId: 'c1' })).toEqual({ screen: 'Kanban', params: { projectId: 'p1', cardId: 'c1' } });
        expect(notificationRouteToNavigation({ kind: 'threads', projectId: 'p1', threadId: 't1' })).toEqual({ screen: 'Threads', params: { projectId: 'p1', threadId: 't1' } });
        expect(notificationRouteToNavigation({ kind: 'support', projectId: 'p1', ticketId: 'tk1' })).toEqual({ screen: 'CustomerSupport', params: { projectId: 'p1', ticketId: 'tk1' } });
    });
