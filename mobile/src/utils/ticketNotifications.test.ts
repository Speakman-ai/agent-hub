...
describe('ticketNotification formatters', () => {
    ...
    it('formats ready to push and pushed', () => {
        ...
        expect(pushedNotification({ sessionName: 'Ship', prNumber: 7 }).body).toBe('"Ship" was pushed (PR #7)');
    });
    it('formats support ticket created', () => {
        expect(supportTicketCreatedNotification({ subject: 'Help', ticketType: 'bug' }).body).toBe('bug: Help');
    });
    it('formats thread messages and truncates long preview', () => {
        expect(threadMessageNotification({ threadName: 'T', threadType: 'heartbeat' }).title).toBe('Heartbeat message');
        const long = 'y'.repeat(200);
        expect(threadMessageNotification({ threadName: 'T', threadType: 'cron', preview: long }).body.endsWith('…')).toBe(true);
    });
...
describe('mapBroadcastToNotification', () => {
    ...
    it('maps finalize_run_completed statuses', () => {
        ...
        expect(mapBroadcastToNotification({ type: 'finalize_run_completed', status: 'failed' })).toBeNull();
    });
    it('maps card_moved to review_assigned_to_you only for Review', () => {
        expect(mapBroadcastToNotification({
            ...
            columnName: 'Review',
            cardTitle: 'T',
        }).event).toBe('review_assigned_to_you');
        expect(mapBroadcastToNotification({ type: 'card_moved', columnName: 'Done', cardTitle: 'T' })).toBeNull();
    });
    ...
    it('maps support_ticket_created and webhook_pr_merged', () => {
        ...
        }).event).toBe('support_ticket_created');
        expect(mapBroadcastToNotification({
            type: 'webhook_pr_merged',
            cardTitle: 'X',
            prNumber: 5,
        }).event).toBe('pr_merged');
    });
});
describe('mapBroadcastToNotification — account scoping (ownerUserId)', () => {
    ...
    it("suppresses another user's session events", () => {
        expect(mapBroadcastToNotification(awaitingEvent('kevin'), { currentUserId: 'ryan' })).toBeNull();
        expect(mapBroadcastToNotification({
            type: 'finalize_run_completed',
            status: 'ready_to_push',
            sessionId: 's1',
            ownerUserId: 'kevin',
        }, { currentUserId: 'ryan' })).toBeNull();
    });
    ...
    it('shows unowned events (cron/system sessions) to everyone', () => {
        expect(mapBroadcastToNotification(awaitingEvent(null), { currentUserId: 'ryan' })?.event).toBe('awaiting_feedback');
        expect(mapBroadcastToNotification(awaitingEvent(undefined), { currentUserId: 'ryan' })?.event).toBe('awaiting_feedback');
    });
...
describe('mapBroadcastToNotification — project scoping (ownerUserId on project)', () => {
    ...
    it("suppresses another user's project events", () => {
        expect(mapBroadcastToNotification({
            ...
            threadType: 'cron',
            entry: { content: 'ok' },
        }, { currentUserId: 'ryan', projects })).toBeNull();
        expect(mapBroadcastToNotification({ type: 'card_moved', columnName: 'Review', cardTitle: 'T', projectId: 'theirs' }, { currentUserId: 'ryan', projects })).toBeNull();
    });
    ...
    it("suppresses another user's project event for an unattributed client without a real local bypass", () => {
        ...
        const r = mapBroadcastToNotification({ type: 'card_moved', columnName: 'Review', cardTitle: 'T', projectId: 'theirs' }, { projects });
        expect(r).toBeNull();
    });
    it('delivers an owned project event only with an explicit local/bundled bypass', () => {
        ...
        const r = mapBroadcastToNotification({ type: 'card_moved', columnName: 'Review', cardTitle: 'T', projectId: 'theirs' }, { projects, localBypass: true });
