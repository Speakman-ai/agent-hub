...
import { cardShortLabel, assigneeInitials, assigneeColors, cardLabelList, priorityMeta, reviewMeta, cardMetaModel, cardShareUrl, toggleLabelCsv, } from './kanbanCard';
...
describe('cardShareUrl', () => {
    it('builds the same deep-link format as the web card menu', () => {
        expect(cardShareUrl('https://hub.example.com', 'agent-hub', 'c1')).toBe('https://hub.example.com/projects/agent-hub/board?card=c1');
    });
    it('strips a trailing slash from the base url', () => {
        expect(cardShareUrl('https://hub.example.com/', 'p', 'c')).toBe('https://hub.example.com/projects/p/board?card=c');
    });
...
describe('cardMetaModel', () => {
    ...
    it('flags orphaned when the card carries orphaned_at', () => {
        const m = cardMetaModel({ id: 'c6', title: 't', orphaned_at: '2026-06-19 20:00:00' }, { board, epics });
