 ...
 */
export function applyTranscriptAtAnchor(prev, transcript, anchor) {
  const pos =
    anchor === null || anchor === undefined
      ? prev.length
      : Math.min(Math.max(0, anchor), prev.length);
  if (!transcript) return { text: prev, caret: pos };
  const trimmed = transcript.trim();
  if (!trimmed) return { text: prev, caret: pos };
...
export function insertTranscriptAtAnchor(prev, transcript, anchor) {
