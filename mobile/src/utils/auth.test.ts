...
import { loadAuthToken, getToken, getAuthRecord, setToken, clearToken, isAuthenticated, login, setup, getAuthStatus, logout, } from './auth';
...
describe('mobile auth network helpers', () => {
    ...
    it('login surfaces the server error message on 401', async () => {
        ...
        await expect(login({ baseUrl: '/api', username: 'owner', password: 'wrong' })).rejects.toThrow(/Invalid username/);
        expect(getToken()).toBeNull();
    });
