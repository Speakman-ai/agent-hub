...
import {
  ...
  modelsForCronEngine,
  resolveCronSkillPrincipalEngine,
} from './cronEngine';
/**
...
describe('modelsForCronEngine / defaultModelForCronEngine', () => {
  it('falls back to claude-code when the engine slot is blank', () => {
    expect(modelsForCronEngine(MODEL_CONFIG, '')).toEqual(['claude-opus-4-8', 'claude-sonnet-4-5']);
...
describe('resolveCronSkillPrincipalEngine — server-parity resolution', () => {
  it('uses the cron row’s skill_principal_agent_id when set', () => {
    const result = resolveCronSkillPrincipalEngine(
      { project_id: 'multi-project', skill_principal_agent_id: 'claude-agent-id' },
      [PROJECT_MULTI],
    );
  ...
  it('falls back to project.cronSkillPrincipalAgentId when the row is blank', () => {
    const result = resolveCronSkillPrincipalEngine(
      { project_id: 'multi-project', skill_principal_agent_id: '' },
      [PROJECT_MULTI],
    );
    expect(result).toBe('cursor-agent');
  });
  it('falls back to the sole project agent when nothing else is set', () => {
    const result = resolveCronSkillPrincipalEngine({ project_id: 'codex-project' }, [
      PROJECT_CODEX,
    ]);
...
describe('effectiveCronEngine', () => {
  it('honors the explicit per-row engine first', () => {
    expect(
      effectiveCronEngine({ engine: 'gemini-cli', project_id: 'codex-project' }, [PROJECT_CODEX]),
    ).toBe('gemini-cli');
  });
  it('inherits from the skill principal when engine is blank', () => {
    expect(effectiveCronEngine({ engine: '', project_id: 'codex-project' }, [PROJECT_CODEX])).toBe(
      'codex-cli',
    );
  });
...
describe('inheritedCronEngineForHelper', () => {
  it('returns the inherited engine when it differs from the default', () => {
    expect(inheritedCronEngineForHelper({ project_id: 'codex-project' }, [PROJECT_CODEX])).toBe(
      'codex-cli',
    );
  });
  ...
  it('returns null when the picker already has an explicit engine', () => {
    expect(
      inheritedCronEngineForHelper({ engine: 'codex-cli', project_id: 'codex-project' }, [
        PROJECT_CODEX,
      ]),
    ).toBeNull();
  });
