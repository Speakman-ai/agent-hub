...
describe('shouldShowDateSeparator', () => {
    ...
    it('returns false when same calendar day', () => {
        expect(shouldShowDateSeparator({ timestamp: '2026-04-18T12:00:00Z' }, { timestamp: '2026-04-18T18:00:00Z' })).toBe(false);
    });
    it('returns true when the day rolls over', () => {
        expect(shouldShowDateSeparator({ timestamp: '2026-04-17T23:59:00Z' }, { timestamp: '2026-04-19T05:00:00Z' })).toBe(true);
    });
    it('handles SQLite (no T) previous timestamps', () => {
        expect(shouldShowDateSeparator({ timestamp: '2026-04-17 23:59:00' }, { timestamp: '2026-04-18T05:00:00Z' })).toBe(true);
    });
});
...
describe('mergeLiveEntry', () => {
    it('appends a new entry', () => {
        ...
        expect(mergeLiveEntry(existing, { id: '3' })).toEqual([
            { id: '1' },
            { id: '2' },
            { id: '3' },
        ]);
