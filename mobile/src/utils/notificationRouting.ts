...
export function routeNotificationTap(data: any, ctx: any = {}) {
    ...
    switch (event) {
        ...
        case 'support_ticket_created': {
            ...
            if (!projectId)
                return null;
            const ticketId = typeof data.ticketId === 'string'
                ? data.ticketId
        ...
        case 'review_assigned_to_you': {
            ...
            if (cardId)
                return { kind: 'kanban', projectId, cardId };
            if (prNumber != null && projectId)
                return { kind: 'pulls', projectId, prNumber };
            if (projectId)
                return { kind: 'kanban', projectId, cardId: null };
            return null;
        }
        case 'pr_merged': {
            return {
