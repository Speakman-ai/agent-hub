/**
 * Presentation helpers for the mobile Logs module (LOG-QUERY live tail +
 * Issues): severity buckets and tone keys, facet filters, history paging, and
 * list scroll geometry.
 *
 * The transport half of the contract (record shape, `logs_subscribe` frame,
 * reconnect-safe merge, keyset cursor) lives in `@shared/utils/logTailWire`,
 * shared byte-for-byte with the web module, and is re-exported below so
...
export {
  ...
  type LogCursor,
  type LogRecord,
  type LogSubscribeFrameInput,
} from '@shared/utils/logTailWire';
import { compareLogRecords, type LogCursor, type LogRecord } from '@shared/utils/logTailWire';
