 ...
 */
export function useVoiceTranscription({ value, setValue, cursorRef, disabled, isProcessing, onError, }: any) {
    ...
    const applyTranscript = useCallback((text: any) => {
    ...
    const uploadRecording = useCallback(async (uri: any) => {
        if (!uri) {
    ...
    const startRecording = useCallback(async () => {
        ...
        try {
            ...
            if (permission.status !== 'granted') {
                reportError('Microphone permission denied. Enable mic access in device settings, then tap the mic again.');
                return;
            }
