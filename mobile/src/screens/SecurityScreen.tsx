  ...
  const handleFix = async () => {
    ...
    try {
      ...
      if (result?.sessionId) {
        ...
        if (result.reused) {
          Alert.alert(
            'Fix session already running',
            'A fix session is already running for this project. See the sessions list.',
          );
        } else {
          Alert.alert(
            'Fix session started',
          ...
          <View style={styles.footerActions}>
            {item.fixed_version ? (
              <TouchableOpacity
                onPress={handleFix}
                disabled={fixing}
                testID="finding-fix-button"
                style={[styles.fixButton, fixing && styles.dismissButtonDisabled]}
              >
...
export default function SecurityScreen({ route }: any) {
  ...
  const [scheduleConfig, setScheduleConfig] = useState<SecurityScheduleConfig>(() =>
    readSecurityScheduleConfig(project),
  );
  ...
  const persistSchedule = useCallback(
    async (next: SecurityScheduleConfig) => {
      if (!projectId || scheduleSaving) return;
  ...
  const [autofixConfig, setAutofixConfig] = useState<SecurityAutofixConfig>(() =>
    readSecurityAutofixConfig(project),
  );
  useEffect(() => {
    if (scheduleSaving) return;
  ...
  const persistAutofix = useCallback(
    async (next: SecurityAutofixConfig) => {
      if (!projectId || scheduleSaving) return;
      ...
      try {
        ...
        Alert.alert(
          'Could not update auto-fix',
          err?.message || 'Failed to update auto-fix setting',
        );
      } finally {
        setScheduleSaving(false);
      }
  ...
  const renderItem = ({ item }: any) => (
    <FindingCard item={item} projectId={projectId} onDismissed={handleDismissed} onFixed={load} />
  );
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
        ...
        <View style={styles.scheduleRow} testID="security-schedule">
          ...
          >
            <Text
              style={[
                styles.scheduleButtonText,
                scheduleConfig.onPush && styles.scheduleButtonTextActive,
              ]}
            >
          ...
          {SECURITY_AUTOFIX_OPTIONS.map((opt) => (
            ...
            >
              <Text
                style={[
                  styles.scheduleButtonText,
                  securityAutofixMode(autofixConfig) === opt.value &&
      ...
      {severityCounts.all > 0 ? (
        <View style={styles.severityRow} testID="severity-breakdown">
          {SEVERITY_ORDER.filter((s) => severityCounts[s.key] > 0).map((s) => (
            <View
              key={s.key}
              testID={`severity-count-${s.key}`}
              style={[
                styles.severityChip,
                { borderColor: SEVERITY_COLOR[s.key] || colors.gray500 },
              ]}
            >
              <Text
                style={[
                  styles.severityChipLabel,
                  { color: SEVERITY_COLOR[s.key] || colors.gray500 },
                ]}
              >
                {s.label}
              </Text>
