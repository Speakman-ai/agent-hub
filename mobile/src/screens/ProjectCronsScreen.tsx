...
export default function ProjectCronsScreen({ route, navigation }: any) {
  ...
  const renderForm = (f: any, setF: any, onSubmit: any, submitLabel: any) => (
    <View style={styles.formCard}>
      <Text style={styles.label}>Name</Text>
      <TextInput
        style={styles.input}
        value={f.name}
        onChangeText={(v: any) => setF({ ...f, name: v })}
      ...
      <TextInput
        style={styles.input}
        value={f.schedule}
        onChangeText={(v: any) => setF({ ...f, schedule: v })}
        placeholder="*/30 * * * *"
        placeholderTextColor={colors.gray600}
      />
      ...
      <TextInput
        ...
        value={f.prompt}
        onChangeText={(v: any) => setF({ ...f, prompt: v })}
        multiline
        placeholderTextColor={colors.gray600}
      />
      <View style={styles.switchRow}>
        ...
        <Switch
          value={!!f.enabled}
          onValueChange={(v: any) => setF({ ...f, enabled: v })}
      ...
      <View style={styles.switchRow}>
        ...
        <Switch
          value={!!f.shared}
          onValueChange={(v: any) => setF({ ...f, shared: v })}
          trackColor={{ false: colors.gray700, true: colors.blue500 }}
          thumbColor={f.shared ? colors.blue400 : colors.gray500}
        />
      </View>
                ...
                <>
                  ...
                  <View style={styles.actions}>
                    <TouchableOpacity
                      style={[
                        styles.actionBtn,
                        (!cronJob.can_manage || running[cronJob.id]) && styles.disabledBtn,
                      ]}
                      onPress={() => triggerRun(cronJob.id)}
                      disabled={!cronJob.can_manage || running[cronJob.id]}
                    >
                      <Text style={styles.actionText}>
                        {running[cronJob.id] ? 'Running…' : 'Run'}
                      </Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                      style={[styles.actionBtn, !cronJob.can_manage && styles.disabledBtn]}
                      disabled={!cronJob.can_manage}
                      onPress={() => {
