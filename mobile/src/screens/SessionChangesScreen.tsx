...
import { View, Text, TouchableOpacity, FlatList, ScrollView, StyleSheet, ActivityIndicator, RefreshControl, Platform, } from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { api } from '../utils/api';
import { colors } from '../theme/colors';
import { normalizeChangesSummary, describeDiff, statusMeta, basename, dirname, worktreeStatusLine, } from '../utils/sessionChangesView';
...
function DiffNotice({ children, tone }: any) {
    return (<Text style={[styles.diffNotice, tone === 'warn' && { color: colors.amber400 }]}>
      {children}
    </Text>);
}
...
function DiffBody({ body }: any) {
    if (body.kind === 'loading') {
        return (<View style={styles.diffLoading}>
    ...
    return (<ScrollView horizontal showsHorizontalScrollIndicator style={styles.diffScroll}>
      <View>
        {body.lines.map((line: any, i: any) => (<Text key={i} style={[
                styles.diffLine,
        ...
        {body.hiddenLines > 0 && (<Text style={styles.diffHiddenNote}>… {body.hiddenLines} more lines not shown</Text>)}
      </View>
    </ScrollView>);
}
export default function SessionChangesScreen({ navigation, route }: any) {
    ...
    const load = useCallback(async ({ asRefresh = false }: any = {}) => {
        if (!sessionId)
            return;
    ...
    const toggleFile = useCallback((path: any) => {
        const opening = !expanded[path];
        setExpanded((prev: any) => ({ ...prev, [path]: !prev[path] }));
        if (!opening || diffs[path] || diffLoading[path])
            return;
    ...
    const finalizeCards = projectId ? (<>
      ...
      <ReviewerThreadsCard projectId={projectId} runId={finalize.run?.id} status={finalize.status}/>
    </>) : null;
    const renderFile = ({ item }: any) => {
        ...
        <TouchableOpacity style={styles.fileRow} onPress={() => toggleFile(item.path)} accessibilityLabel={`${meta.label}: ${item.path}`} accessibilityState={{ expanded: isOpen }}>
          ...
          <View style={styles.fileNameBlock}>
            ...
            {dir ? (<Text style={styles.fileDir} numberOfLines={1}>
                {dir}
              </Text>) : null}
            {item.oldPath ? (<Text style={styles.fileDir} numberOfLines={1}>
                ← {item.oldPath}
              </Text>) : null}
          </View>
          {item.binary ? (<Text style={styles.fileBin}>bin</Text>) : (<Text style={styles.fileCounts}>
              <Text style={{ color: colors.emerald400 }}>+{item.additions}</Text>{' '}
              <Text style={{ color: colors.red400 }}>−{item.deletions}</Text>
            </Text>)}
          <Text style={styles.chevron}>{isOpen ? '▾' : '▸'}</Text>
        </TouchableOpacity>
        {isOpen && (<View style={styles.diffContainer}>
    ...
    const listHeader = (<View>
      {finalizeCards}
      {summary?.branch ? (<View style={styles.branchBlock}>
      ...
      {summary?.truncated ? (<Text style={styles.truncatedNote}>
    ...
    const emptyState = (<View style={styles.emptyBlock}>
    ...
    return (<SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      <View style={styles.topBar}>
        ...
        <View style={styles.titleBlock}>
          ...
          {sessionName ? (<Text style={styles.subtitle} numberOfLines={1}>
              {sessionName}
            </Text>) : null}
        </View>
        {files.length > 0 && (<Text style={styles.totals}>
      ...
      {loading ? (<View style={styles.centerState}>
        ...
        </View>) : (<FlatList data={files} keyExtractor={(item: any) => item.path} renderItem={renderFile} ListHeaderComponent={listHeader} ListEmptyComponent={emptyState} contentContainerStyle={hasChanges ? styles.listContent : styles.listContentEmpty} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => load({ asRefresh: true })} tintColor={colors.gray400}/>}/>)}
    </SafeAreaView>);
}
