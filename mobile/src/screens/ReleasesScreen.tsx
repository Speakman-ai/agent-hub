...
export default function ReleasesScreen() {
  ...
  const headerRight = (
    <TouchableOpacity style={styles.refreshBtn} onPress={() => load(true)} disabled={loading}>
      {loading ? (
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
      ...
      {data?.currentVersion ? (
        <Text style={styles.subtitle}>You're on v{data.currentVersion}</Text>
      ) : null}
      ...
      <View style={styles.body}>
        <ScrollView style={styles.versionList} contentContainerStyle={styles.versionListContent}>
          {loading && list.length === 0 ? (
            <Text style={styles.muted}>Loading releases…</Text>
          ) : list.length === 0 ? (
            <Text style={styles.muted}>No releases found.</Text>
          ) : (
            list.map((r: any) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  onPress={() => setPickedVersion(r.version)}
                >
                  ...
                  {r.summary ? (
                    <Text style={styles.versionSummary} numberOfLines={2}>
                      {r.summary}
                    </Text>
                  ) : null}
                </TouchableOpacity>
        ...
        <ScrollView style={styles.detail} contentContainerStyle={styles.detailContent}>
          {!selected && !loading ? (
            <Text style={styles.muted}>Select a version to see what changed.</Text>
          ) : selected ? (
            <>
              <View style={styles.detailHeader}>
                <Text style={styles.detailTitle}>v{selected.version}</Text>
                {data?.currentVersion === selected.version ? (
                  <View style={styles.yourVersionBadge}>
