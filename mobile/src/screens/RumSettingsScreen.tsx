...
export function buildRetentionReplayPatch(
  currentReplay: any,
  key: 'retentionDays' | 'extendedRetentionMonths',
  value: number,
): Record<string, unknown> {
  const replay: Record<string, unknown> = { ...(currentReplay || {}) };
  if (key === 'retentionDays' && value <= 0) delete replay.retentionDays;
  else replay[key] = value;
  return replay;
}
export default function RumSettingsScreen({ route, navigation }: any) {
  ...
  const baseRetentionDays =
    typeof replayCfg?.retentionDays === 'number' ? replayCfg.retentionDays : 0;
  const extendedRetentionMonths =
    typeof replayCfg?.extendedRetentionMonths === 'number' ? replayCfg.extendedRetentionMonths : 15;
  const saveRetention = useCallback(
    async (key: 'retentionDays' | 'extendedRetentionMonths', value: number) => {
      if (!projectId || savingRetention) return;
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        {draft && (
          <View style={styles.card}>
            ...
            {draft.cspHits?.length > 0 && (
              <Text style={styles.row}>CSP locations: {draft.cspHits.length}</Text>
            )}
          </View>
        )}
        ...
        {lastSessionId && (
          <TouchableOpacity style={styles.secondaryBtn} onPress={handleOpenWizardChat}>
        ...
        <View style={styles.createRow}>
          ...
          />
          <TouchableOpacity
            style={[styles.primaryBtn, { marginTop: 0 }, minting && styles.btnDisabled]}
        ...
        {!clientsLoading && clients.length === 0 && !clientsError && (
          <Text style={styles.hint}>No ingest clients yet.</Text>
        )}

        <Text style={styles.sectionTitle}>Retention (this project)</Text>
        <Text style={styles.hint}>
          Captures live for the base window, then expire. Flag a session in the replay player (the
          Keep button) to move it to the extended tier — kept for the window below (up to 15
          ...
          <View style={styles.chipRow}>
            {BASE_RETENTION_OPTIONS.map((opt) => {
              ...
              return (
                <TouchableOpacity
                  key={opt.value}
                  testID={`rum-base-retention-${opt.value}`}
                  disabled={savingRetention}
                  onPress={() => saveRetention('retentionDays', opt.value)}
                  style={[
                    styles.chip,
                    selected && styles.chipActive,
                    savingRetention && styles.btnDisabled,
                  ]}
                >
          ...
          <View style={styles.chipRow}>
            {EXTENDED_RETENTION_OPTIONS.map((opt) => {
              ...
              return (
                <TouchableOpacity
                  key={opt.value}
                  testID={`rum-extended-retention-${opt.value}`}
                  disabled={savingRetention}
                  onPress={() => saveRetention('extendedRetentionMonths', opt.value)}
                  style={[
                    styles.chip,
                    selected && styles.chipActive,
                    savingRetention && styles.btnDisabled,
