...
export default function AwsProfilesScreen({ route, navigation }) {
  ...
  const [statusByProfile, setStatusByProfile] = useState({});
  const [loginByProfile, setLoginByProfile] = useState({});
  ...
  const checkStatus = async (profileName) => {
    setStatusByProfile((s) => ({ ...s, [profileName]: { loading: true } }));
    try {
      const st = await api.getProjectAwsSsoStatus(projectId, profileName);
      setStatusByProfile((s) => ({ ...s, [profileName]: { loading: false, data: st } }));
    } catch (err) {
      setStatusByProfile((s) => ({
        ...s,
  ...
  const startLogin = async (profileName) => {
    setLoginByProfile((s) => ({ ...s, [profileName]: { loading: true } }));
    try {
      ...
      if (data.loginUrl) {
        ...
        if (can) await Linking.openURL(data.loginUrl);
        else Alert.alert('Open in browser', data.loginUrl);
      }
      if (data.completed) await checkStatus(profileName);
    } catch (err) {
      setLoginByProfile((s) => ({
        ...s,
    ...
    <SafeAreaView style={styles.screen} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {!loading && profiles.length === 0 && !error && (
          <Text style={styles.empty}>No AWS profiles configured.</Text>
        )}
        {profiles.map((row) => {
          ...
          return (
            <View key={row.name} style={styles.card}>
              ...
              {loggedIn != null && (
                <Text style={[styles.status, loggedIn ? styles.ok : styles.warn]}>
