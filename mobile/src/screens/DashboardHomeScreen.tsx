...
export function WorkCardRow({ card, onOpen }: { card: any; onOpen: () => void }) {
  const badge =
    PRIORITY_BADGE[(card.priority as CardPriority) || 'medium'] || PRIORITY_BADGE.medium;
  return (
    <TouchableOpacity style={styles.workRow} onPress={onOpen}>
      <View style={styles.workRowTop}>
        <View
          style={[styles.priorityBadge, { backgroundColor: badge.bg, borderColor: badge.border }]}
        >
        ...
        {card.prUrl ? <HubIcon name="GitPullRequest" size={12} color={colors.gray500} /> : null}
      </View>
...
export function TodoRow({ todo }: { todo: any }) {
  ...
  return (
    <View style={styles.todoRow}>
      ...
      {doDate ? (
        <View style={[styles.dueBadge, { backgroundColor: badge.bg, borderColor: badge.border }]}>
          <Text style={[styles.dueBadgeText, { color: badge.text }]}>{dueLabel(doDate)}</Text>
        </View>
      ) : null}
    </View>
  );
          ...
          <Pane title="My Work" icon="LayoutGrid" count={work?.counts?.open}>
            {openCards.length === 0 ? (
              ...
              openCards.map((card: any) => (
                <WorkCardRow key={card.id} card={card} onOpen={() => openKanban(card.projectId)} />
              ))
            )}
          </Pane>
          ...
          >
            {mailState !== 'ready' ? (
              ...
              google.mail.messages.map((msg: DashboardMailMessage, i: number) => (
                <MailRow key={msg.id ?? i} msg={msg} onOpen={() => navigation.navigate('Gmail')} />
              ))
            )}
          </Pane>
        </ScrollView>
