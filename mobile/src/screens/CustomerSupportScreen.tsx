...
function TicketCard({ item, projectId, onOpenReplay, onDeleted, onPress, onSetStatus, onWontDo, onReclassify }: any) {
    ...
    return (<TouchableOpacity activeOpacity={0.7} onPress={() => onPress?.(item)} testID="support-ticket-card" accessibilityState={{ selected: !isUnread }} style={[styles.card, isUnread && styles.cardUnread]}>
      ...
      {item.ai_summary ? (<View style={styles.aiBox}>
          <Text style={styles.aiLabel}>AI investigation</Text>
          <Text style={styles.aiText}>{item.ai_summary}</Text>
        </View>) : null}

      {screenshotUrl ? (<TouchableOpacity onPress={() => Linking.openURL(screenshotUrl).catch(() => { })} testID="ticket-screenshot">
          <Image source={{ uri: screenshotUrl }} style={styles.screenshot} resizeMode="contain"/>
        </TouchableOpacity>) : null}

      {hasReplay ? (<TouchableOpacity onPress={() => onOpenReplay(item.replay_ref)}>
      ...
      {item.status !== 'converted' ? (<View style={styles.statusActionRow}>
          ...
          {[
                ...
                return (<TouchableOpacity key={s.value} testID={`status-action-${s.value}`} onPress={() => s.value === 'wont_do' ? onWontDo?.(item) : onSetStatus?.(item, s.value)} style={[styles.statusActionButton, active && styles.statusActionButtonActive]}>
...
export default function CustomerSupportScreen({ route }: any) {
    const { projects, lastSupportTicketEvent, refreshSupportUnreadCount, setSupportUnreadCount, } = useApp();
    ...
    const load = useCallback(async () => {
        ...
        setError(null);
        try {
            const data = await api.getSupportTickets(projectId, activeStatusFilter.statuses.join(','), typeFilter === 'all' ? undefined : typeFilter);
    ...
    const markRead = useCallback((ticket: any) => {
        if (!ticket || ticket.read_at)
            return;
        const stamp = new Date().toISOString();
        setTickets((prev: any) => prev.map((t: any) => (t.id === ticket.id && !t.read_at ? { ...t, read_at: stamp } : t)));
    ...
    useEffect(() => {
        if (!deepLinkedTicketId)
            return;
        if (deepLinkAppliedRef.current === deepLinkedTicketId)
            return;
    ...
    if (selectedTicket) {
        ...
        <ScrollView contentContainerStyle={{ padding: 16 }}>
          ...
          {selectedTicket.ai_summary ? (<View style={styles.aiBox}>
              <Text style={styles.aiLabel}>AI investigation</Text>
    ...
    return (<SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      <View style={styles.topBar}>
        ...
        {hasUnread ? (<TouchableOpacity onPress={markAllRead} testID="mark-all-read" style={styles.markAllButton}>
      ...
      <View style={styles.typeFilterRow}>
        {TYPE_FILTERS.map((f: any) => (<TouchableOpacity key={f.key} testID={`type-filter-${f.key}`} onPress={() => setTypeFilter(f.key)} style={[styles.typeFilterButton, typeFilter === f.key && styles.filterButtonActive]}>
      ...
      <Modal visible={!!wontDoTicket} transparent animationType="fade" onRequestClose={() => setWontDoTicket(null)}>
        <View style={styles.modalBackdrop}>
          <View style={styles.modalCard} testID="wont-do-modal">
            ...
            <TextInput testID="wont-do-input" value={wontDoReason} onChangeText={setWontDoReason} placeholder="Reason this won't be done" placeholderTextColor={colors.gray600} style={styles.modalInput} multiline autoFocus/>
            <View style={styles.modalActions}>
