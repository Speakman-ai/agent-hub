...
function TemplateDialog({
  ...
  const update = (patch: Partial<KanbanCardTemplateInput>) => setForm((current) => ({ ...current, ...patch }));
      ...
      >
        <View style={styles.dialog}>
          ...
          <ScrollView style={styles.dialogScroll} keyboardShouldPersistTaps="handled">
            ...
            <View style={styles.choiceRow}>
              {PRIORITIES.map((priority) => (
                ...
                >
                  <Text style={[styles.choiceText, form.priority === priority && styles.choiceTextSelected]}>
                    {priority}
                  </Text>
                </TouchableOpacity>
              ))}
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.choiceRow}>
              ...
              >
                <Text style={[styles.choiceText, !form.epicId && styles.choiceTextSelected]}>None</Text>
              </TouchableOpacity>
              {epics.map((epic) => (
                <TouchableOpacity
                  key={epic.id}
                ...
                >
                  <Text style={[styles.choiceText, form.epicId === epic.id && styles.choiceTextSelected]} numberOfLines={1}>
                    {epic.name}
                  </Text>
                </TouchableOpacity>
              ))}
          ...
          <View style={styles.dialogActions}>
            ...
            >
              {saving ? <ActivityIndicator color={colors.white} size="small" /> : <Text style={styles.saveText}>{template ? 'Save changes' : 'Create template'}</Text>}
            </TouchableOpacity>
          </View>
        </View>
      </KeyboardAvoidingView>
...
export default function KanbanCardTemplatesScreen({ route, navigation }: Props) {
  ...
  const [dialog, setDialog] = useState<{ mode: 'create' | 'edit'; template?: KanbanCardTemplate } | null>(null);
  ...
  useEffect(() => {
    void fetchTemplates();
    api.getEpics(projectId).then((rows: any) => setEpics(Array.isArray(rows) ? rows : [])).catch(() => setEpics([]));
    ...
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      <View style={styles.topBar}>
        ...
        <View style={styles.titleBlock}>
          <Text style={styles.topBarTitle} numberOfLines={1}>Card templates</Text>
          <Text style={styles.subtitle} numberOfLines={1}>{project?.name || 'Project'}</Text>
        </View>
        <TouchableOpacity style={styles.newButton} onPress={() => { setDialogError(null); setDialog({ mode: 'create' }); }} testID="templates-new">
          <Text style={styles.newButtonText}>+ New</Text>
        </TouchableOpacity>
      </View>
      <ScrollView contentContainerStyle={styles.content}>
        {error ? <Text style={styles.errorBanner}>{error}</Text> : null}
        <Text style={styles.helpText}>Templates pre-fill new cards with a title, description, priority, labels, and epic.</Text>
        {templates.length === 0 ? (
          <View style={styles.empty} testID="templates-empty">
            <Text style={styles.emptyTitle}>No templates yet.</Text>
            <TouchableOpacity style={styles.saveButton} onPress={() => { setDialogError(null); setDialog({ mode: 'create' }); }}>
              <Text style={styles.saveText}>Create your first template</Text>
            </TouchableOpacity>
          </View>
        ) : templates.map((template) => (
          <View key={template.id} style={styles.templateRow} testID={`template-row-${template.id}`}>
            <View style={styles.templateInfo}>
              <Text style={styles.templateName} numberOfLines={1}>{template.name}</Text>
              {template.title ? <Text style={styles.templateTitle} numberOfLines={1}>Title: {template.title}</Text> : null}
              <View style={styles.metaRow}>
                <Text style={styles.meta}>{template.priority}</Text>
                {template.labels ? <Text style={styles.meta} numberOfLines={1}>{template.labels}</Text> : null}
                {template.epicId ? <Text style={styles.meta} numberOfLines={1}>Epic: {epics.find((epic) => epic.id === template.epicId)?.name || template.epicId}</Text> : null}
              </View>
            </View>
            <View style={styles.rowActions}>
              <TouchableOpacity style={styles.smallButton} onPress={() => navigation.navigate('Kanban', { projectId, project, pendingTemplate: template })} testID={`template-use-${template.id}`}>
                <Text style={styles.smallButtonText}>Use</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.iconButton} onPress={() => { setDialogError(null); setDialog({ mode: 'edit', template }); }} testID={`template-edit-${template.id}`}>
                <Text style={styles.iconText}>Edit</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.iconButton, deletingId === template.id && styles.deleteConfirm]} onPress={() => deleteTemplate(template)} testID={`template-delete-${template.id}`}>
                <Text style={styles.deleteText}>{deletingId === template.id ? 'Confirm' : 'Delete'}</Text>
              </TouchableOpacity>
            </View>
          </View>
        ))}
      ...
      <TemplateDialog
        ...
        saving={saving}
        error={dialogError}
        onClose={() => { if (!saving) { setDialog(null); setDialogError(null); } }}
        onSave={saveTemplate}
      />
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  topBar: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: colors.gray800, gap: 8 },
  ...
  newButton: { backgroundColor: colors.blue600, borderRadius: 7, paddingHorizontal: 11, paddingVertical: 8 },
  ...
  errorBanner: { color: colors.red400, backgroundColor: colors.red900_50, borderRadius: 8, padding: 10, marginBottom: 12 },
  empty: { borderWidth: 1, borderColor: colors.gray800, borderRadius: 12, padding: 24, alignItems: 'center' },
  emptyTitle: { color: colors.gray400, fontSize: 14, marginBottom: 14 },
  templateRow: { flexDirection: 'row', alignItems: 'flex-start', gap: 10, borderWidth: 1, borderColor: colors.gray800, backgroundColor: colors.gray900, borderRadius: 12, padding: 12, marginBottom: 8 },
  ...
  meta: { color: colors.gray400, backgroundColor: colors.gray800, borderRadius: 5, paddingHorizontal: 6, paddingVertical: 3, fontSize: 10, maxWidth: 150 },
  rowActions: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  smallButton: { backgroundColor: colors.gray800, borderRadius: 6, paddingHorizontal: 9, paddingVertical: 7 },
  ...
  dialog: { maxHeight: '90%', backgroundColor: colors.gray900, borderRadius: 14, borderWidth: 1, borderColor: colors.gray700, overflow: 'hidden' },
  dialogHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingVertical: 13, borderBottomWidth: 1, borderBottomColor: colors.gray800 },
  ...
  label: { color: colors.gray400, fontSize: 11, fontWeight: '600', marginTop: 13, marginBottom: 6, textTransform: 'uppercase' },
  input: { color: colors.white, backgroundColor: colors.gray950, borderWidth: 1, borderColor: colors.gray700, borderRadius: 7, paddingHorizontal: 10, paddingVertical: 9, fontSize: 13 },
  multiline: { minHeight: 90, textAlignVertical: 'top' },
  choiceRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 6, paddingBottom: 2 },
  choice: { borderWidth: 1, borderColor: colors.gray700, borderRadius: 6, backgroundColor: colors.gray950, paddingHorizontal: 9, paddingVertical: 7, maxWidth: 180 },
  ...
  dialogActions: { flexDirection: 'row', justifyContent: 'flex-end', gap: 8, padding: 14, borderTopWidth: 1, borderTopColor: colors.gray800 },
  cancelButton: { paddingHorizontal: 12, paddingVertical: 9 },
  cancelText: { color: colors.gray400, fontSize: 12 },
  saveButton: { backgroundColor: colors.blue600, borderRadius: 7, paddingHorizontal: 12, paddingVertical: 9 },
