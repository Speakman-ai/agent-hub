...
export default function ThreadsScreen({ route, navigation }: any) {
  ...
  useEffect(() => {
    ...
    } else if (type === 'thread_entry_created' && selectedThread?.id === threadId) {
      const { entry } = lastThreadEvent;
      if (entry) setEntries((prev: any) => mergeLiveEntry(prev, entry));
    }
  ...
  useEffect(() => {
    ...
    if (!thread || isRetiredHeartbeatThread(thread)) return;
  ...
  const handleForwardToAgent = useCallback(
    async (targetAgentId: any) => {
      if (!forwardEntry?.id || !selectedThread?.id || forwardSubmitting) return;
  ...
  const forwardTargets = (Array.isArray(agents) ? agents : []).filter(
      ...
      <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
        <View style={styles.topBar}>
          ...
          <View style={[styles.typeBadge, { borderColor: typeColor }]}>
            <Text style={[styles.typeBadgeText, { color: typeColor }]}>{selectedThread.type}</Text>
          </View>
        </View>
              ...
              {entries.map((entry: any, idx: any) => {
                ...
                return (
                  <View key={entry.id}>
                    ...
                    <View style={[styles.entryCard, isError && styles.entryCardError]}>
                      <View style={styles.entryHeader}>
                        <Text style={styles.entryTimestamp}>
                          {formatEntryTimestamp(entry.timestamp)}
                        </Text>
                        <TouchableOpacity
                          onPress={() => openForward(entry)}
                          style={styles.entryForwardBtn}
                          accessibilityLabel="Forward message to an agent"
                        >
              ...
              <View style={styles.composeRow}>
                ...
                <TouchableOpacity
                  style={[
                    ...
                    (!composeText.trim() || composeSending) && styles.composeButtonDisabled,
                  ]}
        ...
        {forwardDone && (
          <View style={styles.forwardToast}>
          ...
          <View style={styles.modalBackdrop}>
            <View style={styles.modalCard}>
              ...
              <TextInput
                ...
                value={forwardPrompt}
                onChangeText={setForwardPrompt}
                placeholder="Extra instructions (optional)"
                placeholderTextColor={colors.gray600}
                multiline
                editable={!forwardSubmitting}
              />
              {forwardError && <Text style={styles.modalError}>{forwardError}</Text>}
              {forwardTargets.length === 0 ? (
                <Text style={styles.modalEmpty}>No agents available to forward to.</Text>
              ) : (
                <ScrollView style={styles.modalAgentList}>
                  {forwardTargets.map((agent: any) => (
                    <TouchableOpacity
                      key={agent.id}
                      style={styles.modalAgentRow}
                      onPress={() => handleForwardToAgent(agent.id)}
                      disabled={forwardSubmitting}
                    >
                      <View
                        style={[
                          styles.modalAgentDot,
                          { backgroundColor: agent.color || colors.gray500 },
                        ]}
                      />
                      <View style={{ flex: 1, minWidth: 0 }}>
                        <Text style={styles.modalAgentName} numberOfLines={1}>
                          {agent.name || agent.id}
                        </Text>
                        <Text style={styles.modalAgentMeta} numberOfLines={1}>
                          {agent.engine}
                        </Text>
                      </View>
                    </TouchableOpacity>
              ...
              {forwardSubmitting && (
                <View style={styles.modalSubmitting}>
                  <ActivityIndicator size="small" color={colors.gray400} />
    ...
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      {loading ? (
        ...
        <View style={styles.centerState}>
          ...
          <Text style={styles.emptyDesc}>Threads are created automatically by cron jobs</Text>
        </View>
      ) : (
        <FlatList
          data={threads}
          ...
          renderItem={({ item }: any) => {
            const typeColor = colors.blue400;
            return (
              <TouchableOpacity style={styles.threadItem} onPress={() => handleSelectThread(item)}>
                <View style={[styles.typeDot, { backgroundColor: typeColor }]} />
                <View style={styles.threadInfo}>
                  <Text style={styles.threadName} numberOfLines={1}>
                    {item.name}
                  </Text>
                  <Text style={styles.threadMeta}>{relativeTime(item.created_at)}</Text>
                </View>
                <View style={[styles.typeBadge, { borderColor: typeColor }]}>
