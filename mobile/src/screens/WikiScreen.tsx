...
function CategoryBadge({ category }: any) {
    ...
    return (<View style={[styles.badge, { backgroundColor: cat.color + '22' }]}>
...
export default function WikiScreen({ route }: any) {
    ...
    if (!selectedPage && !creating) {
        ...
        <View style={styles.topBar}>
          ...
          {project && (<Text style={styles.projectLabel} numberOfLines={1}>{project.name}</Text>)}
        ...
        <View style={styles.searchContainer}>
          <TextInput style={styles.searchInput} placeholder="Search wiki..." placeholderTextColor={colors.gray600} value={searchQuery} onChangeText={handleSearch}/>
        </View>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.categoryBar} contentContainerStyle={styles.categoryContent}>
          {CATEGORIES.map((cat: any) => (<TouchableOpacity key={cat.key} style={[
                    styles.categoryChip,
        ...
        {/* Page list */}
        {pages.length === 0 ? (<View style={styles.emptyState}>
            <Text style={styles.emptyTitle}>No wiki pages yet</Text>
            <Text style={styles.emptyDesc}>
              Create pages to build a shared knowledge base for your agents.
              Agents can search and update the wiki as they work.
            </Text>
            <TouchableOpacity style={styles.emptyButton} onPress={handleCreate}>
              <Text style={styles.emptyButtonText}>Create First Page</Text>
            </TouchableOpacity>
          </View>) : (<FlatList data={pages} keyExtractor={(item: any) => item.id || item.slug} renderItem={({ item }: any) => (<TouchableOpacity style={styles.pageItem} onPress={() => handleSelectPage(item)} onLongPress={() => handleDelete(item)}>
                <View style={styles.pageItemHeader}>
                  <Text style={styles.pageTitle} numberOfLines={1}>{item.title}</Text>
                  <CategoryBadge category={item.category}/>
                </View>
                {item.snippet && (<Text style={styles.pageSnippet} numberOfLines={2}>{item.snippet.replace(/<\/?mark>/g, '')}</Text>)}
    ...
    return (<SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      <View style={styles.topBar}>
        ...
        {!editing && !creating && selectedPage && (<>
            ...
            <TouchableOpacity onPress={() => handleDelete(selectedPage)} style={styles.headerAction}>
      ...
      {editing || creating ? (<KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={0}>
          <ScrollView style={styles.editContainer} keyboardShouldPersistTaps="handled">
            ...
            <TextInput style={styles.fieldInput} value={editTitle} onChangeText={setEditTitle} placeholder="Page title" placeholderTextColor={colors.gray600}/>

            {/* Category */}
            <Text style={styles.fieldLabel}>Category</Text>
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 16 }}>
              {CATEGORIES.filter((c: any) => c.key !== 'all').map((cat: any) => (<TouchableOpacity key={cat.key} style={[
                    styles.categoryChip,
            ...
            <TextInput style={styles.contentInput} value={editContent} onChangeText={setEditContent} placeholder="Write your wiki page in markdown..." placeholderTextColor={colors.gray600} multiline textAlignVertical="top"/>
          ...
          <View style={styles.markdownContainer}>
            <Markdown style={mdStyles as any}>{selectedPage?.content || '*No content yet*'}</Markdown>
          </View>
        </ScrollView>)}
    </SafeAreaView>);
}
