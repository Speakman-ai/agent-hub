...
function ProvisioningBanner({ info }) {
  if (!info) return null;
  const lines = [];
  if (info.jobId) lines.push(`Job: ${info.jobId}`);
  if (info.status) lines.push(`Status: ${info.status}`);
  if (info.message) lines.push(info.message);
  if (lines.length === 0) return null;
  return (
    <View style={styles.provisionBox}>
      <Text style={styles.provisionTitle}>Provisioning</Text>
      {lines.map((line) => (
        <Text key={line} style={styles.provisionLine}>
          {line}
        </Text>
      ))}
    </View>
...
export default function NewProjectScreen({ navigation }) {
  ...
  const [provisioningInfo, setProvisioningInfo] = useState(null);

  const slugPreview = useMemo(() => slugifyProjectId(name.trim()), [name]);
        ...
        <ScrollView contentContainerStyle={styles.form} keyboardShouldPersistTaps="handled">
          {createdProject ? (
            <View style={styles.successBox}>
              ...
              {createdProject.githubRepo ? (
                <Text style={styles.successMeta}>GitHub: {createdProject.githubRepo}</Text>
              ) : null}
            ...
            <>
              ...
              />
              {slugPreview ? (
                <Text style={styles.hint}>Project id: {slugPreview}</Text>
              ) : null}
