      ...
      {detail && (
        <>
          ...
          {(detail.parents || []).map((p) => (
            <TouchableOpacity key={p} onPress={() => onOpenCommit(p)}>
              <Text style={detailStyles.parent}>parent {shortSha(p)}</Text>
            </TouchableOpacity>
          ))}
          {files.map((section, i) => (
            <View key={`${section.filename}-${i}`} style={detailStyles.fileBlock}>
              <Text style={detailStyles.fileName}>
                {section.filename} (+{section.additions} −{section.deletions})
              </Text>
              {section.isBinary ? (
                <Text style={detailStyles.binary}>Binary file</Text>
              ) : (
          ...
          {detail.patchTruncated && (
            <Text style={detailStyles.truncated}>Diff truncated at 1 MiB.</Text>
          )}
        </>
      )}
    </View>
...
export default function RepositoryScreen({ route, navigation }) {
  ...
  const refresh = useCallback(async (branch) => {
    if (!projectId) return;
    setLoading(true);
    setError(null);
    try {
          ...
          <>
            ...
            {commits.map((commit) => (
              <TouchableOpacity
                ...
                onPress={() => setCommitSha(commit.sha)}
              >
            ...
            {commits.length === 0 && !error && (
              <Text style={styles.empty}>No commits on this branch.</Text>
            )}
          </>
        )}
        ...
        {tab === 'branches' && !loading && (
          <>
            {branches.map((b) => {
              ...
              return (
                <View key={name} style={styles.branchRow}>
