...
function MessageBubble({ message }: any) {
    ...
    return (<View style={[styles.bubble, isUser ? styles.userBubble : styles.assistantBubble]}>
      <Text style={styles.roleLabel}>{isUser ? 'You' : 'Design Studio'}</Text>
      {isUser ? (<Text style={styles.messageText}>{message.content}</Text>) : (<Markdown style={mdStyles as any}>{message.content || ''}</Markdown>)}
    </View>);
}
export default function DesignViewScreen({ route, navigation }: any) {
    ...
    useEffect(() => {
        if (!lastDesignEvent || lastDesignEvent.designId !== designId)
            return;
    ...
    const handleSend = () => {
        ...
        if (!trimmed || !designId || processing)
            return;
    ...
    const displayMessages = streaming
        ? [...messages.filter((m: any) => m.id !== streaming.id), streaming]
        : messages;
    return (<SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
      <ProjectScreenHeader title={design?.name || 'Design'} onBack={() => navigation.goBack()} right={!connected ? (<Text style={styles.offline}>Offline</Text>) : processing ? (<ActivityIndicator size="small" color={colors.purple400} style={{ marginLeft: 'auto' }}/>) : null}/>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={Platform.OS === 'ios' ? 8 : 0}>
        {loading ? (<View style={styles.center}>
          ...
          </View>) : (<FlatList ref={listRef} data={displayMessages} keyExtractor={(item: any, index: any) => item.id || String(index)} renderItem={({ item }: any) => <MessageBubble message={item}/>} contentContainerStyle={styles.messages} onContentSizeChange={() => listRef.current?.scrollToEnd({ animated: true })} ListFooterComponent={thinking ? (<View style={styles.thinkingRow}>
                  <ActivityIndicator size="small" color={colors.gray500}/>
                  <Text style={styles.thinkingText}>Thinking…</Text>
                </View>) : null} ListEmptyComponent={<Text style={styles.emptyText}>
                Chat with the Design Studio agent to generate UI artifacts.
              </Text>}/>)}

        <View style={styles.composer}>
          <TextInput style={styles.composerInput} value={input} onChangeText={setInput} placeholder="Describe your design…" placeholderTextColor={colors.gray600} multiline editable={!processing}/>
          <TouchableOpacity style={[styles.sendBtn, (!input.trim() || processing) && styles.sendBtnDisabled]} onPress={handleSend} disabled={!input.trim() || processing}>
