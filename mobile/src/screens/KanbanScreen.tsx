...
export default function KanbanScreen({ route, navigation }: any) {
  ...
  const projectAgents = useMemo<any>(
  ...
  const [selectedCreateTemplate, setSelectedCreateTemplate] = useState<KanbanCardTemplate | null>(
    null,
  );
  ...
  const doneColumnIds = new Set(
    columns
      .filter((c: any) => /done|complete|closed/i.test(c.name || c.id || ''))
      .map((c: any) => c.id),
  );
  ...
  const reconcileBoard = useCallback(async () => {
    ...
    try {
      ...
      setActiveColumn((cur: any) => {
        ...
        if (cur && cols.some((c: any) => c.id === cur)) return cur;
        return cols[0].id;
      });
    } catch (err: any) {
  ...
  useFocusEffect(
    useCallback(() => {
      if (!boardLoadedRef.current) return undefined;
      void reconcileBoard();
      return undefined;
    }, [reconcileBoard]),
  );
  ...
  useEffect(() => {
    ...
    reconcileBoard();
  }, [kanbanRefreshKey, acknowledgeKanbanRefresh, projectId, reconcileBoard]);
  ...
  const drainColumn = useCallback(
    async (columnId: any) => {
      ...
      try {
        ...
        if (collected.length) setCards((prev: any) => appendCardPage(prev, collected));
        setColumnPaging((prev: any) => ({
          ...prev,
  ...
  const handleCreateCard = async () => {
    ...
    try {
      ...
      if (selectedCreateTemplate) payload.priority = newCardPriority;
  ...
  const quickAssign = useCallback(
    async (card: any, agentId: any, agentName: any) => {
      try {
      ...
      if (isPrematureDoneMoveError(err)) {
        Alert.alert(PREMATURE_DONE_MOVE_TITLE, PREMATURE_DONE_MOVE_EXPLANATION, [
          { text: 'Cancel', style: 'cancel' },
          {
            ...
            onPress: async () => {
              try {
                await api.moveKanbanCard(projectId, cardId, {
  ...
  const runCardAction = (card: any, action: any) => {
    ...
    switch (action.type) {
      ...
      case 'copyLink': {
        ...
        copyToClipboard(
          cardCopyPayload(card, action.type, {
          ...
          {hasFooter && (
            <View style={styles.cardFooter}>
              <View style={styles.cardFooterLeft}>
                ...
                {meta.labels.map((label: any, i: any) => (
                ...
                >
                  ...
                  <Text
                    style={[
                      ...
                      editPriority === p.value && { color: colors.white },
                    ]}
                  >
                    {p.label}
                  </Text>
            ...
            {hasActiveSession(selectedCard) && !showReassign ? (
              <View>
                <View style={styles.assigneeActiveRow}>
                  <Text style={styles.assigneeActiveName}>{editAssignee || 'Assigned'}</Text>
              ...
              <View>
                ...
                {!!editAssignee &&
                  validModelsForAgent(agents, modelConfig, editAssignee, editAssignEngine).length >
                    0 && (
                    <TouchableOpacity
                      style={[styles.epicPickerBtn, { marginTop: 8 }]}
                ...
                {!!editAssignee && (
                  <TouchableOpacity
                    style={[styles.assignStartBtn, assigning && styles.assignStartBtnDisabled]}
                ...
                return (
                  <View style={styles.epicPickerRow}>
                    <View
                      style={[
                        styles.epicDot,
                        { backgroundColor: linked.color || DEFAULT_EPIC_COLOR },
                      ]}
                    />
            ...
            {(selectedCard.blockers || []).length === 0 ? (
              ...
              selectedCard.blockers.map((b: any) => (
                <View
                  ...
                  style={[
                    styles.blockerRow,
                    b.done ? styles.blockerRowDone : styles.blockerRowOpen,
                  ]}
                >
                  <Text
          ...
          >
            <View style={styles.modalContent}>
              ...
              <ScrollView style={{ maxHeight: 320 }}>
                {buildAssigneeOptions(projectAgents).map((opt: any) => (
                  <TouchableOpacity
          ...
          >
            <View style={styles.modalContent}>
              ...
              <ScrollView style={{ maxHeight: 320 }}>
                ...
                {engineEntriesWithModels(modelConfig).map((eng: any) => (
                  <TouchableOpacity
                    key={eng}
                    style={styles.modalOption}
          ...
          >
            <View style={styles.modalContent}>
              ...
              <ScrollView style={{ maxHeight: 320 }}>
                ...
                {validModelsForAgent(agents, modelConfig, editAssignee, editAssignEngine).map(
                  (m: any) => (
                    <TouchableOpacity
                      key={m}
                      style={styles.modalOption}
                ...
                {epics.map((epic: any) => (
                  ...
                  >
                    <View
                      style={[
                        styles.modalOptionDot,
                        { backgroundColor: epic.color || DEFAULT_EPIC_COLOR },
                      ]}
                    />
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      ...
      <View style={styles.epicBar}>
          ...
          {selectedEpic ? (
            <>
              <View
                style={[
                  styles.epicDot,
                  { backgroundColor: selectedEpic.color || DEFAULT_EPIC_COLOR },
                ]}
              />
        ...
        {selectedEpic && (
          <TouchableOpacity style={styles.epicEditBtn} onPress={() => openEpicEdit(selectedEpic)}>
            ...
            >
              ...
              {count > 0 && (
                <View
                  style={[
                    styles.tabBadge,
                    { backgroundColor: isActive ? col.color : colors.gray700 },
                  ]}
                >
        ...
        ListEmptyComponent={
          <View style={styles.emptyCol}>
            <Text style={styles.emptyColText}>
              No cards in {activeColumnObj?.name || 'this column'}
            </Text>
          </View>
        }
        ListFooterComponent={
            ...
            <View style={styles.addCardPriorityRow}>
              {PRIORITY_OPTIONS.map((p: any) => (
                ...
                >
                  ...
                  <Text
                    style={[
                      styles.addCardPriorityText,
                      newCardPriority === p.value && { color: colors.white },
                    ]}
                  >
                    {p.label}
                  </Text>
            ...
            <View style={styles.addCardActions}>
              ...
              <TouchableOpacity
                style={[
                  ...
                  (!newCardTitle.trim() || saving) && styles.addCardCreateDisabled,
                ]}
            ...
            <ScrollView style={{ maxHeight: 360 }}>
              ...
              {templateOptions.map((template) => (
                ...
                >
                  ...
                  {selectedCreateTemplate?.id === template.id && (
              ...
              {templateOptions.length === 0 && (
                <Text style={styles.noTemplatesText}>
                  No templates yet. Create one from the Templates screen.
                </Text>
              )}
            </ScrollView>
            <TouchableOpacity
                    ...
                    <>
                      ...
                      <ScrollView style={{ maxHeight: 360 }}>
                        {options.map((opt: any) => (
                          ...
                          >
                            {opt.color != null && (
                              <View style={[styles.actionDot, { backgroundColor: opt.color }]} />
                            )}
                            <Text
                              style={[
          ...
          <View style={styles.modalContent}>
            ...
            <ScrollView style={{ maxHeight: 360 }}>
              ...
              >
                <View style={[styles.modalOptionDot, { backgroundColor: colors.gray600 }]} />
                <Text style={styles.modalOptionText}>
                  No epic ({filterCardsByEpic(cards, NO_EPIC_FILTER_ID).length})
                </Text>
              </TouchableOpacity>
              {epicsWithActiveCards(
                epics,
                (id: any) => countOpenCardsForEpic(cards, id, doneColumnIds),
                selectedEpicId,
              ).map((epic: any) => {
      ...
      >
        ...
        >
          <View style={[styles.modalContent, { width: 320 }]}>
            <ScrollView>
              <Text style={styles.modalTitle}>{editingEpic ? 'Edit Epic' : 'New Epic'}</Text>
              ...
              {editingEpic && (
                <TouchableOpacity style={styles.deleteEpicBtn} onPress={handleDeleteEpic}>
