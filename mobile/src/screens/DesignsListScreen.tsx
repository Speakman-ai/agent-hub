...
import { View, Text, TouchableOpacity, FlatList, StyleSheet, ActivityIndicator, Alert, Modal, TextInput, RefreshControl, } from 'react-native';
...
export default function DesignsListScreen({ navigation }: any) {
    ...
    const headerRight = (<TouchableOpacity style={styles.newBtn} onPress={() => setShowNew(true)}>
      <Text style={styles.newBtnText}>+ New</Text>
    </TouchableOpacity>);
    const renderItem = ({ item: d }: any) => (<TouchableOpacity style={styles.card} onPress={() => openDesign(d)} onLongPress={() => handleDelete(d.id)}>
      <View style={styles.cardTop}>
        <Text style={styles.cardTitle} numberOfLines={1}>
          {d.name}
        </Text>
        {isDesignMigrated(d) ? <Text style={styles.migratedBadge}>Migrated</Text> : null}
        {deletingId === d.id ? (<ActivityIndicator size="small" color={colors.gray500}/>) : null}
      </View>
      {d.linkedProjects?.length > 0 && (<View style={styles.linkedRow}>
          {d.linkedProjects.slice(0, 3).map((p: any) => (<Text key={p.id} style={[styles.linkedTag, { color: p.color || colors.gray400 }]}>
              {p.name}
            </Text>))}
        </View>)}
    ...
    return (<SafeAreaView style={styles.container} edges={['top', 'left', 'right']}>
      ...
      {loading && designs.length === 0 ? (<View style={styles.center}>
          <ActivityIndicator size="large" color={colors.blue600}/>
        </View>) : (<FlatList data={designs} keyExtractor={(item: any) => item.id} renderItem={renderItem} numColumns={1} contentContainerStyle={designs.length === 0 ? styles.center : styles.list} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => {
                    setRefreshing(true);
                    load();
                }} tintColor={colors.gray400}/>} ListEmptyComponent={<Text style={styles.emptyText}>
      ...
      <Modal visible={showNew} transparent animationType="fade" onRequestClose={() => setShowNew(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <TextInput style={styles.modalInput} value={newName} onChangeText={setNewName} placeholder="Design name" placeholderTextColor={colors.gray600} autoFocus/>
            <View style={styles.modalActions}>
              <TouchableOpacity style={styles.modalCancel} onPress={() => setShowNew(false)}>
                <Text style={styles.modalCancelText}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.modalCreate, creating && { opacity: 0.6 }]} onPress={handleCreate} disabled={creating || !newName.trim()}>
                {creating ? (<ActivityIndicator size="small" color={colors.white}/>) : (<Text style={styles.modalCreateText}>Create</Text>)}
              </TouchableOpacity>
            </View>
          </View>
        </View>
