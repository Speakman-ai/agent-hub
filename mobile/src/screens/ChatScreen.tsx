...
export default function ChatScreen({ embedded = false, composePrefix = '', emptyHint, onClearChat }: { embedded?: boolean; composePrefix?: string; emptyHint?: string; onClearChat?: () => void; } = {}) {
    ...
    return (<SafeAreaView style={styles.container} edges={embedded ? ['bottom'] : ['top', 'bottom']}>
      ...
      {activeSessionId && !hubSession ? (<SessionArtifactsPanel sessionId={activeSessionId} reloadNonce={artifactReloadBySession?.[activeSessionId] || 0} presentedArtifact={presentedArtifactBySession?.[activeSessionId] || null} onPresentedArtifact={acknowledgePresentedArtifact}/>) : null}
      ...
      <AgentSwitcher visible={showSwitcher} agents={agents} onSelect={(id: any) => setActiveAgentId(id)} onClose={() => setShowSwitcher(false)}/>
    </SafeAreaView>);
}
