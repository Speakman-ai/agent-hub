...
function OrganizationsSection() {
  ...
  const renderColorPicker = (selectedColor: any, onSelect: any) => (
  ...
  const renderForm = (form: any, setForm: any, onTest: any) => (
    <View style={styles.orgForm}>
      <Text style={styles.inputLabel}>Name</Text>
      <TextInput
        style={styles.textInput}
        value={form.name}
        onChangeText={(v: any) => setForm((f: any) => ({ ...f, name: v }))}
      ...
      <TextInput
        ...
        onChangeText={(v: any) => setForm((f: any) => ({ ...f, remoteUrl: v }))}
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
        <TextInput
          ...
          value={form.apiKey}
          onChangeText={(v: any) => setForm((f: any) => ({ ...f, apiKey: v }))}
          ...
          <View key={org.id} style={styles.orgCard}>
            <TouchableOpacity style={styles.orgCardHeader} onPress={() => handleExpand(org.id)}>
              ...
              <View style={{ flex: 1 }}>
                ...
                {org.remoteUrl ? (
                  <Text style={styles.orgCardUrl} numberOfLines={1}>
                    {org.remoteUrl}
                  </Text>
                ) : (
                  <Text style={[styles.orgCardUrl, { color: colors.yellow400 }]}>
                    No URL configured
                  </Text>
                )}
              </View>
        ...
        <View style={styles.newOrgCard}>
          ...
          <View style={styles.orgCardActions}>
            ...
            <TouchableOpacity
              style={[
                styles.saveBtn,
                (!newForm.name?.trim() || !newForm.remoteUrl?.trim()) && { opacity: 0.4 },
              ]}
      ...
      {loading ? (
        ...
        <>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            ...
            />
            <TouchableOpacity
              onPress={() => setShowApiKey((value: any) => !value)}
              style={{ paddingHorizontal: 8, paddingVertical: 10 }}
            >
              <Text style={{ color: colors.gray400, fontSize: 13 }}>
                {showApiKey ? 'Hide' : 'Show'}
    ...
    <View>
      ...
      <View style={styles.usageGrid}>
        <View style={styles.usageCard}>
          <Text style={[styles.usageValue, { color: colors.emerald400 }]}>
            {fmtCost(usage.totals.total_cost)}
          </Text>
          <Text style={styles.usageLabel}>Total Cost</Text>
        </View>
        <View style={styles.usageCard}>
          <Text style={[styles.usageValue, { color: colors.blue400 }]}>
            {fmtDuration(usage.totals.total_duration_ms)}
          </Text>
      ...
      {usage.byAgent?.length > 0 && (
        <View style={{ marginTop: 20 }}>
          ...
          {usage.byAgent.map((a: any) => (
            <View key={a.agent_id} style={styles.agentUsageRow}>
              <View
                style={[styles.agentUsageDot, { backgroundColor: a.agent_color || colors.gray500 }]}
              />
              <Text style={styles.agentUsageName} numberOfLines={1}>
                {a.agent_name}
              </Text>
              <Text style={[styles.agentUsageStat, { color: colors.emerald400 }]}>
                {fmtCost(a.total_cost)}
              </Text>
          ...
          {usage.byDay.map((d: any) => (
            <View key={d.day} style={styles.dailyRow}>
              ...
              <View style={styles.dailyBarContainer}>
                <View
                  style={[styles.dailyBar, { flex: Math.max((d.cost || 0) / maxDailyCost, 0.01) }]}
                />
              </View>
      ...
      {usage.recentSessions?.length > 0 && (
        <View style={{ marginTop: 20 }}>
          ...
          {usage.recentSessions.slice(0, 10).map((s: any) => (
            <View key={s.id} style={styles.recentSessionRow}>
              <View
                style={[styles.agentUsageDot, { backgroundColor: s.agent_color || colors.gray500 }]}
              />
              <View style={{ flex: 1 }}>
                <Text style={styles.recentSessionName} numberOfLines={1}>
                  {s.session_name}
                </Text>
                <Text style={styles.recentSessionAgent}>
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      await Sharing.shareAsync(filePath, {
        ...
        dialogTitle: 'Export Agent Hub Config',
      });
    } catch (err: any) {
            ...
            <View style={styles.previewCard}>
              ...
              {preview.agents && (
                <Text style={styles.previewLine}>
                  Agents: {Array.isArray(preview.agents) ? preview.agents.length : 0}
                </Text>
              )}
              {preview.crons && (
                <Text style={styles.previewLine}>
                  Crons: {Array.isArray(preview.crons) ? preview.crons.length : 0}
                </Text>
              )}
              {preview.rooms && (
                <Text style={styles.previewLine}>
                  Rooms: {Array.isArray(preview.rooms) ? preview.rooms.length : 0}
                </Text>
              )}
              {preview.projects && (
                <Text style={styles.previewLine}>
                  Projects: {Array.isArray(preview.projects) ? preview.projects.length : 0}
                </Text>
              )}
              {preview.exportedAt && (
                <Text style={styles.previewLine}>
...
function CronFormFields({ form, setForm, projects, modelConfig }: any) {
  ...
  return (
    <>
      ...
      <TextInput
        value={form.schedule}
        onChangeText={(v: any) => setForm({ ...form, schedule: v })}
      ...
      <TextInput
        value={form.prompt}
        onChangeText={(v: any) => setForm({ ...form, prompt: v })}
        placeholder="Prompt"
        placeholderTextColor={colors.gray500}
        ...
        <>
          ...
          >
            <TouchableOpacity
              ...
              style={[styles.projectChip, !form.project_id && styles.projectChipActive]}
            >
              <Text
                style={[styles.projectChipText, !form.project_id && styles.projectChipTextActive]}
              >
                No project
              </Text>
            </TouchableOpacity>
      ...
      <TextInput
        value={form.cwd}
        onChangeText={(v: any) => setForm({ ...form, cwd: v })}
      ...
      <TextInput
        ...
        onChangeText={(v: any) => setForm({ ...form, timeoutMinutes: v })}
        placeholder="e.g. 30"
        placeholderTextColor={colors.gray500}
        style={styles.formInput}
        ...
        <>
          ...
          >
            <TouchableOpacity
              ...
              style={[styles.projectChip, !form.engine && styles.projectChipActive]}
              accessibilityLabel="Use default cron engine"
            >
              <Text style={[styles.projectChipText, !form.engine && styles.projectChipTextActive]}>
                Default
              </Text>
            </TouchableOpacity>
        ...
        <>
          ...
          >
            <TouchableOpacity
              ...
              style={[styles.projectChip, !form.model && styles.projectChipActive]}
            >
              <Text style={[styles.projectChipText, !form.model && styles.projectChipTextActive]}>
      ...
      >
        <View style={[styles.notifyCheckbox, !!form.notify_on_run && styles.notifyCheckboxChecked]}>
      ...
      <TouchableOpacity
        onPress={() => setForm({ ...form, shared: !form.shared })}
        style={styles.notifyToggleRow}
        accessibilityRole="checkbox"
        accessibilityState={{ checked: !!form.shared }}
      >
        <View style={[styles.notifyCheckbox, !!form.shared && styles.notifyCheckboxChecked]}>
...
function CronSection() {
  ...
  const startEditing = (cronJob: any) => {
    ...
    setEditForm({
      ...
      timeoutMinutes: cronJob.timeout_ms ? String(Math.round(cronJob.timeout_ms / 60000)) : '',
            ...
            {editingId === cronJob.id ? (
              ...
              <View style={styles.cardRow}>
                        ...
                        return (
                          ...
                          >
                            ...
                            {log.result ? (
                              ...
                              <Text
                                style={{ fontSize: 11, color: colors.gray600, fontStyle: 'italic' }}
                              >
                                No output yet
                              </Text>
                            )}
                ...
                <View style={styles.actionButtons}>
                  <TouchableOpacity
                    style={[styles.smallButton, !cronJob.can_manage && styles.disabledButton]}
                  ...
                  <TouchableOpacity
                    style={[
                      styles.smallButton,
                      (!cronJob.can_manage || running[cronJob.id]) && styles.disabledButton,
                    ]}
                    onPress={() => triggerRun(cronJob.id)}
                    disabled={!cronJob.can_manage || running[cronJob.id]}
                  >
                  ...
                  <Text
                    style={[
                      ...
                      bot.connected
                        ...
                        : styles.statusPillTextDisconnected,
                    ]}
                  >
    ...
    <View>
        ...
        <View style={[styles.formCard, { marginBottom: 12 }]}>
          ...
          <View style={styles.engineToggle}>
            {engineChoices.map((eng: any) => (
              <TouchableOpacity
                key={eng}
                style={[styles.engineOption, bulkEngine === eng && styles.engineOptionActive]}
                onPress={() => {
                  setBulkEngine(eng);
                  setBulkModel('');
                }}
            ...
            {getModelsForEngine(bulkEngine).map((m: any) => (
              ...
              >
                <Text
                  style={[
                    styles.engineOptionText,
                    (bulkModel || getDefaultModel(bulkEngine)) === m &&
      ...
      {showNew && (
        <View style={styles.formCard}>
          <TextInput
            value={newForm.id}
            onChangeText={(v: any) => setNewForm({ ...newForm, id: v })}
          ...
          <TextInput
            value={newForm.name}
            onChangeText={(v: any) => setNewForm({ ...newForm, name: v })}
            placeholder="Name"
            placeholderTextColor={colors.gray500}
            style={styles.formInput}
          />
          <TextInput
            value={newForm.cwd}
            onChangeText={(v: any) => setNewForm({ ...newForm, cwd: v })}
            placeholder="Working Directory"
            placeholderTextColor={colors.gray500}
            style={styles.formInput}
          />
          <TextInput
            ...
            onChangeText={(v: any) => setNewForm({ ...newForm, workspace: v })}
          ...
          <View style={styles.engineToggle}>
            {engineChoices.map((eng: any) => (
              <TouchableOpacity
                key={eng}
                style={[styles.engineOption, newForm.engine === eng && styles.engineOptionActive]}
          ...
          <View style={styles.engineToggle}>
            {getModelsForEngine(newForm.engine).map((m: any) => (
              <TouchableOpacity
                ...
                style={[
                  styles.engineOption,
                  (newForm.model || getDefaultModel(newForm.engine)) === m &&
          ...
          <TextInput
            ...
            onChangeText={(v: any) => setNewForm({ ...newForm, systemPrompt: v })}
            ...
            <View key={agent.id} style={styles.card}>
              ...
              {isExpanded && (
                <View style={styles.expandedSection}>
                  ...
                  <TextInput
                    value={edit.name || ''}
                    onChangeText={(v: any) => setEdit(agent.id, 'name', v)}
                    style={styles.formInput}
                  />
                          ...
                          <Text
                            style={[
                              ...
                              (edit.model ||
                                agent.model ||
                                getDefaultModel(edit.engine || agent.engine || 'claude-code')) ===
                                m && styles.engineOptionTextActive,
                            ]}
                          >
                            {m.replace(/^claude-/, '').replace(/^gpt-/, '')}
                          </Text>
                  ...
                  <TextInput
                    value={edit.cwd || ''}
                    onChangeText={(v: any) => setEdit(agent.id, 'cwd', v)}
                    style={styles.formInput}
                  />
                  ...
                  <TextInput
                    ...
                    onChangeText={(v: any) => setEdit(agent.id, 'workspace', v)}
                    style={styles.formInput}
                  />
                  ...
                  <TextInput
                    ...
                    onChangeText={(v: any) => setEdit(agent.id, 'systemPrompt', v)}
                  ...
                  <View style={styles.expandedActions}>
                    ...
                    <TouchableOpacity
                      ...
                      style={[
                        styles.saveAgentButton,
                        (!isDirty || saving[agent.id]) && { opacity: 0.5 },
                      ]}
                    >
              ...
              return (
                ...
                >
                  <Text
                    style={[
                      styles.smallButtonText,
                      active ? styles.buttonOnText : styles.buttonOffText,
                    ]}
                  >
...
export default function SettingsScreen({ route }: any) {
  ...
  useEffect(() => {
    if (tab === 'jobs' && !isAdminPlus) setTab('general');
  }, [tab, isAdminPlus]);
  const activeTab = useMemo<any>(
    () => SETTINGS_TABS.find((t: any) => t.id === tab) ?? SETTINGS_TABS[1],
    [tab],
  );
