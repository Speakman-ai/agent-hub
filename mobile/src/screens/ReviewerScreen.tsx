...
function ContextFilePanel({ filename, content, agentId, onSaved }: any) {
    ...
    if (content === null || content === undefined)
        return null;
    return (<View style={panelStyles.card}>
      ...
      {expanded && (<View style={panelStyles.body}>
          <View style={panelStyles.editRow}>
            <TouchableOpacity style={[panelStyles.editBtn, editing && panelStyles.editBtnActive]} onPress={() => setEditing(!editing)}>
              <Text style={panelStyles.editBtnText}>{editing ? 'Editing' : 'Edit'}</Text>
            </TouchableOpacity>
            {editing && (<TouchableOpacity style={panelStyles.saveBtn} onPress={handleSave} disabled={saving}>
                <Text style={panelStyles.saveBtnText}>{saving ? 'Saving…' : 'Save'}</Text>
              </TouchableOpacity>)}
          </View>
          {editing ? (<TextInput value={editContent} onChangeText={setEditContent} multiline style={panelStyles.textarea} textAlignVertical="top"/>) : (<ScrollView style={panelStyles.scroll} nestedScrollEnabled>
              <Markdown style={mdStyles as any}>{content || '*(empty)*'}</Markdown>
            </ScrollView>)}
        </View>)}
    </View>);
}
export default function ReviewerScreen({ route, navigation }: any) {
    const { projectId, project: routeProject } = route.params || {};
    const { projects } = useApp();
    const project = routeProject || projects?.find((p: any) => p.id === projectId);
    const reviewerAgent = useMemo<any>(() => {
    ...
    useEffect(() => {
        ...
        api
            ...
            .catch((err: any) => {
            if (!cancelled)
                setError(err?.message || 'Failed to load reviewer files');
        })
            .finally(() => {
            if (!cancelled)
                setLoading(false);
    ...
    return (<SafeAreaView style={styles.screen} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {!project ? (<Text style={styles.empty}>Project not found.</Text>) : !reviewerAgent ? (<Text style={styles.empty}>
            No reviewer agent yet. Created automatically once GitHub integration is enabled.
          </Text>) : loading ? (<ActivityIndicator color={colors.gray400} style={{ marginTop: 24 }}/>) : error ? (<Text style={styles.error}>{error}</Text>) : fileEntries.length === 0 ? (<Text style={styles.empty}>No markdown files found for this reviewer.</Text>) : (fileEntries.map(([filename, content]: any) => (<ContextFilePanel key={filename} filename={filename} content={content} agentId={reviewerAgent.id} onSaved={handleSaved}/>)))}
      </ScrollView>
    </SafeAreaView>);
}
