          ...
          return (
            <View key={row.name} style={styles.card}>
              ...
              <Text style={styles.meta}>
                Type: {isStatic ? 'static' : isRole ? 'assume role' : 'SSO'}
              </Text>
              {isSso && <Text style={styles.meta}>Account: {row.sso_account_id || '-'}</Text>}
              {isSso && <Text style={styles.meta}>Role: {row.sso_role_name || '-'}</Text>}
              {isStatic && (
                <Text style={styles.meta}>Access key: {row.aws_access_key_id || '-'}</Text>
              )}
              {isRole && <Text style={styles.meta}>Role ARN: {row.role_arn || '-'}</Text>}
              {isRole && (
                <Text style={styles.meta}>
                  Source:{' '}
                  {row.source_profile || row.credential_source || ambientCredentialSource || 'auto'}
                </Text>
              )}
              ...
              <View style={styles.actions}>
                <TouchableOpacity
                  style={styles.btn}
                  onPress={() => checkStatus(row.name)}
                  disabled={st?.loading}
                >
                  <Text style={styles.btnText}>
                    {st?.loading ? 'Checking…' : isSso ? 'Check SSO' : 'Check credentials'}
                  </Text>
                </TouchableOpacity>
                {isSso && (
                  <TouchableOpacity
                    style={styles.btn}
                    onPress={() => startLogin(row.name)}
                    disabled={lg?.loading}
                  >
