...
function DelegationPanel({ delegations, onCancel, sessionId }) {
  ...
  const doneCount = delegations.filter((t) => t.status === 'done').length;
  const errorCount = delegations.filter((t) => t.status === 'error').length;
  const total = delegations.length;
  const allFinished = delegations.every((t) => ['done', 'error', 'cancelled'].includes(t.status));
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity style={styles.header} onPress={() => setCollapsed(!collapsed)}>
        ...
        {!allFinished && onCancel && (
          <TouchableOpacity style={styles.cancelBtn} onPress={() => onCancel(sessionId)}>
      ...
      {!collapsed && (
        <View style={styles.taskList}>
          {delegations.map((task, idx) => {
            ...
            return (
              <View key={task.delegationId || idx} style={styles.taskRow}>
                <TouchableOpacity
                  style={styles.taskHeader}
                  onPress={() => hasOutput && toggleTask(idx)}
                  disabled={!hasOutput}
                >
                ...
                {isExpanded && (
                  <ScrollView style={styles.outputScroll} nestedScrollEnabled>
                    ...
                    ) : task.status === 'running' && task.content ? (
                      <View style={styles.outputBox}>
                        <Markdown style={mdStyles}>{task.content}</Markdown>
                      </View>
                    ) : task.status === 'done' && task.output ? (
                      <View style={styles.outputBox}>
                        <Markdown style={mdStyles}>{task.output}</Markdown>
                      </View>
                    ) : task.status === 'pending' ? (
