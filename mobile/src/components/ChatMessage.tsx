...
import { View, Text, StyleSheet, Image, TouchableOpacity, Modal, Dimensions, TextInput, Pressable, ToastAndroid, Platform, Alert, Linking, } from 'react-native';
...
import { getUserMessageFlags } from '../utils/chatMessageUserFlags';
...
function notifyCopied(success: any) {
...
function MessageAttachments({ attachments }: any) {
    const [lightboxUrl, setLightboxUrl] = useState<any>(null);
    ...
    if (parsed.length === 0)
        return null;
    const getDisplayUrl = (att: any) => {
        if (att.url)
            return `${getApiBaseUrl().replace('/api', '')}${att.url}`;
        if (att.dataUrl)
            return att.dataUrl;
        return null;
    };
    const openFile = (src: any) => {
        if (!src)
            return;
    ...
    return (<>
      <View style={imageStyles.row}>
        {parsed.map((att: any, i: any) => {
            ...
            const key = att.id || i;
            if (kind === 'image') {
                return (<TouchableOpacity key={key} onPress={() => setLightboxUrl(src)} activeOpacity={0.8}>
                <Image source={{ uri: src }} style={imageStyles.thumb}/>
              </TouchableOpacity>);
            }
            if (kind === 'video') {
                return (<TouchableOpacity key={key} onPress={() => openFile(src)} activeOpacity={0.8} accessibilityLabel={`Open video ${att.originalName || att.filename || ''}`} style={[imageStyles.thumb, imageStyles.videoChip]}>
            ...
            return (<TouchableOpacity key={key} onPress={() => openFile(src)} activeOpacity={0.8} accessibilityLabel={`Open file ${att.originalName || att.filename || ''}`} style={[imageStyles.thumb, imageStyles.fileChip]}>
      ...
      {lightboxUrl && (<Modal transparent visible animationType="fade" onRequestClose={() => setLightboxUrl(null)}>
          <TouchableOpacity style={imageStyles.lightbox} activeOpacity={1} onPress={() => setLightboxUrl(null)}>
            <Image source={{ uri: lightboxUrl }} style={{ width: screenWidth * 0.9, height: screenWidth * 0.9 }} resizeMode="contain"/>
          </TouchableOpacity>
        </Modal>)}
    </>);
}
...
function SystemShipRequestedMessage({ message }: any) {
    const meta = parseShipRequestedMetadata(message.metadata);
    if (!meta) {
        return (<View style={prStyles.fallbackContainer}>
        <Text style={prStyles.fallbackText}>{message.content || 'System event'}</Text>
      </View>);
    }
    return (<View style={prStyles.container}>
      <View style={shipStyles.card}>
        ...
        {message.created_at ? (<Text style={prStyles.timestamp}>{relativeTime(message.created_at)}</Text>) : null}
      </View>
    </View>);
}
...
function SystemPrCreatedMessage({ message }: any) {
    ...
    return (<View style={prStyles.container}>
      <View style={prStyles.card}>
        ...
        <View style={prStyles.metaRow}>
          ...
          {meta.commitSha ? (<View style={prStyles.shaPill}>
              <Text style={prStyles.shaText}>{shortSha(meta.commitSha)}</Text>
            </View>) : null}
          {meta.commitTitle ? (<Text style={prStyles.commitTitle} numberOfLines={1}>
              {meta.commitTitle}
            </Text>) : null}
        </View>
        {meta.cardTitle ? (<Text style={prStyles.cardLine}>
...
function ChatMessage({ message, agentColor, agentName, onDequeue, onEditQueued, onInterrupt, inFlightWhileStreaming = false, fromAgent, agents, sessionHandoffs, onOpenSession, }: any) {
    const isSystem = message.role === 'system';
    const { isUser, isQueued, showInFlightActions } = getUserMessageFlags(message, inFlightWhileStreaming);
    ...
    const modelLabel = !isUser && message.model
        ...
        : null;
    const displayContent = useMemo<any>(() => {
        if (message.content === '(image attached)' && message.attachments)
            return '';
    ...
    const assistantBlocks = useMemo<any>(() => {
        if (isUser)
            return { stripped: message.content, handoff: null };
    ...
    const handoffRow = useMemo<any>(() => (assistantBlocks.handoff ? pickHandoffRow(assistantBlocks.handoff, sessionHandoffs) : null), [assistantBlocks.handoff, sessionHandoffs]);
    ...
    return (<View style={[styles.container, isUser ? styles.containerUser : styles.containerAssistant]}>
      <Pressable onLongPress={handleLongPress} delayLongPress={350} accessibilityRole="button" accessibilityLabel="Long press to copy message" style={({ pressed }: any) => [
            styles.bubble,
            isUser ? (isQueued ? styles.bubbleQueued : styles.bubbleUser) : styles.bubbleAssistant,
            pressed && styles.bubblePressed,
        ]}>
        {/* Assistant header */}
        {!isUser && (<View style={styles.assistantHeader}>
            ...
            {modelLabel && (<Text style={styles.modelLabel}>· {modelLabel}</Text>)}
          </View>)}
        ...
        {/* Content */}
        {isUser ? (<>
            {isQueued && (<View style={styles.queuedBadge}>
            ...
            {editing ? (<View style={styles.editContainer}>
                ...
                <View style={styles.editActions}>
                  <TouchableOpacity style={styles.editSaveBtn} onPress={() => { onEditQueued?.(message.id, editText); setEditing(false); }}>
              ...
              </View>) : (displayContent ? (<Text style={styles.userText} selectable>
                  {displayContent}
                </Text>) : null)}
            {isQueued && !editing && (<View style={styles.queuedActions}>
            ...
            {assistantBlocks.stripped ? (<Markdown style={markdownStyles as any} rules={selectableMarkdownRules}>
                {assistantBlocks.stripped}
              </Markdown>) : null}
            {assistantBlocks.handoff && (<HandoffCard toAgentId={assistantBlocks.handoff.toAgent} note={assistantBlocks.handoff.note} fromAgent={fromAgent} agents={agents} handoff={handoffRow} onOpenSession={onOpenSession}/>)}
          </>)}
