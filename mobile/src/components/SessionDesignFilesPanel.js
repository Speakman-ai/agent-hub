...
export default function SessionDesignFilesPanel({ sessionId, reloadNonce = 0 }) {
  ...
  const fileUrl = (p) => buildDesignFileUrl(base, sessionId, p);
        ...
        <View style={styles.headerActions}>
          {entry ? (
            <TouchableOpacity
              ...
              onPress={() => openFile(entry)}
              accessibilityRole="button"
              accessibilityLabel="Open design in browser"
              testID="design-open-in-web"
            >
          ...
          <TouchableOpacity
            ...
            disabled={loading}
            accessibilityRole="button"
            accessibilityLabel="Refresh design files"
            testID="design-refresh"
          >
            {loading ? (
        ...
        <ScrollView style={styles.list} nestedScrollEnabled>
          {files.map((f) => (
            <TouchableOpacity
              ...
              accessibilityRole="button"
              accessibilityLabel={`Open ${f.path} in browser`}
            >
