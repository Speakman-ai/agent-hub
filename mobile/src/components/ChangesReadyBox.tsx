...
export default function ChangesReadyBox({ sessionId, changes, onTrigger, onDismiss, isSessionProcessing = false, shipFailureAt = null, }: any) {
    ...
    useEffect(() => {
        if (!awaitingAgentTurn)
            return undefined;
        const timer = setTimeout(() => setAwaitingAgentTurn(false), 120000);
    ...
    return (<View style={styles.container}>
      <View style={styles.headerRow}>
        <View style={styles.headerLeft}>
          ...
          {!!changes?.branch && (<Text style={styles.branch} numberOfLines={1}>
              ({changes.branch})
            </Text>)}
        </View>
        {onDismiss && !isCreating ? (<TouchableOpacity onPress={() => onDismiss(sessionId)} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }} accessibilityLabel="Dismiss">
            <Text style={styles.dismiss}>✕</Text>
          </TouchableOpacity>) : null}
      </View>
      <TouchableOpacity onPress={handlePress} disabled={isCreating} style={[styles.button, isCreating && styles.buttonDisabled]} accessibilityRole="button" accessibilityLabel={label} accessibilityState={{ disabled: isCreating, busy: isCreating }}>
        {isCreating ? (<View style={styles.creatingRow}>
            <ActivityIndicator color={colors.white} size="small"/>
            <Text style={styles.buttonText}>{label}</Text>
          </View>) : (<Text style={styles.buttonText}>{label}</Text>)}
      </TouchableOpacity>
    </View>);
}
const PURPLE = '#7C3AED';
