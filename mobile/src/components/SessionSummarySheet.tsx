...
import { View, Text, TouchableOpacity, Modal, Pressable, StyleSheet, ActivityIndicator, ScrollView, Linking, Alert, } from 'react-native';
import AppIcon from './AppIcon';
import { colors } from '../theme/colors';
import { api } from '../utils/api';
import { normalizeSessionSummary, formatInjectedBytes, splitSessionRoster, } from '../utils/sessionExtras';
 ...
 */
export default function SessionSummarySheet({ visible, onClose, sessionId, sessionAgents = [], }: any) {
      ...
      <Pressable style={styles.overlay} onPress={onClose}>
        <Pressable style={styles.sheet} onPress={() => { }}>
          ...
          <ScrollView style={styles.body} contentContainerStyle={styles.bodyContent}>
            ...
            {!loading && error && (<View style={styles.errorBox}>
                <Text style={styles.errorText}>{error}</Text>
              </View>)}

            {!loading && !error && summary && (<>
                {/* Session name + engine/model */}
                {summary.sessionName ? (<Text style={styles.sessionName} numberOfLines={2}>
                    {summary.sessionName}
                  </Text>) : null}
                {(summary.engine || summary.model) && (<Text style={styles.dimText}>
                ...
                {summary.linkedCardTitle ? (<>
                    ...
                    <View style={styles.prCard}>
                      <View style={styles.prTitleRow}>
                        ...
                        {summary.linkedCardColumn ? (<View style={[styles.badge, { backgroundColor: colors.gray800 }]}>
                ...
                {summary.linkedPrUrl ? (<TouchableOpacity style={styles.prCard} onPress={() => openPr(summary.linkedPrUrl)} accessibilityRole="link" accessibilityLabel={`Open pull request ${summary.prNumber ? `#${summary.prNumber}` : ''}`}>
                    <View style={styles.prTitleRow}>
                      ...
                      {summary.prBadge && tone && (<View style={[styles.badge, { backgroundColor: tone.bg }]}>
                  ...
                  </TouchableOpacity>) : (<Text style={styles.emptyText}>No PR linked to this session.</Text>)}

                {/* Skills */}
                <Text style={styles.sectionLabel}>SKILLS USED</Text>
                {summary.skills.length === 0 ? (<Text style={styles.emptyText}>No skills loaded yet.</Text>) : (<View style={styles.chipWrap}>
                    {summary.skills.map((s: any) => (<View key={s.id} style={styles.chip}>
                        <Text style={styles.chipText}>{s.skillId}</Text>
                        {formatInjectedBytes(s.injectedBytes) ? (<Text style={styles.chipMeta}>
                ...
                {!executor && advisors.length === 0 ? (<Text style={styles.emptyText}>No agents on this session.</Text>) : (<View style={styles.agentList}>
                    {executor && (<View style={styles.agentRow}>
                        <View style={[styles.agentDot, { backgroundColor: executor.color || colors.gray500 }]}/>
                        <Text style={styles.agentName} numberOfLines={1}>
                          {executor.name || executor.id}
                        </Text>
                        <Text style={styles.agentRole}>executor</Text>
                      </View>)}
                    {advisors.map((a: any) => (<View key={a.id} style={styles.agentRow}>
                        <View style={[styles.agentDot, { backgroundColor: a.color || colors.gray500 }]}/>
                        <Text style={styles.agentName} numberOfLines={1}>
                          {a.name || a.id}
                        </Text>
