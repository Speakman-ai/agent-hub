...
function AskUserQuestion({ askId, questions, onSubmit, submitted }: any) {
    ...
    // what the user typed.
    const [state, setState] = useState<any>(() => questions.map((q: any) => ({
    ...
    const anyPreview = useMemo<any>(() => questions.map((q: any) => q.options.some((o: any) => !!o.preview)), [questions]);
    ...
    function toggleMulti(i: any, label: any) {
        setState((prev: any) => prev.map((s: any, idx: any) => {
            if (idx !== i)
                return s;
    ...
    return (<View style={styles.container} testID={`ask-${askId}`}>
      ...
      <View style={styles.body}>
        {questions.map((q: any, i: any) => (<QuestionCard key={`${askId}-${i}`} question={q} state={state[i]} hasPreview={anyPreview[i]} disabled={submitted} errored={error?.questionIdx === i} onSelectSingle={(label: any) => updateQuestion(i, { selected: label })} onToggleMulti={(label: any) => toggleMulti(i, label)} onOtherText={(txt: any) => updateQuestion(i, { otherText: txt })} onNotes={(txt: any) => updateQuestion(i, { notes: txt })} onFocusOption={(idx: any) => updateQuestion(i, { focusedIdx: idx })}/>))}
        ...
        <View style={styles.submitRow}>
          <TouchableOpacity onPress={handleSubmit} disabled={submitted} style={[styles.submitBtn, submitted && styles.submitBtnDisabled]} accessibilityRole="button" accessibilityLabel={submitted ? 'Answers submitted' : 'Submit answers'}>
...
function QuestionCard({ question, state, hasPreview, disabled, errored, onSelectSingle, onToggleMulti, onOtherText, onNotes, onFocusOption, }: any) {
    const { selected, otherText, notes, focusedIdx } = state;
    const isMulti = question.multiSelect;
    const showPreview = hasPreview && !isMulti;
    const focusedOption = showPreview && focusedIdx < question.options.length ? question.options[focusedIdx] : null;
    return (<View style={[styles.questionCard, errored && styles.questionCardErrored]}>
      ...
      <View style={styles.optionList}>
        {question.options.map((opt: any, idx: any) => {
            ...
            return (<OptionRow key={opt.label} option={opt} multi={isMulti} checked={isChecked} focused={showPreview && focusedIdx === idx} disabled={disabled} onPress={() => {
                    if (isMulti)
        ...
        <OtherRow multi={isMulti} checked={isMulti ? selected.has(OTHER_SENTINEL) : selected === OTHER_SENTINEL} disabled={disabled} otherText={otherText} onSelect={() => {
            if (isMulti)
                onToggleMulti(OTHER_SENTINEL);
            else
                onSelectSingle(OTHER_SENTINEL);
      ...
      <View style={styles.notesSection}>
        ...
        <TextInput value={notes} onChangeText={onNotes} editable={!disabled} multiline placeholder="Add context or reasoning…" placeholderTextColor={colors.gray600} style={[styles.notesInput, disabled && styles.disabledFaded]}/>
      </View>
    </View>);
}
function OptionRow({ option, multi, checked, focused, disabled, onPress }: any) {
    return (<TouchableOpacity onPress={onPress} disabled={disabled} activeOpacity={0.7} accessibilityRole={multi ? 'checkbox' : 'radio'} accessibilityState={{ checked, disabled: !!disabled }} accessibilityLabel={option.label} style={[
            styles.optionRow,
      ...
      <View style={[styles.indicator, multi ? styles.indicatorSquare : styles.indicatorRound]}>
        {checked && (multi ? (<AppIcon name="checkmark" size={12} color={colors.white}/>) : (<View style={styles.radioDot}/>))}
      </View>
...
function OtherRow({ multi, checked, disabled, otherText, onSelect, onTextChange }: any) {
    return (<View style={[styles.otherRow, checked && styles.otherRowActive, disabled && styles.disabledFaded]}>
      <TouchableOpacity onPress={onSelect} disabled={disabled} activeOpacity={0.7} accessibilityRole={multi ? 'checkbox' : 'radio'} accessibilityState={{ checked, disabled: !!disabled }} accessibilityLabel="Other" style={[styles.indicator, multi ? styles.indicatorSquare : styles.indicatorRound]}>
        {checked && (multi ? (<AppIcon name="checkmark" size={12} color={colors.white}/>) : (<View style={styles.radioDot}/>))}
      </TouchableOpacity>
      <View style={styles.otherContent}>
        <Text style={styles.otherLabel}>Other…</Text>
        <TextInput value={otherText} onChangeText={(txt: any) => {
            onTextChange(txt);
            if (!checked && txt.length > 0)
                onSelect();
        }} editable={!disabled} placeholder="Type your own answer" placeholderTextColor={colors.gray600} style={styles.otherInput}/>
      </View>
    </View>);
}
function PreviewPane({ preview }: any) {
    if (!preview) {
        return (<View style={styles.previewEmpty}>
        <Text style={styles.previewEmptyText}>Tap an option to preview</Text>
      </View>);
    }
    return (<ScrollView style={styles.previewScroll} contentContainerStyle={styles.previewContent} nestedScrollEnabled>
