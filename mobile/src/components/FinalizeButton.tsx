...
import { isTerminalStatus, isFinalizeBlocked, describeRunPhase, shouldPollFinalizeFallback, } from '../utils/finalizeRun';
import { hasCommittableChangesFromReady } from '../utils/changesReady';
import { colors } from '../theme/colors';
const WORKTREE_POLL_MS = 15000;
/**
 ...
 */
export default function FinalizeButton({ projectId, cardId, sessionId, prefetchedRun, branchLabel, pendingChanges = null, onError, variant = 'default', }: any) {
    const { startFinalizeRun, startFinalizeRunForSession, cancelFinalizeRun, lastFinalizeRunEvent, connected, } = useApp();
    ...
    useEffect(() => {
        if (!lastFinalizeRunEvent || !sessionId)
            return;
        if (lastFinalizeRunEvent.sessionId !== sessionId)
            return;
        setRun((prev: any) => ({
            ...(prev || {}),
    ...
    const hasCommittableChanges = worktreeCommittable || hasCommittableChangesFromReady(pendingChanges);
    ...
    const handleStart = useCallback(async () => {
        if (!projectId || !sessionId)
            return;
        if (blocked || optimisticPending || !canShip)
            return;
    ...
    if (showInFlight) {
        ...
        {!compact && (<View style={styles.headerRow}>
            ...
            {!!branchLabel && (<Text style={styles.branch} numberOfLines={1}>
                ({branchLabel})
              </Text>)}
          </View>)}
        <View style={styles.inFlightRow}>
          <TouchableOpacity disabled style={[styles.button, styles.buttonDisabled, compact && styles.buttonCompact]} accessibilityRole="button" accessibilityLabel={label} accessibilityState={{ disabled: true, busy: true }} testID="finalize-code-changes-button">
            <View style={styles.creatingRow}>
              <ActivityIndicator color={colors.white} size="small"/>
              <Text style={[styles.buttonText, compact && styles.buttonTextCompact]}>{label}</Text>
            </View>
          </TouchableOpacity>
          {run?.id ? (<TouchableOpacity onPress={handleCancel} disabled={cancelPending} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }} accessibilityRole="button" accessibilityLabel="Cancel runner" testID="finalize-code-changes-cancel" style={styles.cancelButton}>
              {cancelPending ? (<ActivityIndicator color={colors.gray300} size="small"/>) : (<Text style={styles.cancelText}>✕</Text>)}
    ...
    return (<View style={[styles.container, compact && styles.containerCompact]}>
      ...
      <TouchableOpacity onPress={handleStart} disabled={idleDisabled} style={[
            styles.button,
            idleDisabled && styles.buttonDisabled,
            compact && styles.buttonCompact,
        ]} accessibilityRole="button" accessibilityLabel={idleLabel} accessibilityHint={canShip
            ? 'Runs rebase, review, tests, and opens a PR when green.'
            : 'No committable changes yet.'} accessibilityState={{ disabled: idleDisabled }} testID="finalize-code-changes-button">
        <Text style={[styles.buttonText, compact && styles.buttonTextCompact]}>{idleLabel}</Text>
      </TouchableOpacity>
      {!compact && (<Text style={styles.v0Caveat}>
