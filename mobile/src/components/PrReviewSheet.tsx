...
const EVENT_ACCENTS: Record<string, any> = {
 ...
 */
export default function PrReviewSheet({ visible, prNumber, onClose, onSubmit }: any) {
    ...
    return (<PrActionSheet visible={visible} title={`Review PR #${prNumber}`} submitLabel="Submit review" onSubmit={submit} onClose={onClose} busy={busy} submitDisabled={needsBody} error={error}>
      <View style={styles.verdictRow}>
        {REVIEW_EVENTS.map((opt: any) => {
            const active = event === opt.event;
            const accent = EVENT_ACCENTS[opt.event] || colors.gray400;
            return (<TouchableOpacity key={opt.event} style={[styles.verdictButton, active && { borderColor: accent }]} onPress={() => {
                    setEvent(opt.event);
                    setError(null);
                }} disabled={busy} accessibilityState={{ selected: active }}>
      ...
      <TextInput style={styles.bodyInput} value={body} onChangeText={(t: any) => {
            ...
            if (error)
                setError(null);
        }} placeholder="Review notes (required for comments, optional otherwise)…" placeholderTextColor={colors.gray500} multiline editable={!busy}/>
    </PrActionSheet>);
}
