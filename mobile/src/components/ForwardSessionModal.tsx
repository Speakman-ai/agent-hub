...
import { View, Text, TextInput, TouchableOpacity, Modal, Pressable, StyleSheet, ActivityIndicator, ScrollView, Switch, } from 'react-native';
...
export default function ForwardSessionModal({ visible, sourceAgent, agents, sessionId, onClose, onForward, onForwarded, onError, }: any) {
    const [selectedAgentId, setSelectedAgentId] = useState<any>(null);
    const [prompt, setPrompt] = useState('');
    const [autoStart, setAutoStart] = useState(false);
    const [submitting, setSubmitting] = useState(false);
    const [error, setError] = useState<any>(null);
    const candidates = useMemo<any>(() => filterForwardTargets(agents, sourceAgent), [agents, sourceAgent]);
    const reset = () => {
    ...
    const handleSubmit = async () => {
        if (!selectedAgentId || submitting || !sessionId)
            return;
        setSubmitting(true);
        setError(null);
        try {
    ...
    return (<Modal visible={visible} transparent animationType="fade" onRequestClose={handleClose}>
      <Pressable style={styles.overlay} onPress={handleClose}>
        <Pressable style={styles.sheet} onPress={() => { }}>
          <View style={styles.header}>
            <View style={styles.headerLeft}>
              ...
              {sourceAgent?.name ? (<Text style={styles.headerSubtitle} numberOfLines={1}>
          ...
          {candidates.length === 0 ? (<View style={styles.empty}>
              ...
              <ScrollView style={styles.agentList} keyboardShouldPersistTaps="handled">
                ...
                return (<TouchableOpacity key={agent.id} style={[styles.agentRow, selected && styles.agentRowSelected]} onPress={() => setSelectedAgentId(agent.id)}>
                      ...
                      <View style={styles.agentText}>
                        <View style={styles.agentNameRow}>
                          ...
                          {isSelf && (<View style={styles.selfBadge}>
              ...
              <View style={styles.controls}>
                ...
                <TextInput value={prompt} onChangeText={setPrompt} multiline numberOfLines={2} placeholder="What should the target agent do with this context?" placeholderTextColor={colors.gray500} style={styles.textarea}/>
                <View style={styles.autoStartRow}>
                  <Text style={styles.autoStartLabel}>Auto-start target agent</Text>
                  <Switch value={autoStart} onValueChange={setAutoStart} trackColor={{ true: colors.blue400, false: colors.gray700 }}/>
                </View>
        ...
        ]} onPress={handleSubmit} disabled={!selectedAgentId || submitting || candidates.length === 0}>
              {submitting ? (<ActivityIndicator size="small" color={colors.white}/>) : (<AppIcon name="send" size={14} color={colors.white}/>)}
