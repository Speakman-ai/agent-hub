...
import { groupThreadsByFile, emptyReviewerThreads, normalizeReviewerThreads, isFreshGeneration, } from '../utils/finalizeView';
import { isTerminalStatus } from '../utils/finalizeRun';
const MONO = Platform.select({ ios: 'Menlo', default: 'monospace' });
const POLL_MS = 4000;
const VERDICT_META: Record<string, any> = {
...
export default function ReviewerThreadsCard({ projectId, runId, status }: any) {
    ...
    const fetchThreads = useCallback(async () => {
        ...
        try {
            ...
            if (!isFreshGeneration(myGen, genRef.current))
                return;
    ...
    if (!runId || (!threads.length && !verdict))
        return null;
    if (!loaded && !threads.length && !verdict)
        return null;
    const groups = groupThreadsByFile(threads);
    const vmeta = verdict ? VERDICT_META[verdict] : null;
    return (<View style={styles.card} testID="reviewer-threads-card">
      <View style={styles.header}>
        <AppIcon name="chatbubbles-outline" size={14} color={colors.gray300}/>
        <Text style={styles.title}>Review</Text>
        {threads.length > 0 && (<Text style={styles.count}>
            {threads.length} finding{threads.length === 1 ? '' : 's'}
          </Text>)}
        {vmeta && (<View style={[styles.verdictPill, { borderColor: vmeta.color }]}>
      ...
      {groups.map((g: any) => {
            ...
            return (<View key={g.file} style={styles.fileGroup}>
            <TouchableOpacity style={styles.fileHeader} onPress={() => setCollapsed((p: any) => ({ ...p, [g.file]: !p[g.file] }))}>
              <AppIcon name={isCollapsed ? 'chevron-forward' : 'chevron-down'} size={14} color={colors.gray500}/>
            ...
            {!isCollapsed &&
                    g.items.map((t: any, i: any) => (<View key={t.id || `${g.file}-${i}`} style={styles.finding}>
                  {(t.line_start || t.line_end) && (<Text style={styles.lineRef}>
