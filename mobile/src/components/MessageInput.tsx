...
import { View, Text, TextInput, TouchableOpacity, StyleSheet, Platform, Image, ScrollView, Alert, ActionSheetIOS, ActivityIndicator, } from 'react-native';
...
export default function MessageInput({ onSend, onCancel, disabled, isProcessing, agentColor, skills, queueLength, askMode, consultMode = askMode, consultHint = null, readOnly, toolbarStart, }: any) {
    ...
    const filteredSkills = (skills || []).filter((s: any) => slashQuery === null
        ? false
    ...
    const insertSkill = useCallback((skillId: any) => {
        if (slashStart === null)
            return;
    ...
    const handleChangeText = useCallback((val: any) => {
        setValue(val);
    ...
    return (<View style={styles.container}>
      ...
      {slashQuery !== null && filteredSkills.length > 0 && (<View style={styles.slashPopup}>
          ...
          <ScrollView style={styles.slashList} keyboardShouldPersistTaps="always">
            {filteredSkills.map((skill: any) => (<TouchableOpacity key={skill.id} onPress={() => insertSkill(skill.id)} style={styles.slashItem} activeOpacity={0.7}>
                ...
                <View style={styles.slashItemContent}>
                  ...
                  {skill.description ? (<Text style={styles.slashItemDesc} numberOfLines={1}>
                      {skill.description}
                    </Text>) : null}
                </View>
              </TouchableOpacity>))}
      ...
      {/* Attachment previews */}
      {images.length > 0 && (<ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.imagePreviewRow} contentContainerStyle={styles.imagePreviewContent}>
          {images.map((item: any) => (<View key={item.id} style={styles.imagePreviewItem}>
              {item.kind === 'image' ? (<Image source={{ uri: item.uri }} style={styles.imagePreview}/>) : item.kind === 'video' ? (<View style={[styles.imagePreview, styles.videoPreview]}>
                  <AppIcon name="videocam" size={22} color={colors.gray300}/>
                  <Text style={styles.mediaBadge}>VIDEO</Text>
                </View>) : (<View style={[styles.imagePreview, styles.filePreview]}>
                  <AppIcon name="document-outline" size={20} color={colors.gray300}/>
                  <Text style={styles.fileName} numberOfLines={1}>
                    {item.name}
                  </Text>
                </View>)}
              <TouchableOpacity style={styles.removeImageBtn} onPress={() => removeImage(item.id)} accessibilityLabel="Remove attachment" accessibilityRole="button">
      ...
      <View style={styles.inner}>
        ...
        <TouchableOpacity style={styles.imageButton} onPress={openAttachMenu} disabled={disabled && !isProcessing} activeOpacity={0.7} accessibilityLabel="Add attachment" accessibilityRole="button">
          <AppIcon name="attach" size={22} color={disabled && !isProcessing ? colors.gray600 : colors.gray400}/>
        </TouchableOpacity>

        {/* Voice input — mirrors web MessageInput mic (attach | mic | composer | send). */}
        <TouchableOpacity style={styles.micButton} onPress={handleMicClick} disabled={micDisabled} activeOpacity={0.7} accessibilityRole="button" accessibilityLabel={isTranscribing
            ? 'Transcribing voice input'
            : isRecording
                ? 'Stop recording'
                : 'Voice input'} accessibilityState={{
            disabled: !!micDisabled,
            selected: !!isRecording,
        }}>
          {isTranscribing ? (<ActivityIndicator size={20} color={colors.gray500}/>) : (<AppIcon name={isRecording ? 'stop' : 'mic-outline'} size={22} color={micDisabled
                ? colors.gray600
                : isRecording
                    ? colors.red400
                    : colors.gray400}/>)}
        ...
        {isProcessing ? (<TouchableOpacity style={styles.cancelButton} onPress={onCancel} activeOpacity={0.7}>
            <AppIcon name="close-circle" size={22} color={colors.white}/>
                ...
                styles.sendButton,
                {
                    backgroundColor: disabled || (!value.trim() && images.length === 0)
                        ? colors.gray600
                        : agentColor || '#4F46E5',
                },
            ]} onPress={handleSubmit} disabled={disabled || (!value.trim() && images.length === 0)} activeOpacity={0.7}>
            <AppIcon name="send" size={18} color={colors.white}/>
            {isProcessing && queueLength > 0 && (<View style={styles.queueBadge}>
