...
function HandoffCard({ note, toAgentId, fromAgent, agents, handoff, onOpenSession }: any) {
    ...
    return (<View style={styles.card}>
      ...
      <View style={styles.body}>
        ...
        <Markdown style={markdownStyles as any}>{note}</Markdown>
      </View>
      <View style={styles.footer}>
        <Text style={styles.footerNote}>
          Ownership transferred — the new session continues with the transcript above as context.
        </Text>
        {canOpen && (<TouchableOpacity style={styles.openBtn} onPress={() => onOpenSession(resolvedToAgentId, toSessionId)} accessibilityRole="button" accessibilityLabel={`Open the ${toName} session started by this handoff`}>
            <Text style={styles.openBtnText}>↗ Open session</Text>
          </TouchableOpacity>)}
        {!canOpen && status === 'pending' && (<View style={styles.pendingPill}>
            <View style={styles.pendingDot}/>
            <Text style={styles.pendingText}>Delivering…</Text>
          </View>)}
        {status === 'failed' && (<View style={styles.failedPill}>
...
function Chip({ name, color, highlight }: any) {
    return (<View style={[styles.chip, highlight && styles.chipHighlight]}>
      <View style={[styles.chipDot, { backgroundColor: color }]}/>
      <Text style={[styles.chipText, highlight && styles.chipTextHighlight]} numberOfLines={1}>
        {name}
      </Text>
    </View>);
}
function resolveAgent(agentId: any, agents: any) {
    if (!agentId || !Array.isArray(agents))
        return null;
