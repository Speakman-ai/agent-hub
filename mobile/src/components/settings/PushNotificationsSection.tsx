...
import { View, Text, TouchableOpacity, Switch, StyleSheet, ActivityIndicator, Alert, } from 'react-native';
...
export default function PushNotificationsSection() {
    ...
    const [enabledEvents, setEnabledEvents] = useState<any>(null);
    ...
    const handleToggle = async (key: any, value: any) => {
        if (!pushToken)
            return;
        const current = enabledEvents || PUSH_EVENT_OPTIONS.map((o: any) => o.key);
        const next = value ? [...new Set([...current, key])] : current.filter((k: any) => k !== key);
    ...
    const handleEnableAll = async () => {
        if (!pushToken)
            return;
        const all = PUSH_EVENT_OPTIONS.map((o: any) => o.key);
    ...
    return (<View style={styles.container}>
      ...
      {!pushToken ? (<Text style={styles.muted}>
          Push notifications require a physical device with notification permission granted.
        </Text>) : loading ? (<ActivityIndicator color={colors.gray400} style={{ marginVertical: 16 }}/>) : (<>
          ...
          {PUSH_EVENT_OPTIONS.map((opt: any) => (<View key={opt.key} style={styles.eventRow}>
              <View style={styles.eventInfo}>
                <Text style={styles.eventLabel}>{opt.label}</Text>
                <Text style={styles.eventDesc}>{opt.desc}</Text>
              </View>
              <Switch value={isEventEnabled(opt.key)} onValueChange={(v: any) => handleToggle(opt.key, v)} disabled={saving} trackColor={{ false: colors.gray700, true: colors.blue600 }} thumbColor={colors.white}/>
            </View>))}
        </>)}
    </View>);
}
