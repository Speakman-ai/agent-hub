...
export default function GeneralSettingsSection() {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (form.defaultCwd != null)
                patch.defaultCwd = form.defaultCwd;
    ...
    return (<View style={styles.container}>
      ...
      {BIN_FIELDS.map((f: any) => (<View key={f.key} style={styles.field}>
          <Text style={styles.label}>{f.label}</Text>
          <TextInput style={styles.input} value={form[f.key] || ''} onChangeText={(v: any) => setForm((prev: any) => ({ ...prev, [f.key]: v }))} placeholder="/usr/local/bin/…" placeholderTextColor={colors.gray600} autoCapitalize="none" autoCorrect={false}/>
        </View>))}
      <View style={styles.field}>
        <Text style={styles.label}>Default CWD</Text>
        <TextInput style={styles.input} value={form.defaultCwd || ''} onChangeText={(v: any) => setForm((prev: any) => ({ ...prev, defaultCwd: v }))} placeholder="/path/to/projects" placeholderTextColor={colors.gray600} autoCapitalize="none"/>
      </View>
