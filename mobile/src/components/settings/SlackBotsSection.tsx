...
import { validateSlackBotForm, buildSlackBotPayload, describeSlackTestResult, } from '../../utils/settingsSlackBots';
const EMPTY_FORM: Record<string, any> = { name: '', agent_id: '', bot_token: '', app_token: '' };
function BotForm({ form, setForm, agents, isNew }: any) {
    return (<>
      <Text style={styles.fieldLabel}>Name</Text>
      <TextInput value={form.name} onChangeText={(v: any) => setForm({ ...form, name: v })} placeholder="e.g. Support Bot" placeholderTextColor={colors.gray500} style={styles.formInput}/>
      <Text style={styles.fieldLabel}>Agent</Text>
      <View style={styles.chipRow}>
        {agents.map((a: any) => {
            const active = form.agent_id === a.id;
            return (<TouchableOpacity key={a.id} style={[styles.chip, active && styles.chipActive]} onPress={() => setForm({ ...form, agent_id: a.id })}>
      ...
      <TextInput value={form.bot_token} onChangeText={(v: any) => setForm({ ...form, bot_token: v })} placeholder="xoxb-..." placeholderTextColor={colors.gray500} style={styles.formInput} autoCapitalize="none" autoCorrect={false} secureTextEntry/>
      <Text style={styles.fieldLabel}>
        App token (xapp-…){isNew ? '' : ' — leave blank to keep current'}
      </Text>
      <TextInput value={form.app_token} onChangeText={(v: any) => setForm({ ...form, app_token: v })} placeholder="xapp-..." placeholderTextColor={colors.gray500} style={styles.formInput} autoCapitalize="none" autoCorrect={false} secureTextEntry/>
    </>);
}
export default function SlackBotsSection() {
    ...
    const handleToggle = async (bot: any) => {
        try {
            ...
            setBots((prev: any) => prev.map((b: any) => (b.id === bot.id ? { ...b, enabled: result.enabled ? 1 : 0 } : b)));
        }
        catch (err: any) {
    ...
    const handleTest = async (bot: any) => {
        setTesting((prev: any) => ({ ...prev, [bot.id]: true }));
        try {
            ...
            setTesting((prev: any) => ({ ...prev, [bot.id]: false }));
        }
    };
    ...
    return (<View style={{ marginBottom: 24 }}>
      ...
      {showNew && (<View style={styles.formCard}>
          ...
          <TouchableOpacity style={[styles.primaryBtn, busy && { opacity: 0.5 }]} onPress={handleCreate} disabled={busy}>
            <Text style={styles.primaryBtnText}>{busy ? 'Creating…' : 'Create Bot'}</Text>
          </TouchableOpacity>
        </View>)}

      {bots.length === 0 && !loadError ? (<Text style={styles.emptyText}>No managed Slack bots yet.</Text>) : (bots.map((bot: any) => {
            const isExpanded = expanded === bot.id;
            return (<View key={bot.id} style={styles.card}>
              <TouchableOpacity style={styles.cardRow} onPress={() => {
                ...
                <View style={styles.actionButtons}>
                  <TouchableOpacity style={styles.smallButton} onPress={() => handleTest(bot)} disabled={!!testing[bot.id]}>
                    <Text style={styles.smallButtonText}>{testing[bot.id] ? '…' : 'Test'}</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.smallButton, bot.enabled ? styles.buttonOn : null]} onPress={() => handleToggle(bot)}>
                    <Text style={[styles.smallButtonText, bot.enabled ? styles.buttonOnText : null]}>
                      {bot.enabled ? 'ON' : 'OFF'}
                    </Text>
                  </TouchableOpacity>
                </View>
              ...
              {isExpanded && (<View style={styles.expandedSection}>
                  ...
                  <View style={styles.actionRow}>
                    ...
                    <TouchableOpacity style={[styles.primaryBtn, busy && { opacity: 0.5 }]} onPress={() => handleSave(bot)} disabled={busy}>
