...
function ProviderCard({ provider }: any) {
    ...
    return (<View style={styles.card}>
      <View style={styles.cardHeaderRow}>
        ...
        {!loading && !loadError && (<Text style={[styles.statusText, { color: configured ? colors.emerald400 : colors.gray500 }]}>
            {configured ? 'Configured' : 'Not configured'}
          </Text>)}
      </View>

      {loading ? (<ActivityIndicator size="small" color={colors.gray500} style={{ marginVertical: 12 }}/>) : loadError ? (<View style={styles.errorBox}>
          ...
          <View style={styles.inputRow}>
            <TextInput value={keyInput} onChangeText={setKeyInput} placeholder={provider.placeholder} placeholderTextColor={colors.gray500} style={[styles.formInput, { flex: 1 }]} secureTextEntry={!showKey} autoCapitalize="none" autoCorrect={false}/>
          ...
          <View style={styles.actionRow}>
            <TouchableOpacity style={[styles.primaryBtn, (!keyInput.trim() || saving) && { opacity: 0.4 }]} onPress={() => save(keyInput)} disabled={!keyInput.trim() || saving}>
              <Text style={styles.primaryBtnText}>{saving ? 'Saving…' : 'Save Key'}</Text>
            </TouchableOpacity>
            {configured && (<TouchableOpacity style={[styles.dangerBtn, saving && { opacity: 0.4 }]} onPress={handleClear} disabled={saving}>
...
function GithubCard() {
    ...
    return (<View style={styles.card}>
      <View style={styles.cardHeaderRow}>
        ...
        {!loading && !loadError && (<Text style={[styles.statusText, { color: connected ? colors.emerald400 : colors.gray500 }]}>
...
export default function MyCliKeysSection() {
    return (<View>
      ...
      {CLI_KEY_PROVIDERS.map((provider: any) => (<ProviderCard key={provider.id} provider={provider}/>))}
      <GithubCard />
    </View>);
}
