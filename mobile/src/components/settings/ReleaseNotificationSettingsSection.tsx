...
export default function ReleaseNotificationSettingsSection({ projectId }: any) {
  ...
  const recipientValidationError = useMemo(() => {
    ...
    if (
      recipients?.some(
        (recipient: any) => recipient.email.trim().toLowerCase() === normalizedRecipientEmail,
      )
    )
      ...
      try {
        ...
        setSettings((current: any) =>
          current
            ? {
                ...current,
                releaseDigestRecipients: (current.releaseDigestRecipients || []).map((item: any) =>
  ...
  return (
    <View style={styles.section}>
      ...
      <TextInput
        ...
        editable={!saving && !!projectId}
        maxLength={maxLength + 1}
        placeholder="Release digest guidance"
        ...
        <View style={styles.buttons}>
          <TouchableOpacity
            style={[
              ...
              (saving || loading || settings?.isDefault) && styles.buttonDisabled,
            ]}
            onPress={handleReset}
          ...
          <TouchableOpacity
            style={[
              ...
              (saving || loading || !dirty || !!validationError) && styles.buttonDisabled,
            ]}
            onPress={handleSave}
      ...
      {recipients && (
        <View style={styles.recipientsSection}>
          ...
          <TouchableOpacity
            style={[
              styles.primaryButton,
              (!recipientEmail.trim() || !!recipientValidationError || recipientSaving) &&
                styles.buttonDisabled,
            ]}
            onPress={handleAddRecipient}
            disabled={!recipientEmail.trim() || !!recipientValidationError || recipientSaving}
          >
          ...
          <View style={styles.recipientList}>
            {recipients.length === 0 ? (
              <Text style={styles.emptyText}>No release digest recipients.</Text>
            ) : (
                ...
                <View key={recipient.id} style={styles.recipientRow}>
                  <View style={styles.recipientInfo}>
                    ...
                    {recipient.displayLabel ? (
                      <Text style={styles.recipientLabel}>{recipient.displayLabel}</Text>
                    ) : null}
                  </View>
                  <View style={styles.recipientActions}>
                    <TouchableOpacity
                      style={styles.secondaryButton}
                      onPress={() => handleToggleRecipient(recipient)}
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() => handleRemoveRecipient(recipient)}
                      disabled={recipientSaving}
                    >
