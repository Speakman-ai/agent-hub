  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.hint}>
        Host-wide job queue drained by crons and autonomous tasks. Retry a dead-lettered job or
        delete a stale row.
      </Text>
          ...
          {(['total', 'queued', 'running', 'done', 'dead_letter'] as const).map((k) => (
            <View key={k} style={styles.countCard}>
              ...
              <Text
                style={[
                  styles.countValue,
                  k === 'dead_letter' && counts.dead_letter > 0 ? { color: colors.red400 } : null,
                ]}
              >
                {counts[k]}
              </Text>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.chipRow}>
        {STATUS_FILTERS.map((f) => (
          <TouchableOpacity
            ...
            testID={`jobs-filter-${f.value || 'all'}`}
          >
      ...
      {loading && !data ? (
        <ActivityIndicator color={colors.gray400} style={{ marginTop: 12 }} />
      ) : jobs.length === 0 ? (
        <Text style={styles.muted}>No jobs match this filter.</Text>
      ) : (
        jobs.map((job) => (
          <View key={job.id} style={styles.card}>
            ...
            <View style={styles.actionRow}>
              {jobIsRetryable(job.status) ? (
                <TouchableOpacity
                  style={[styles.actionBtn, styles.retryBtn]}
                  disabled={busyId === job.id}
                  onPress={() => handleRetry(job.id)}
                  testID={`jobs-retry-${job.id}`}
                >
