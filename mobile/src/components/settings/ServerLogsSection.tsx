      ...
      try {
        ...
        if (data?.type === 'server-log' && data.entry) {
          setLines((prev: any) => [
            ...
            formatServerLogLine(data.entry),
          ]);
        }
      } catch {
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.logBox} nestedScrollEnabled>
        {lines.length === 0 ? (
          <Text style={styles.muted}>Waiting for log lines…</Text>
        ) : (
          orderServerLogsNewestFirst(lines).map((line: any, i: any) => (
            <Text key={`${i}-${line.slice(0, 20)}`} style={styles.logLine}>
              {line}
            </Text>
          ))
        )}
