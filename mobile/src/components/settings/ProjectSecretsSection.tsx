...
import { validateSecretKey, buildUpsertSecretsPayload, displaySecretValue, describeSecretsPermissionError, } from '../../utils/settingsSecrets';
const EMPTY_FORM: Record<string, any> = { key: '', value: '', kind: 'secret' };
/**
 ...
 *   shows the picker.
 */
export default function ProjectSecretsSection({ projectId: lockedProjectId = null }: any = {}) {
    ...
    useEffect(() => {
        ...
        api
            ...
            setProjects(safe);
            if (safe.length > 0)
                setProjectId(safe[0].id);
        })
            .catch((err: any) => setLoadError(err?.message || 'Failed to load projects.'))
            .finally(() => setLoadingProjects(false));
    }, [lockedProjectId]);
    const loadSecrets = useCallback(async (pid: any) => {
        if (!pid)
            return;
        ...
        try {
            ...
            setLoadError(describeSecretsPermissionError(err, 'read') || err?.message || 'Failed to load secrets.');
        }
        finally {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            Alert.alert('Save failed', describeSecretsPermissionError(err, 'write') || err?.message || 'Could not save secret.');
        }
        finally {
            setSaving(false);
        }
            ...
            { text: 'Cancel', style: 'cancel' },
            {
                ...
                onPress: async () => {
                    try {
                        ...
                        setSecrets((prev: any) => prev.filter((s: any) => s.key !== row.key));
                    }
                    catch (err: any) {
                        Alert.alert('Delete failed', describeSecretsPermissionError(err, 'write') ||
                            err?.message ||
    ...
    return (<View>
      ...
      {!lockedProjectId &&
            (projects.length === 0 ? (<Text style={styles.emptyText}>No projects configured.</Text>) : (<View style={styles.chipRow}>
            {projects.map((p: any) => {
                    const active = projectId === p.id;
                    return (<TouchableOpacity key={p.id} style={[styles.chip, active && styles.chipActive]} onPress={() => setProjectId(p.id)}>
      ...
      {loadingSecrets && (<ActivityIndicator size="small" color={colors.gray500} style={{ marginVertical: 20 }}/>)}

      {loadError && !loadingSecrets && (<View style={styles.errorBox}>
      ...
      {!loadingSecrets && !loadError && projectId && (<>
          {secrets.length === 0 ? (<Text style={styles.emptyText}>No secrets for this project yet.</Text>) : (secrets.map((row: any) => (<View key={row.key} style={styles.secretRow}>
                ...
                <View style={[styles.kindBadge, row.kind === 'plain' && styles.kindBadgePlain]}>
                  <Text style={[styles.kindBadgeText, row.kind === 'plain' && styles.kindBadgeTextPlain]}>
                    {row.kind}
                  </Text>
                </View>
                <TouchableOpacity style={styles.smallButton} onPress={() => setForm({ key: row.key, value: '', kind: row.kind })} accessibilityLabel={`Replace ${row.key}`}>
                  <Text style={styles.smallButtonText}>✎</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.smallButton} onPress={() => handleDelete(row)} accessibilityLabel={`Delete ${row.key}`}>
          ...
          <View style={styles.formCard}>
            ...
            <TextInput value={form.key} onChangeText={(v: any) => setForm({ ...form, key: v })} placeholder="e.g. DATABASE_URL" placeholderTextColor={colors.gray500} style={styles.formInput} autoCapitalize="characters" autoCorrect={false}/>
            <Text style={styles.fieldLabel}>Value (write-only)</Text>
            <TextInput value={form.value} onChangeText={(v: any) => setForm({ ...form, value: v })} placeholder="Value" placeholderTextColor={colors.gray500} style={styles.formInput} autoCapitalize="none" autoCorrect={false} secureTextEntry={form.kind === 'secret'}/>
            <Text style={styles.fieldLabel}>Kind</Text>
            <View style={styles.chipRow}>
              {['secret', 'plain'].map((kind: any) => {
                const active = form.kind === kind;
                return (<TouchableOpacity key={kind} style={[styles.chip, active && styles.chipActive]} onPress={() => setForm({ ...form, kind })}>
