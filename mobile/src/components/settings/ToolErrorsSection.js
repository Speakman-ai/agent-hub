...
export default function ToolErrorsSection() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((data) => setErrors(Array.isArray(data) ? data : data?.errors || []))
      .catch(() => setErrors([]))
  ...
  return (
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <Text style={styles.muted}>No recent tool errors.</Text>
      ) : (
        errors.map((err) => (
          <View key={err.id || `${err.timestamp}-${err.tool}`} style={styles.card}>
