      ...
      {models.length > 0 && (
        <>
          ...
          <ChipRow
            ...
            onSelect={onModel}
            labelFor={(m: any) => m.replace(/^claude-/, '').replace(/^gpt-/, '')}
          />
        </>
      )}
    </>
...
export default function AgentsSection({ projectId: filterProjectId, hideBulk = false }: any = {}) {
  ...
  const loadOverrides = useCallback(async () => {
    try {
      ...
      for (const [id, entry] of Object.entries(raw)) {
        ...
        if (row && typeof row.engine === 'string') flat[id] = row.engine;
      }
  ...
  const saveModelOverride = async (agentId: any, model: any) => {
    setModelOverrides((prev: any) => {
      ...
      else delete (next as any)[agentId];
      return next;
    });
  ...
  const saveEngineOverride = async (
    agentId: any,
    engine: any,
    sharedEngine: any = 'claude-code',
  ) => {
        ...
        <View style={styles.formCard}>
          ...
          {!filterProjectId && (
            <>
              ...
              <ChipRow
                options={projects.map((p: any) => p.id)}
                selected={newForm.projectId}
                onSelect={(projectId: any) => setNewForm({ ...newForm, projectId })}
                labelFor={(id: any) => projects.find((p: any) => p.id === id)?.name || id}
              />
            </>
          )}
          <SharedEnginePicker
          ...
          <DevToggle
            value={newForm.isDev}
            onChange={(isDev: any) => setNewForm({ ...newForm, isDev })}
          />
          <TouchableOpacity
        ...
        <View key={group.projectId || 'other'} style={styles.projectGroup}>
          {!filterProjectId && (
            <View style={styles.projectHeader}>
              <View
                style={[styles.projectDot, { backgroundColor: group.color || colors.gray600 }]}
              />
              ...
              return (
                <View key={agent.id} style={styles.card}>
                  <TouchableOpacity style={styles.cardRow} onPress={() => handleExpand(agent)}>
                    <View
                      style={[styles.dot, { backgroundColor: agent.color || colors.gray500 }]}
                    />
                  ...
                  {isExpanded && (
                    <View style={styles.expandedSection}>
                      ...
                      <TextInput
                        value={editForm.name}
                        onChangeText={(v: any) => setEditForm({ ...editForm, name: v })}
                        style={styles.formInput}
                        placeholderTextColor={colors.gray500}
                      />
                      <SharedEnginePicker
                      ...
                      <TextInput
                        ...
                        onChangeText={(v: any) => setEditForm({ ...editForm, systemPrompt: v })}
                        style={[styles.formInput, { minHeight: 80 }]}
                        multiline
                        textAlignVertical="top"
                        placeholderTextColor={colors.gray500}
                      ...
                      <DevToggle
                        ...
                        onChange={(isDev: any) => setEditForm({ ...editForm, isDev })}
