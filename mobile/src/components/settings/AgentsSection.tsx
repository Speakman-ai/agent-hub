...
function ChipRow({ options, selected, onSelect, labelFor = (o: any) => o }: any) {
    return (<View style={styles.chipRow}>
      {options.map((opt: any) => {
            const active = selected === opt;
            return (<TouchableOpacity key={String(opt)} style={[styles.chip, active && styles.chipActive]} onPress={() => onSelect(opt)}>
...
function DevToggle({ value, onChange, locked = false, lockedOn = false, fallbackOn = false }: any) {
    ...
    return (<>
      ...
      <TouchableOpacity style={[styles.chip, on && styles.chipActive, locked && { opacity: 0.6 }]} onPress={() => !locked && onChange(!on)} disabled={locked}>
...
function PerUserEngineModelPickers({ modelConfig, sharedEngine, engineOverride, modelOverride, onEngineOverride, onModelOverride, overrideSaving, }: any) {
    ...
    return (<View style={styles.onlyForMeBox}>
      <Text style={styles.onlyForMeTitle}>Only for me</Text>
      {engines.length > 0 && (<>
          <Text style={styles.fieldLabel}>Engine (only for me)</Text>
          <ChipRow options={[SHARED_ENGINE_PICK, ...engines]} selected={engineOverride ? engineOverride : SHARED_ENGINE_PICK} onSelect={(eng: any) => onEngineOverride(eng === SHARED_ENGINE_PICK ? '' : eng)} labelFor={(eng: any) => eng === SHARED_ENGINE_PICK ? `Shared (${sharedEngine || 'claude-code'})` : eng}/>
        </>)}
      {models.length > 0 && (<>
          ...
          <ChipRow options={[PER_USER_DEFAULT_MODEL, ...models]} selected={selectedModelChip} onSelect={(chip: any) => onModelOverride(settingsResolveModelChip(chip))} labelFor={(m: any) => m === PER_USER_DEFAULT_MODEL
...
function BulkEngineModelPickers({ modelConfig, engine, model, onEngine, onModel }: any) {
    ...
    return (<>
      ...
      {models.length > 0 && (<>
          ...
          <ChipRow options={models} selected={model || defaultModel} onSelect={onModel} labelFor={(m: any) => m.replace(/^claude-/, '').replace(/^gpt-/, '')}/>
        </>)}
    </>);
}
export default function AgentsSection({ projectId: filterProjectId, hideBulk = false }: any = {}) {
    ...
    const loadOverrides = useCallback(async () => {
        try {
            ...
            setModelOverrides(modelBody?.agentModelOverrides && typeof modelBody.agentModelOverrides === 'object'
                ? modelBody.agentModelOverrides
                : {});
            const raw = engineBody?.agentEngineOverrides && typeof engineBody.agentEngineOverrides === 'object'
                ? engineBody.agentEngineOverrides
                : {};
            const flat: Record<string, any> = {};
            for (const [id, entry] of Object.entries(raw)) {
                const row = entry as any;
                if (row && typeof row.engine === 'string')
                    flat[id] = row.engine;
            }
    ...
    const groups = useMemo<any>(() => {
        ...
        if (!filterProjectId)
            return all;
        return all
            .filter((g: any) => g.projectId === filterProjectId)
            .map((g: any) => ({
            ...g,
    ...
    const saveModelOverride = async (agentId: any, model: any) => {
        setModelOverrides((prev: any) => {
            const next = { ...prev };
            if (model)
                next[agentId] = model;
            else
                delete (next as any)[agentId];
            return next;
        });
        setOverrideSaving((prev: any) => ({ ...prev, [agentId]: true }));
        try {
            const body = model
            ...
            setOverrideSaving((prev: any) => ({ ...prev, [agentId]: false }));
        }
    };
    const saveEngineOverride = async (agentId: any, engine: any, sharedEngine: any = 'claude-code') => {
        ...
        const mustClearModel = settingsModelOverrideIsStale(modelOverrides[agentId] || '', effectiveEngine, modelConfig);
        setEngineOverrides((prev: any) => {
            const next = { ...prev };
            if (engine)
                next[agentId] = engine;
            else
        ...
        try {
            ...
            const raw = body?.agentEngineOverrides && typeof body.agentEngineOverrides === 'object'
    ...
    const handleBulkApplyAll = () => {
        ...
        Alert.alert('Switch all agents', `Set your personal defaults for every agent to ${bulkEngine} / ${effectiveModel}? This only affects your sessions.`, [
            { text: 'Cancel', style: 'cancel' },
            {
                text: 'Apply',
                onPress: async () => {
    ...
    return (<View>
      <View style={styles.sectionHeaderRow}>
        ...
        <TouchableOpacity style={styles.headerButton} onPress={() => setShowNew((v: any) => !v)}>
      ...
      {agents.length > 0 && modelConfig && !hideBulk && !filterProjectId && (<View style={styles.formCard}>
          ...
          <BulkEngineModelPickers modelConfig={modelConfig} engine={bulkEngine} model={bulkModel} onEngine={(engine: any) => {
                setBulkEngine(engine);
                setBulkModel('');
            }} onModel={setBulkModel}/>
          <TouchableOpacity style={[styles.primaryBtn, bulkSaving && { opacity: 0.5 }]} onPress={handleBulkApplyAll} disabled={bulkSaving}>
      ...
      {showNew && (<View style={styles.formCard}>
          ...
          <TextInput value={newForm.id} onChangeText={(v: any) => setNewForm({ ...newForm, id: v })} placeholder="e.g. my-project-helper" placeholderTextColor={colors.gray500} style={styles.formInput} autoCapitalize="none" autoCorrect={false}/>
          <Text style={styles.fieldLabel}>Name (optional — defaults to ID)</Text>
          <TextInput value={newForm.name} onChangeText={(v: any) => setNewForm({ ...newForm, name: v })} placeholder="Display name" placeholderTextColor={colors.gray500} style={styles.formInput}/>
          {!filterProjectId && (<>
              <Text style={styles.fieldLabel}>Project</Text>
              <ChipRow options={projects.map((p: any) => p.id)} selected={newForm.projectId} onSelect={(projectId: any) => setNewForm({ ...newForm, projectId })} labelFor={(id: any) => projects.find((p: any) => p.id === id)?.name || id}/>
            </>)}
          <SharedEnginePicker modelConfig={modelConfig} engine={newForm.engine} onEngine={(engine: any) => setNewForm({ ...newForm, engine })} label="Engine (shared default)"/>
          ...
          <DevToggle value={newForm.isDev} onChange={(isDev: any) => setNewForm({ ...newForm, isDev })}/>
          <TouchableOpacity style={[styles.primaryBtn, creating && { opacity: 0.5 }]} onPress={handleCreate} disabled={creating}>
            <Text style={styles.primaryBtnText}>{creating ? 'Creating…' : 'Create Agent'}</Text>
          </TouchableOpacity>
        </View>)}

      {groups.map((group: any) => (<View key={group.projectId || 'other'} style={styles.projectGroup}>
          {!filterProjectId && (<View style={styles.projectHeader}>
          ...
          {group.agents.length === 0 ? (<Text style={styles.emptyText}>No agents in this project</Text>) : (group.agents.map((agent: any) => {
                ...
                return (<View key={agent.id} style={styles.card}>
                  ...
                  {isExpanded && (<View style={styles.expandedSection}>
                      ...
                      <TextInput value={editForm.name} onChangeText={(v: any) => setEditForm({ ...editForm, name: v })} style={styles.formInput} placeholderTextColor={colors.gray500}/>
                      <SharedEnginePicker modelConfig={modelConfig} engine={editForm.engine} onEngine={(engine: any) => setEditForm({ ...editForm, engine })}/>
                      <PerUserEngineModelPickers modelConfig={modelConfig} sharedEngine={editForm.engine || agent.engine || 'claude-code'} engineOverride={engineOverrides[agent.id] || ''} modelOverride={modelOverrides[agent.id] || ''} onEngineOverride={(engine: any) => saveEngineOverride(agent.id, engine, editForm.engine || agent.engine || 'claude-code')} onModelOverride={(model: any) => saveModelOverride(agent.id, model)} overrideSaving={!!overrideSaving[agent.id]}/>
                      ...
                      <View style={styles.actionRow}>
                        ...
                        <TouchableOpacity style={[styles.primaryBtn, saving && { opacity: 0.5 }]} onPress={() => handleSave(agent)} disabled={saving}>
      ...
      {!loadError && groups.length === 0 && (<Text style={styles.emptyText}>No projects configured yet.</Text>)}
    </View>);
}
