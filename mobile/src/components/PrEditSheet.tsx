...
export default function PrEditSheet({ visible, pr, onClose, onSubmit }: any) {
    ...
    return (<PrActionSheet visible={visible} title={`Edit PR #${pr?.number ?? ''}`} submitLabel="Save" onSubmit={submit} onClose={onClose} busy={busy} submitDisabled={!title.trim()} error={error}>
      <TextInput style={styles.titleInput} value={title} onChangeText={(t: any) => {
            setTitle(t);
            if (error)
                setError(null);
        }} placeholder="PR title" placeholderTextColor={colors.gray500} editable={!busy}/>
      <TextInput style={styles.bodyInput} value={body} onChangeText={setBody} placeholder="Description (markdown)" placeholderTextColor={colors.gray500} multiline editable={!busy}/>
    </PrActionSheet>);
}
