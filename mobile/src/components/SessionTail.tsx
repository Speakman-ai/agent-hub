...
function ToolResultImages({ images }: any) {
  ...
  return (
    <View style={toolImageStyles.row} testID="tool-result-images">
...
function ExploredChip({ items }: any) {
  ...
  return (
    <View style={exploredStyles.wrap}>
      <TouchableOpacity
        style={exploredStyles.header}
      ...
      {open ? (
        <View style={exploredStyles.list}>
          {items.map((it: any, i: any) => {
            const d = describeTool(it.use?.tool, it.use?.input);
            return (
              <View key={`ex${i}`} style={exploredStyles.listRow}>
      ...
      {todos.length > 0 ? (
        <View style={todoStyles.list}>
          {visible.map((t: any, i: any) => {
            ...
            return (
              <View key={`todo${i}`} style={todoStyles.row}>
...
function ScheduleWakeupCard({ use, result, scheduledAt }: any) {
  ...
  return (
    <View
      style={[wakeupStyles.card, countdown.state === 'due' && wakeupStyles.cardDue]}
      testID="schedule-wakeup-card"
    >
      <TouchableOpacity
        style={wakeupStyles.header}
        onPress={() => setOpen((v: any) => !v)}
        accessibilityRole="button"
      >
        ...
        {countdown.label ? (
          <Text
            style={[wakeupStyles.countdown, countdown.state === 'due' && wakeupStyles.countdownDue]}
            testID="schedule-wakeup-countdown"
          >
            {countdown.label}
          </Text>
        ) : null}
      ...
      {open ? (
        <View style={wakeupStyles.body}>
          {wakeup.reason ? <Text style={wakeupStyles.reason}>{wakeup.reason}</Text> : null}
          {wakeup.firesAtMs !== null ? (
            <Text style={wakeupStyles.times} testID="schedule-wakeup-times">
              Scheduled {formatTime(scheduledAt)} · fires {formatTime(wakeup.firesAtMs)}
            </Text>
          ) : null}
          {wakeup.prompt ? (
            <Text style={wakeupStyles.prompt} numberOfLines={8}>
              {wakeup.prompt}
            </Text>
          ) : null}
          {resultPanel ? (
            <View
              style={[wakeupStyles.codeBox, resultPanel.errored && wakeupStyles.errorCodeBox]}
              testID="schedule-wakeup-result"
            >
              <Text style={wakeupStyles.codeLabel}>{resultPanel.label}</Text>
              <Text
                style={[wakeupStyles.codeText, resultPanel.errored && wakeupStyles.codeTextError]}
              >
                {resultPanel.text}
              </Text>
            </View>
    ...
    <View style={planStyles.card}>
      ...
      {open ? (
        <View style={planStyles.body}>
          {planText ? (
            <Markdown style={planMarkdownStyles as any}>{planText}</Markdown>
          ) : (
            <Text style={planStyles.empty}>(empty plan)</Text>
          )}
          {!stillRunning && declined ? (
            <Text style={planStyles.note}>
...
function SessionTail({
  ...
  const askBlocks = useMemo<any>(
    () => blocks.filter((b: any) => b.kind === 'ask_question'),
    [blocks],
  );
  const credentialBlocks = useMemo<any>(
    () => blocks.filter((b: any) => b.kind === 'credential_request'),
    [blocks],
  );
  ...
  const toolCount = blocks.filter((b: any) =>
    ['tool', 'subagent', 'explored', 'todos', 'plan_proposal', 'wakeup'].includes(b.kind),
  ).length;
      ...
      <View>
        {askBlocks.map((b: any) => (
          <AskUserQuestion
            key={b.event.askId}
            askId={b.event.askId}
            questions={b.event.questions || []}
            onSubmit={(text: any) => onAskSubmit?.(b.event.askId, text)}
            submitted={askSubmittedIds?.has(b.event.askId)}
          />
        ))}
        {credentialBlocks.map((b: any) => (
          <CredentialRequestPrompt
            key={b.event.request.requestId}
            sessionId={message?.session_id}
            request={b.event.request}
            onSubmit={(text: any) => onCredentialSubmit?.(b.event.request.requestId, text)}
          />
        ))}
        {browserPanel}
        {hasClassicMeta && (
          <TouchableOpacity style={styles.summaryBar} onPress={() => setExpanded(true)}>
            ...
            {resultBlock ? (
              <Text style={styles.summaryText}>
                ...
                {resultBlock.event?.costUsd
                  ? ` \u00B7 $${resultBlock.event.costUsd.toFixed(4)}`
                  : ''}
              </Text>
            ) : null}
              ...
              >
                ...
                <View style={styles.eventContent}>
                  <View style={styles.toolHeader}>
                    ...
                    {isBackgroundBash && (
                      <Text style={styles.backgroundBadge} testID="bash-background-badge">
                        background
                      </Text>
                    )}
                    {!hasResult && <Text style={styles.runningBadge}>running…</Text>}
                  ...
                  {arg ? (
                    <Text style={styles.argMono} numberOfLines={2}>
                      {arg}
                    </Text>
                  ) : null}
                  {block.result?.images?.length > 0 && (
                    <ToolResultImages images={block.result.images} />
                  )}
                  {isBlockExpanded && (
                    <ScrollView style={styles.expandedContent} nestedScrollEnabled>
                      {use.tool === 'Bash' && typeof use.input?.command === 'string' ? (
                        ...
                        <View style={styles.bashTerminal} testID="bash-terminal">
                          ...
                          {isBackgroundBash ? (
                            <Text style={styles.bashBackgroundNote} testID="bash-background-note">
                              Launched in the background. This shell is a child of this turn's CLI,
                              so it can't be monitored or resumed after the turn ends. To track work
                              across turns, start it as a Hub-owned background shell (the bg.sh
                              wrapper) — it appears in the Background shells panel with
                              status/logs/stop — or use Start preview / the Terminal tab.
                            </Text>
                          ) : null}
                          {block.result?.output ? (
                            <Text style={[styles.bashOutput, isError && { color: colors.red400 }]}>
