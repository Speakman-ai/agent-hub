...
export default function FinalizeChecksCard({ steps, round }: any) {
    ...
    return (<View style={styles.card} testID="finalize-checks-card">
      ...
      <View style={styles.steps}>
        {list.map((s: any, i: any) => (<View key={`${s.name || 'step'}-${i}`} style={styles.stepRow}>
            <StepIcon state={s.state}/>
            <Text style={styles.stepName} numberOfLines={1}>
              {s.name || `Step ${i + 1}`}
            </Text>
            {typeof s.exitCode === 'number' && s.state === 'failed' ? (<Text style={styles.exit}>exit {s.exitCode}</Text>) : null}
          </View>))}
      </View>
    </View>);
}
