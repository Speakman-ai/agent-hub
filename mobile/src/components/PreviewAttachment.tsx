...
export default function PreviewAttachment({ event, onRetry, onTouch }: any) {
    ...
    if (kind === 'preview_unavailable') {
        const headline = event.unavailableReason === 'preview-disabled'
        ...
        {event.agentReason ? (<Text style={styles.unavailableReason}>The agent asked for: {event.agentReason}</Text>) : null}
        {event.wizardUrl ? (<TouchableOpacity style={styles.button} onPress={() => Linking.openURL(event.wizardUrl).catch(() => { })}>
    ...
    if (kind === 'preview_failed') {
        ...
        {Array.isArray(event.logTail) && event.logTail.length > 0 ? (<View>
            <TouchableOpacity onPress={() => setShowLogs((v: any) => !v)}>
              <Text style={styles.toggle}>{showLogs ? 'Hide logs' : 'Show logs'}</Text>
            </TouchableOpacity>
            {showLogs ? (<ScrollView horizontal style={styles.logBox}>
                <Text style={styles.logText}>{event.logTail.join('\n')}</Text>
              </ScrollView>) : null}
          </View>) : null}
        {onRetry ? (<TouchableOpacity style={styles.button} onPress={() => onRetry(event)}>
    ...
    return (<View style={styles.ready}>
      ...
      {event.screenshotPath ? (<Image source={{ uri: event.screenshotPath }} style={styles.screenshot} resizeMode="contain"/>) : null}
      <View style={styles.chipRow}>
        {renderUrl ? (<TouchableOpacity style={styles.button} onPress={() => Linking.openURL(renderUrl).catch(() => { })}>
            <Text style={styles.buttonText}>Open in browser</Text>
          </TouchableOpacity>) : null}
        {onTouch ? (<TouchableOpacity style={styles.buttonSecondary} onPress={() => onTouch(event)}>
...
function Chip({ label, tone }: any) {
    ...
    return (<View style={[styles.chip, { backgroundColor: p.bg }]}>
