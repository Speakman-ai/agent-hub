...
export default function FinalizeSection({ navigation, fixedProjectId }: any) {
    const { projects, refreshProjects, setActiveAgentId, setActiveSessionId, lastFinalizeWizardEvent, } = useApp();
    const [projectId, setProjectId] = useState(() => fixedProjectId || pickInitialProjectId(projects, ''));
    ...
    useEffect(() => {
        if (shouldRefreshOnWizardComplete(lastFinalizeWizardEvent, projectId) &&
    ...
    return (<View>
      ...
      {!fixedProjectId && (<>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.chipRow} testID="finalize-project-picker">
            {projects.map((p: any) => {
                const active = p.id === projectId;
                return (<TouchableOpacity key={p.id} onPress={() => setProjectId(p.id)} style={[styles.chip, active && styles.chipActive]} testID={`finalize-project-${p.id}`}>
                  <View style={[
                        styles.chipDot,
      ...
      <View style={styles.card}>
        ...
        {lastSessionId && (<Text style={styles.successInline} testID="finalize-last-session">
            Last wizard session: <Text style={styles.mono}>{lastSessionId}</Text>
          </Text>)}

        <TouchableOpacity onPress={handleStartWalkthrough} disabled={!project || wizardStarting} style={[
            styles.primaryButton,
            (!project || wizardStarting) && styles.primaryButtonDisabled,
        ]} testID="finalize-start-button">
          {wizardStarting ? (<ActivityIndicator size="small" color={colors.white}/>) : (<Text style={styles.primaryButtonText}>
              {lastSessionId ? 'Re-run wizard' : 'Set up Runner'}
            </Text>)}
        </TouchableOpacity>

        {lastSessionId && (<TouchableOpacity onPress={handleOpenWizardChat} style={styles.secondaryButton} testID="finalize-open-chat-button">
            <Text style={styles.secondaryButtonText}>Open wizard chat</Text>
          </TouchableOpacity>)}

        {wizardError && (<Text style={styles.errorInline} testID="finalize-error">
            {wizardError}
          </Text>)}
      </View>
