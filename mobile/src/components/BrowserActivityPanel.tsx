...
function runningGlyph(row: any, streaming: any) {
    ...
    if (pending)
        return { char: '\u2022', color: colors.blue400 };
    if (row.phase === 'done' && row.ok)
        return { char: '\u2713', color: colors.emerald400 };
    if (row.phase === 'done' && row.ok === false)
        return { char: '\u2715', color: colors.rose400 };
...
export default function BrowserActivityPanel({ timelineEntries, streaming, screenshots }: any) {
    ...
    return (<View style={styles.wrap} testID="browser-activity-panel">
      {hasLive && (<View style={styles.hintRow}>
          <Text style={styles.globe}>{'\uD83C\uDF10'}</Text>
          <Text style={styles.hint}>{hint}</Text>
        </View>)}
      <TouchableOpacity accessibilityRole="button" accessibilityState={{ expanded: showDetail }} onPress={() => setOpen((v: any) => !v)}>
      ...
      {showDetail ? (<ScrollView nestedScrollEnabled style={styles.listScroll} contentContainerStyle={styles.listContent}>
          {rows.map((row: any) => {
                const aid = row.actionId;
                const caption = row.phase === 'done' ? row.summary || row.startedLabel : row.startedLabel || row.op;
                const g = runningGlyph(row, streaming);
                const shot = screenshots?.[aid];
                const duration = row.durationMs != null && row.phase === 'done' ? ` ${row.durationMs}ms` : '';
                return (<View key={aid} style={styles.row}>
                <Text style={[styles.glyph, { color: g.color }]}>
                  {g.char}
                </Text>
                <View style={styles.rowBody}>
                  ...
                  {row.targetSummary ? (<Text style={styles.mono} selectable>
                      {row.targetSummary}
                    </Text>) : null}
                  {row.error ? <Text style={styles.err}>{row.error}</Text> : null}
                  {row.extractPreview ? (<Text style={styles.extract} selectable numberOfLines={8}>
                      {row.extractPreview}
                    </Text>) : null}
                  {shot ? (<Image source={{ uri: shot }} accessibilityLabel="Browser screenshot preview" style={styles.shot} resizeMode="contain"/>) : null}
                  {!shot && row.hasScreenshot ? (<Text style={styles.muted}>
