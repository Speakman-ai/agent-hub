...
function TodoListCard({ use, result }) {
...
function PlanProposalCard({ use, result }) {
  ...
  return (
    <View style={planStyles.card}>
      <TouchableOpacity style={planStyles.header} onPress={() => setOpen((v) => !v)}>
        ...
        {!stillRunning && declined ? (
          <Text style={planStyles.await}>awaiting review</Text>
        ) : null}
      ...
      {open ? (
        <View style={planStyles.body}>
          {planText ? (
            <Markdown style={planMarkdownStyles}>{planText}</Markdown>
          ) : (
      ...
      <View>
        ...
        {askBlocks.map((b) => (
          <AskUserQuestion
            ...
            onSubmit={(text) => onAskSubmit?.(b.event.askId, text)}
