...
import { parseDiffLines, shortenPath, diffHasDisplayableLines, isExplicitEmptyWrite, } from '../utils/diff';
 ...
 */
function DiffView({ tool, input }: any) {
    ...
    const addedCount = additions.filter((l: any) => l && l.trim()).length;
    const removedCount = removals.filter((l: any) => l && l.trim()).length;
    ...
    const diffBody = !hasLines ? (<Text style={styles.emptyHint}>{placeholderText}</Text>) : showFull ? (<ScrollView style={styles.fullScroll} nestedScrollEnabled>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} nestedScrollEnabled>
        <View>
          {removals.map((line: any, i: any) => (<View key={`r${i}`} style={styles.removalRow}>
              <Text style={styles.removalGutter}>-</Text>
              <Text style={styles.removalText}>{line || ' '}</Text>
            </View>))}
          {additions.map((line: any, i: any) => (<View key={`a${i}`} style={styles.additionRow}>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} nestedScrollEnabled>
        <View>
          {previewLines.slice(0, DIFF_PREVIEW_LINES).map((line: any, i: any) => (<View key={`p${i}`} style={previewKind === 'add' ? styles.previewAddRow : styles.previewRemoveRow}>
      ...
      {hiddenCount > 0 ? (<TouchableOpacity style={styles.expandFooter} onPress={() => setExpanded(true)} accessibilityRole="button" testID="diff-view-expand">
