...
export default function TopBar() {
    ...
    const handleExtractSkill = async () => {
        if (!activeSessionId || extractingSkill)
            return;
        setExtractingSkill(true);
        try {
            await api.extractSkillFromSession(activeSessionId);
            Alert.alert('Turning session into a skill', 'Skill Builder is drafting a skill from this session. Open the new "[Skill from] …" session to review and save it.', [{ text: 'OK' }]);
        }
        catch (err: any) {
    ...
    const currentModel = engineModels.find((m: any) => m.id === sessionModel) ||
        engineModels[0] ||
    ...
    return (<View style={styles.container}>
      <View style={styles.left}>
        ...
        {activeAgent && (<View style={styles.agentInfo}>
            <View style={[styles.agentDot, { backgroundColor: activeAgent.color }]}/>
            {activeSessionId ? (<SessionStateIcon state={activeSessionState} size={14} testID="topbar-session-state-icon"/>) : null}
            <View style={styles.agentText}>
              <Text style={styles.agentName} numberOfLines={1}>
                {activeAgent.name}
              </Text>
              {activeSessionId ? (<TouchableOpacity onPress={handleCopySessionId} accessibilityRole="button" accessibilityLabel={`Copy session id ${activeSessionId}`} style={styles.sessionIdBadge}>
      ...
      <View style={styles.right}>
        ...
        {/* Engine + Model button */}
        {activeAgent && (<TouchableOpacity style={styles.engineButton} onPress={() => setShowPicker(true)}>
        ...
        {activeAgent && (<TouchableOpacity style={styles.newButton} onPress={handleSummarize} disabled={!activeSessionId || summarizing} accessibilityRole="button" accessibilityLabel="Summarize session and copy to clipboard">
            {summarizing ? (<ActivityIndicator size="small" color={colors.white}/>) : (<AppIcon name="document-text-outline" size={18} color={activeSessionId ? colors.white : colors.gray500}/>)}
          </TouchableOpacity>)}

        {/* Turn this session into a skill — Skill Builder Phase 4 */}
        {activeAgent && (<TouchableOpacity style={styles.newButton} onPress={handleExtractSkill} disabled={!activeSessionId || extractingSkill} accessibilityRole="button" accessibilityLabel="Turn this session into a skill">
            {extractingSkill ? (<ActivityIndicator size="small" color={colors.white}/>) : (<AppIcon name="bulb-outline" size={18} color={activeSessionId ? colors.white : colors.gray500}/>)}
          </TouchableOpacity>)}

      </View>

      {/* Forward Session Modal */}
      <ForwardSessionModal visible={showForward} sourceAgent={activeAgent} agents={agents || []} sessionId={activeSessionId} onClose={() => setShowForward(false)} onForward={({ targetAgentId, prompt, autoStart }: any) => api.forwardSession(activeSessionId, { targetAgentId, prompt, autoStart })} onForwarded={(result: any) => {
            const session = result?.session;
            if (!session)
                return;
      ...
      <SessionEngineModelSheet visible={showPicker} onClose={() => setShowPicker(false)} modelConfig={modelConfig} engine={sessionEngine} model={sessionModel} reasoningEffort={sessionReasoningEffort} onSelectEngine={handleEngineChange} onSelectModel={handleModelChange} onSelectReasoningEffort={handleReasoningEffortChange}/>
    </View>);
}
