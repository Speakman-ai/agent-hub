        ...
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          <View style={styles.card}>
            ...
            <Text style={styles.subtitle}>
              Enter the address of the Agent Hub server you want to use. You'll
              sign in on the next screen. You can find the address in the web app
              under Settings → Server Connections.
            </Text>

            <Text style={styles.inputLabel}>Server address</Text>
            <TextInput style={styles.textInput} value={remoteUrl} onChangeText={(v: any) => {
            setRemoteUrl(v);
            setSaveError(null);
        }} placeholder="https://my-server.example.com:3051" placeholderTextColor={colors.gray500} autoCapitalize="none" autoCorrect={false} keyboardType="url" returnKeyType="go" onSubmitEditing={() => {
            if (canContinue)
                handleContinue();
        }}/>
            ...
            <TouchableOpacity style={[styles.primaryBtn, !canContinue && styles.btnDisabled]} onPress={handleContinue} disabled={!canContinue}>
              {saving ? (<ActivityIndicator size="small" color={colors.white}/>) : (<Text style={styles.primaryBtnText}>Continue to sign in</Text>)}
            </TouchableOpacity>
          </View>
        </ScrollView>
