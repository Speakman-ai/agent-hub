# Example: Post a Heartbeat Summary

**Scenario:** you run as a scheduled heartbeat (e.g. every 6 hours). At the end
...
## Input (the heartbeat fires automatically)

The heartbeat's configured prompt is something like:
...
```bash
scripts/heartbeats.sh thread agent-hub-lead \
...
```
2026-04-18T23:00:12Z - Heartbeat: 0 stuck cards, 1 failing PR (#438 — typecheck).
2026-04-19T05:00:04Z - Heartbeat: 1 stuck card (b7e2…), 0 failing PRs.
```
...
PROJECT_ID=agent-hub scripts/board.sh comment "b7e2aa31-…" '{
  "author": "agent-hub-lead-heartbeat",
  "content": "Heartbeat sweep 2026-04-19T11:00Z: this card has been In Progress >24h with no pr_url. Requesting status from the current assignee."
}'
```
...
```json
{"id":"…","card_id":"b7e2aa31-…","author":"agent-hub-lead-heartbeat","content":"…","created_at":"2026-04-19T11:00:42.000Z"}
```

### 4. End your turn with the one-line summary

The final assistant message in your heartbeat session becomes the thread entry
that the **next** run will read in step 1. Keep it structured so future runs
can diff against it:

```
Heartbeat 2026-04-19T11:00Z | stuck: 1 (b7e2aa31) | review: 3 | failing PRs: 0 | action: commented on stuck card
```
...
- [x] Read previous thread via `scripts/heartbeats.sh thread <agentId>`
...
- **Heartbeat thread is read-only via the API.** You don't POST to the thread
...
- **Heartbeats have short context budgets.** Don't re-read every card's full
  description — filter with `--column` / jq before inspecting.
- **`scripts/heartbeats.sh run <agentId>`** triggers a run *now*; don't call
  this from inside a heartbeat session or you'll recurse.
